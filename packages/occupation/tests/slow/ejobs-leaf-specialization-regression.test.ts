...
let productionDefaultPipeline: OccupationSearchPipeline;
let leafStructurePipeline: OccupationSearchPipeline;
let rows: EjobsRow[];
...
test('ejobs sample: selected=yes titles keep ranking their recorded top leaf', async () => {
  ...
  for (const row of rows) {
    ...
    const rankedOrSelectedLeaf =
      (primarySpan.decision.decisionType === 'leaf' ? primarySpan.decision.selectedLabel : null) ||
...
test('ejobs sample: winning leaves stay within a bounded unsupported-specialization budget', async () => {
  ...
  for (const row of rows) {
    ...
    const hasUnsupportedSpecialization = structure.specializationKinds.some(
      (kind) => !preparedQuerySupportsSpecializationKind(result.preparedQuery, kind)
    );
  ...
  assert.ok(
    unsupportedCount <= MAX_UNSUPPORTED_SPECIALIZATION_WINNERS,
    `unsupported-specialization winners = ${unsupportedCount}, budget = ${MAX_UNSUPPORTED_SPECIALIZATION_WINNERS}`
  );
});
...
function firstSpan(
  result: Pick<
    import('../../src/search-pipeline/occupation-search-pipeline.js').OccupationSearchPipelineResult,
    'spanResults' | 'decision' | 'coverageStatus' | 'rankedFamilies'
  >
) {
...
async function loadFixtureRows(): Promise<EjobsRow[]> {
  const parser = createReadStream(FIXTURE_PATH, { encoding: 'utf8' }).pipe(
    parse({ bom: true, columns: true, relax_column_count: true, skip_empty_lines: true })
  ) as AsyncIterable<Record<string, string>>;

  const seen = new Set<string>();
  const parsed: EjobsRow[] = [];
