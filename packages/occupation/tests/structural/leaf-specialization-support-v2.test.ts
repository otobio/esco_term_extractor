...
import {
  buildSpecializationDimensionProfile,
  compareSpecializationDimensionProfiles
} from '../../src/runtime/occupation-leaf-structure-rules-v2.js';
...
async function scoreContradictionCase(
  prepared: Awaited<ReturnType<typeof prepareQuery>>,
  ...
  return scoreLeaf(
    closeness,
    [],
    genericLeaf(canonicalLabel, graphNodeId),
    prepared,
    canonicalTokens,
    canonicalTokens,
    new Set(),
...
function genericLeaf(canonicalLabel: string, graphNodeId: number): OccupationLeafStructureRecord {
  return {
    graphNodeId,
    canonicalLabel,
    familyNodeId: 14796,
