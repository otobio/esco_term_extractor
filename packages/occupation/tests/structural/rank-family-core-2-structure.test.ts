...
import { rankFamilyCandidatesForRecovery, type FamilyRankingScore } from '../../src/cli/rank-family-core-2.js';
import type {
  PipelineEvidenceRecord,
  PipelineFamilyCandidate,
  PipelineLeafCandidate,
  RankedPipelineFamily
} from '../../src/cli/rank-family-core.js';
import { prepareQuery } from '../../src/query/query-preparation.js';

const SOURCE = 'esco_1_2_1';
const NO_LEAVES = new Map<string, readonly PipelineLeafCandidate[]>();
...
function familyCandidate(
  ...
  familyLabel: string,
  confidence: number,
  evidenceRecords: readonly PipelineEvidenceRecord[]
): PipelineFamilyCandidate {
  return {
...
function evidence(channel: PipelineEvidenceRecord['channel'], score: number, details: Record<string, unknown>): PipelineEvidenceRecord {
  return {
    channel,
    score,
    sourceStage: 'test',
...
function rankedWithScores(
  ranked: readonly RankedPipelineFamily[]
): readonly (RankedPipelineFamily & { rankingScore?: FamilyRankingScore })[] {
  return ranked as readonly (RankedPipelineFamily & { rankingScore?: FamilyRankingScore })[];
}
