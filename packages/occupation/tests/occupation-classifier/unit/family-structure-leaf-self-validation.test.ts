...
import {
  getFamilyStructureQueryRoleHeads,
...
before(async () => {
  ...
  leaves = searchMeta
    ...
    .map((record) => ({ canonicalLabel: record.canonicalLabel, familyNodeId: record.familyNodeId as number }));
});
...
test('every real leaf keeps its true family structurally viable against its own title', () => {
  ...
  const trueFamilyRejected: string[] = [];
  const wrongFamilyOnlyAccepted: string[] = [];
  ...
  assert.ok(
    trueFamilyRejected.length <= MAX_TRUE_FAMILY_REJECTED,
    `expected at most ${MAX_TRUE_FAMILY_REJECTED} leaves whose true family is structurally rejected by their own title, found ${trueFamilyRejected.length}`
  );
  assert.ok(
    wrongFamilyOnlyAccepted.length <= MAX_WRONG_FAMILY_ONLY_ACCEPTED,
    `expected at most ${MAX_WRONG_FAMILY_ONLY_ACCEPTED} leaves where a wrong family is structurally accepted instead of the true family, found ${wrongFamilyOnlyAccepted.length}`
  );
});
