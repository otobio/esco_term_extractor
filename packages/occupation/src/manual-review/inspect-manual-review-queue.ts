...
import { formatTable, type ManualReviewType } from './build-manual-review-queue.js';
...
type ReviewQueueQueryRow = RowDataPacket & {
  id: number;
  review_type: ManualReviewType;
  review_status: ManualReviewQueueStatus;
...
export class ManualReviewQueueInspector {
  ...
  public async run(options: InspectManualReviewQueueOptions = {}): Promise<InspectManualReviewQueueRow[]> {
    ...
    const [rows] = await this.connection.query<ReviewQueueQueryRow[]>(
      `
        SELECT
          review.id,
          review.review_type,
