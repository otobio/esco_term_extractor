...
export class RetrievalBoundaryDebugCollector {
  private readonly aliasNgramRetained: AliasNgramAuditEntry[] = [];
  private readonly aliasNgramSuppressed: AliasNgramAuditEntry[] = [];
  private readonly lexicalContextOnlyAdmissions: LexicalAdmissionAuditEntry[] = [];
  private readonly lexicalContextOnlyAdmissionKeys = new Set<string>();

  public constructor(
    private readonly enabled: boolean,
    private readonly limit: number = 24
  ) {}

  public collectAliasNgramRetained(
    preparedQuery: PreparedQuery,
    entry: Omit<
      AliasNgramAuditEntry,
      'matchedRoleTokens' | 'matchedDomainTokens' | 'roleGrounded' | 'contextOnly' | 'suppressed' | 'suppressionReason'
    >
  ): void {
  ...
  public collectAliasNgramSuppressed(
    preparedQuery: PreparedQuery,
    entry: Omit<
      AliasNgramAuditEntry,
      'matchedRoleTokens' | 'matchedDomainTokens' | 'roleGrounded' | 'contextOnly' | 'suppressed' | 'suppressionReason' | 'score'
    >
  ): void {
  ...
  public collectLexicalAdmission(preparedQuery: PreparedQuery, hit: OccupationTextHit): void {
    ...
    const supportFields = uniqueSorted(
      hit.fieldSignals
  ...
  public snapshot(): RetrievalBoundaryDebugSnapshot {
    return {
      aliasNgram: {
...
function classifyIntentMatch(
  preparedQuery: PreparedQuery,
  aliasRole: string,
  matchedTokens: string[]
): Pick<AliasNgramAuditEntry, 'matchedRoleTokens' | 'matchedDomainTokens' | 'roleGrounded' | 'contextOnly'> {
  ...
  const roleGrounded =
    preparedQuery.intent.roleTokens.length === 0 || !isSupportingAliasRole(aliasRole)
