...
import { DEFAULT_ESCO_SOURCE_NAME, DEFAULT_RETRIEVAL_LOCALE, type RetrievalProfile } from '../retrieval/occupation-candidates.js';
import { prepareOccupationRetrievalQuery } from '../query/occupation-retrieval-query.js';
import {
  analyzeOccupationSemanticSurface,
...
type BranchStats = {
  ...
  branchMarginByKey: Map<string, number | null>;
  leafShareByNodeId: Map<number, number>;
  leafMarginByNodeId: Map<number, number | null>;
};

export const DEFAULT_RESOLVER_WEIGHTS: ResolverScoringWeights = {
  exactness: 0.34,
  specificity: 0.18,
...
export class OccupationResolver {
  public constructor(private readonly branchRetriever: OccupationCandidateBranchRetriever = new OccupationCandidateBranchRetriever()) {}

  public async run(options: ResolveOccupationQueryOptions): Promise<ResolveOccupationQueryResult> {
    if (!options.query) {
...
function scoreBranch(branch: OccupationCandidateBranch, stats: BranchStats, queryIsGeneric: boolean): BranchResolutionScore {
  ...
  const score = roundScore(
    lexicalScore * DEFAULT_RESOLVER_WEIGHTS.exactness +
...
function scoreCandidate(
  ...
  const hierarchyConsistencyScore = scoreCandidateHierarchyConsistency(
    candidate,
    branchShare,
    branchMarginRatio,
  ...
  const unrelatedBranchPenalty = scoreUnrelatedBranchPenalty(branchShare, branchMarginRatio);
  const score = roundScore(
    exactnessScore * DEFAULT_RESOLVER_WEIGHTS.exactness +
...
function selectOutcome(
  ...
  queryIsGeneric: boolean,
  foldedQuery: string,
  broaderBranchFallback: BranchResolutionScore | null
): OccupationResolutionOutcome {
...
function selectCalibratedHybridLeaf(
  branchScores: BranchResolutionScore[],
  queryIsGeneric: boolean
): { branch: BranchResolutionScore; candidate: CandidateResolutionScore } | null {
  const candidates = branchScores.flatMap((branch) =>
    ...
    candidates
      ...
      .sort(
        (left, right) =>
          ...
          left.candidate.canonicalLabel.localeCompare(right.candidate.canonicalLabel)
      )[0] ?? null
  );
}

function isCalibratedHybridLeaf(branch: BranchResolutionScore, candidate: CandidateResolutionScore, queryIsGeneric: boolean): boolean {
  ...
  if (queryIsGeneric) {
    return (
      candidate.evidenceTier === 'exact_alias' &&
...
function selectCalibratedSemanticCapabilityLeaf(
  branchScores: BranchResolutionScore[],
  queryIsGeneric: boolean
): { branch: BranchResolutionScore; candidate: CandidateResolutionScore } | null {
  if (queryIsGeneric) {
...
function selectTrustedLexicalLeaf(
  ...
  return (
    candidates
      ...
      .sort(
        (left, right) =>
          evidenceTierRank(right.candidate.evidenceTier) - evidenceTierRank(left.candidate.evidenceTier) ||
          right.candidate.score - left.candidate.score ||
...
function isTrustedLexicalLeaf(candidate: CandidateResolutionScore, queryIsGeneric: boolean, foldedQuery: string): boolean {
  if (candidate.evidenceTier !== 'exact_alias' && candidate.evidenceTier !== 'folded_alias') {
    return false;
  }
...
function isLeafSafe(topBranch: BranchResolutionScore, topCandidate: CandidateResolutionScore, queryIsGeneric: boolean): boolean {
...
function isBranchFallbackSafe(topBranch: BranchResolutionScore, queryIsGeneric: boolean, foldedQuery: string): boolean {
...
function isBroadRoleQuery(foldedQuery: string): boolean {
  const tokens = foldedQuery.split(/\s+/u).filter(Boolean);
  return tokens.some((token) => token === 'role' || token === 'roles' || token === 'job' || token === 'jobs');
}

function foldResolverText(value: string): string {
  return value.normalize('NFKC').trim().toLowerCase().replace(/\s+/gu, ' ').normalize('NFKD').replace(/\p{M}/gu, '');
}

function buildBranchStats(branches: OccupationCandidateBranch[]): BranchStats {
  const totalBranchScore = branches.reduce((sum, branch) => sum + branch.scoreSummary.totalCandidateScore, 0);
  const branchShareByKey = new Map<string, number>();
  const branchMarginByKey = new Map<string, number | null>();
  const leafShareByNodeId = new Map<number, number>();
  const leafMarginByNodeId = new Map<number, number | null>();
  ...
  for (const branch of branches) {
    ...
    branchMarginByKey.set(
      branch.branchKey,
      bestOtherBranch && bestOtherBranch.score > 0 ? roundScore(branchTotal / bestOtherBranch.score) : null
    );

    const branchCandidateTotal = branch.candidates.reduce((sum, candidate) => sum + candidate.totalScore, 0);
    const sortedCandidates = [...branch.candidates].sort(
      (left, right) => right.totalScore - left.totalScore || left.canonicalLabel.localeCompare(right.canonicalLabel)
    );
    ...
    for (const candidate of branch.candidates) {
      ...
      leafMarginByNodeId.set(
        candidate.graphNodeId,
        bestOtherCandidate && bestOtherCandidate.totalScore > 0 ? roundScore(candidate.totalScore / bestOtherCandidate.totalScore) : null
      );
    }
  }
...
function getBranchEvidenceTier(branch: OccupationCandidateBranch): EvidenceTier {
...
function getCandidateEvidenceTier(candidate: ExpandedOccupationCandidate): EvidenceTier {
...
function getEvidenceTierScore(evidenceTier: EvidenceTier): number {
  if (evidenceTier === 'exact_alias') {
    return 1;
  }
...
function scoreBranchEvidenceStrength(
  ...
  const hasWeakSignalEvidence =
    branch.scoreSummary.channelScores.openSearchLexical > 0 || branch.scoreSummary.channelScores.capabilityTask > 0;
...
function selectBroaderBranchRescue(
  branchScores: BranchResolutionScore[],
  preparedQuery: PreparedQuery,
  queryIsGeneric: boolean
): BranchResolutionScore | null {
...
function branchMatchesRoleHeadIntent(
  branch: BranchResolutionScore,
  roleHeadTokens: readonly string[],
  altRoleHeadTokens: readonly string[]
): boolean {
  ...
  return (
    roleHeadTokens.some((token) => canonicalTokens.has(foldSearchText(token))) ||
    altRoleHeadTokens.some((term) => canonicalTokens.has(term))
  );
}

async function applySemanticAdjustments(
  branchScores: BranchResolutionScore[],
  queryAnalysis: OccupationSemanticLexiconAnalysis,
  locale: string
): Promise<BranchResolutionScore[]> {
  if (!queryAnalysis.supportedLocale || branchScores.length === 0) {
    return branchScores;
  }

  const candidateAnalysisCache = new Map<string, Promise<OccupationSemanticLexiconAnalysis>>();
  const adjusted = await Promise.all(
    branchScores.map(async (branch) => {
      ...
      const adjustedCandidates = branch.candidates.map((candidate, index) =>
        index === 0
          ? {
              ...candidate,
...
async function getCandidateAnalysis(
  surface: string,
  locale: string,
  cache: Map<string, Promise<OccupationSemanticLexiconAnalysis>>
...
function chooseSemanticComparisonSurface(surface: string): string | null {
  const normalized = surface.trim();
  return normalized || null;
}

function semanticAdjustmentScore(comparison: { decision: 'help' | 'neutral' | 'hurt'; score: number }): number {
...
function toRankedBroaderBranch(branch: BranchResolutionScore): RankedBroaderBranch {
  return {
    branchKey: branch.branchKey,
...
function toRankedLeaf(branch: BranchResolutionScore, candidate: CandidateResolutionScore, rank: number): RankedOccupationLeaf {
  return {
    rank,
    graphNodeId: candidate.graphNodeId,
    canonicalLabel: candidate.canonicalLabel,
...
function compareRankedLeaves(left: RankedOccupationLeaf, right: RankedOccupationLeaf): number {
  return (
...
function scoreBranchSpecificity(branch: OccupationCandidateBranch, queryIsGeneric: boolean): number {
  const averageSpecificity =
    branch.candidates.reduce((sum, candidate) => sum + scoreGenericRiskAsSpecificity(candidate.genericRisk), 0) /
    Math.max(branch.candidates.length, 1);

  return clampScore(averageSpecificity - (queryIsGeneric ? 0.12 : 0));
}

function scoreGenericRiskAsSpecificity(risk: ExpandedOccupationCandidate['genericRisk']): number {
  if (risk === 'low') {
    return 0.88;
  }
...
function scoreCandidateHierarchyConsistency(
  candidate: ExpandedOccupationCandidate,
  branchShare: number,
  branchMarginRatio: number | null,
  leafShareWithinBranch: number,
...
function scoreBranchHierarchyConsistency(branch: OccupationCandidateBranch, branchShare: number, branchMarginRatio: number | null): number {
...
function scoreBranchCapabilitySupport(branch: OccupationCandidateBranch): number {
...
function scoreCandidateGenericRiskPenalty(candidate: ExpandedOccupationCandidate, queryIsGeneric: boolean): number {
...
function scoreBranchGenericRiskPenalty(branch: OccupationCandidateBranch, queryIsGeneric: boolean): number {
  const averagePenalty =
    branch.candidates.reduce((sum, candidate) => sum + scoreCandidateGenericRiskPenalty(candidate, queryIsGeneric), 0) /
    Math.max(branch.candidates.length, 1);

  return clampScore(averagePenalty);
}

function scoreUnrelatedBranchPenalty(branchShare: number, branchMarginRatio: number | null): number {
