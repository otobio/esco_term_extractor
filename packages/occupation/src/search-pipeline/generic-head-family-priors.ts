...
type GenericHeadPriorRule = {
  when: readonly GenericHeadVenueCarrier[];
  priors: readonly GenericHeadFamilyPrior[];
};

type GenericHeadPriorProfile = {
  rules?: readonly GenericHeadPriorRule[];
  default?: readonly GenericHeadFamilyPrior[];
};
...
const GENERIC_HEAD_SYNONYMS: Record<GenericHeadCarrier, readonly string[]> = {
...
const GENERIC_HEAD_VENUE_SYNONYMS: Record<GenericHeadVenueCarrier, readonly string[]> = {
...
function deriveVenueCarrierSet(roleTokens: string[], venueTokens: string[]): Set<GenericHeadVenueCarrier> {
  const carriers = new Set<GenericHeadVenueCarrier>();
...
function matchesAnyVenueCarrier(context: ReadonlySet<GenericHeadVenueCarrier>, carriers: readonly GenericHeadVenueCarrier[]): boolean {
  return carriers.some((carrier) => context.has(carrier));
}

function normalizeGenericHeadCarrier(value: string): GenericHeadCarrier | undefined {
...
function normalizeGenericHeadVenueCarriers(value: string): readonly GenericHeadVenueCarrier[] {
  const normalized = normalizePriorToken(value);
...
function buildAliasMap<T extends string>(synonyms: Record<T, readonly string[]>): Map<string, T> {
  const aliases = new Map<string, T>();

  for (const [carrier, variants] of Object.entries(synonyms) as Array<[T, readonly string[]]>) {
...
function buildMultiAliasMap<T extends string>(synonyms: Record<T, readonly string[]>): Map<string, readonly T[]> {
  const aliases = new Map<string, T[]>();
...
function normalizePriorToken(value: string): string {
