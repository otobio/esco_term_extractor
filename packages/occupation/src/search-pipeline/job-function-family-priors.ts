...
const JOB_FUNCTION_FAMILY_PRIORS: Readonly<Record<string, readonly JobFunctionFamilyPrior[]>> = {
  administration: [
...
export function normalizeJobFunction(value: string | undefined): string | null {
  const normalized = value?.trim().toLowerCase().replace(/-/gu, '_') ?? '';
  return normalized || null;
}

export function getJobFunctionFamilyPriors(value: string | undefined): readonly JobFunctionFamilyPrior[] {
  ...
  return JOB_FUNCTION_FAMILY_PRIORS[normalized as keyof typeof JOB_FUNCTION_FAMILY_PRIORS];
}

export function isKnownJobFunction(value: string | undefined): boolean {
  const normalized = normalizeJobFunction(value);
  return Boolean(normalized && normalized in JOB_FUNCTION_FAMILY_PRIORS);
}

function primary(familyNodeId: number, familyLabel: string): JobFunctionFamilyPrior {
  return {
    familyNodeId,
    familyLabel,
    strength: 'primary'
...
function supporting(familyNodeId: number, familyLabel: string): JobFunctionFamilyPrior {
  return {
    familyNodeId,
    familyLabel,
    strength: 'supporting'
