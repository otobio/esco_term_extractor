...
type NoiseVariable = keyof typeof NOISE_VARIABLES;
...
function compileNoisePattern(pattern: string): string {
  let source = '';
  let index = 0;
...
function cleanupPeeledSurface(value: string): string {
...
function buildVariableSource(name: NoiseVariable): string {
  const values = NOISE_VARIABLES[name].map((value) => escapeRegExp(value));
  return `(?:${values.join('|')})`;
}

function matchNoiseVariable(pattern: string, index: number): { name: NoiseVariable; length: number } | null {
  for (const token of VARIABLE_TOKENS) {
    if (pattern.startsWith(token, index)) {
      return {
        name: token.slice(1) as NoiseVariable,
        length: token.length
      };
    }
  }
...
function buildParentheticalLocationPattern(): RegExp {
  const locationAdmin = buildVariableSource('locationAdmin');
  return new RegExp(`\\(\\s*[^)]*${locationAdmin}[^)]*\\)`, 'giu');
}

function protectMeaningfulPlusJoins(value: string): string {
  return value.replace(/([\p{L}\p{N}])\s*\+\s*([\p{L}\p{N}])/gu, '$1__PLUS__$2');
}

function restoreMeaningfulPlusJoins(value: string): string {
