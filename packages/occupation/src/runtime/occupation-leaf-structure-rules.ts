...
export const __ATOMIC_SPECIALIZATION_SYNONYMS: Record<LeafSpecializationKind, Record<string, string[]>> = {
  venue: {
...
const LEAF_STRUCTURE_SPECIALIZATION_EQUIVALENCE_ROWS: Partial<Record<LeafSpecializationKind, string[][]>> = {
  venue: [
...
export function specializationKindAlignedWithQuery(
  ...
  const rowIncludesToken = (row: string[], token: string) => row.some((rowToken) => matchesTokenOrPlural(token, new Set([rowToken])));
  return rows.some(
    (row) =>
      markerTokens.some((marker) => rowIncludesToken(row, marker)) &&
...
// query-supported.
const LEAF_STRUCTURE_VENUE_IMPLIES_INDUSTRY_CONTEXT: Record<string, string[]> = {
