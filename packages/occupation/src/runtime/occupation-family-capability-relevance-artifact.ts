...
const CACHE = new Map<string, OccupationFamilyCapabilityRelevanceArtifactCacheEntry | null>();
...
export function defaultOccupationFamilyCapabilityRelevanceManifestPath(sourceName: string): string {
...
export function loadOccupationFamilyCapabilityRelevanceArtifactIfAvailable(
  sourceName: string
): OccupationFamilyCapabilityRelevanceArtifactCacheEntry | null {
  const manifestPath =
    readOptionalEnv(FAMILY_CAPABILITY_RELEVANCE_ENV) ?? defaultOccupationFamilyCapabilityRelevanceManifestPath(sourceName);
  ...
  let artifact: OccupationFamilyCapabilityRelevanceArtifactCacheEntry | null = null;
...
export function loadOccupationFamilyCapabilityRelevanceArtifactRequired(
  sourceName: string
): OccupationFamilyCapabilityRelevanceArtifactCacheEntry {
  const manifestPath =
    readOptionalEnv(FAMILY_CAPABILITY_RELEVANCE_ENV) ?? defaultOccupationFamilyCapabilityRelevanceManifestPath(sourceName);
...
export function buildOccupationFamilyCapabilityRelevanceBinaryFiles(
...
export function buildOccupationFamilyCapabilityRelevanceReviewData(
  records: RuntimeSearchMetaRecord[]
): OccupationFamilyCapabilityRelevanceReviewData {
  const data = buildArtifactData(records);
...
function loadArtifact(manifestPath: string, sourceName: string): OccupationFamilyCapabilityRelevanceArtifactCacheEntry | null {
  ...
  const familyRows = readFixedTableSync(
    ...
    FAMILY_CAPABILITY_RELEVANCE_FAMILY_ROW_WIDTH,
  ...
  const familyTokenRows = readFileBackedFixedTableSync(
    ...
    FAMILY_CAPABILITY_RELEVANCE_TOKEN_ROW_WIDTH,
  ...
  const genericityLocaleRows = readFixedTableSync(
    ...
    FAMILY_CAPABILITY_RELEVANCE_GENERICITY_LOCALE_ROW_WIDTH,
  ...
  return {
    ...
    maxTokenRelevance(locale: string, token: string): number {
      ...
      return (
        lookupScaledValue(genericityTokenRows, genericityRange.offset, genericityRange.length, tokenId) /
...
function validateManifest(value: unknown, manifestPath: string): OccupationFamilyCapabilityRelevanceArtifactManifest {
...
function isOccupationFamilyCapabilityRelevanceArtifactManifest(
  value: unknown
): value is OccupationFamilyCapabilityRelevanceArtifactManifest {
  return (
    isRecord(value) &&
