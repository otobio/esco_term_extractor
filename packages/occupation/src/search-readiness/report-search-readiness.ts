...
export type QueryReadinessClassification =
  | 'exact_leaf_hit'
  | 'acceptable_hit'
  | 'family_or_group_hit'
  | 'miss'
...
type PendingReviewCountRow = RowDataPacket & {
...
export class SearchReadinessReporter {
  ...
  public async run(options: ReportSearchReadinessOptions): Promise<SearchReadinessReport> {
  ...
  private async buildRunSummary(scope: ResolvedRunScope): Promise<RunReadinessSummary> {
    const queries = await this.loadEvaluationQueries(scope.sourceName, scope.setKey, scope.maxQueries ?? undefined);

    if (queries.length === 0) {
      throw new Error(
        `Search run ${scope.searchRunId} has no matching Phase 8 evaluation queries for source_name="${scope.sourceName}" and set_key="${scope.setKey}".`
      );
    }
    ...
    const candidateResultCounts: Record<string, number> = {};
    const classificationsByQueryId: Record<number, QueryReadinessClassification> = {};
    const categorySummariesByName = new Map<string, CategoryReadinessSummary>();
    const evidenceQueryIds = {
      exactAlias: new Set<number>(),
      foldedAlias: new Set<number>(),
      lexicalOrCapability: new Set<number>()
    };
    const rankedEvidenceQueryIds = new Set<number>();
  ...
  private async loadPendingReviewCounts(
    ...
    const counts: Partial<Record<ManualReviewType, number>> = {};
...
export function formatSearchReadinessReport(
  ...
  if (report.comparison) {
    ...
    lines.push(
      report.comparison.comparable && report.comparison.delta
  ...
  lines.push(
    `readiness_status=${
      report.verdict.searchMachineryReady === null ? 'indeterminate' : report.verdict.searchMachineryReady ? 'ready' : 'not_ready'
    }`
  );
...
function formatRunSummary(label: string, run: RunReadinessSummary): string {
  const lines: string[] = [];
  lines.push(label);
...
function formatDelta(delta: RunReadinessDelta): string {
  return [
...
function buildComparison(
  ...
  return {
    ...
    delta: {
      ...
      bestBroaderBranchHitCount:
        candidateRun.rankedEvidenceCounts.bestBroaderBranchHit - baselineRun.rankedEvidenceCounts.bestBroaderBranchHit,
...
function buildCategoryDeltas(
  baselineSummaries: CategoryReadinessSummary[],
  candidateSummaries: CategoryReadinessSummary[]
): CategoryReadinessDelta[] {
  ...
  const categoryNames = Array.from(new Set([...baselineByCategory.keys(), ...candidateByCategory.keys()])).sort((left, right) =>
...
function buildVerdict(
  ...
  if (evidenceSufficient && baselineRun && candidateRun && comparison?.delta) {
    ...
    if (!searchMachineryReady) {
      remaining.push(
        'The candidate run does not yet show a conservative improvement profile: successful hits must increase without raising misses, unresolved queries, or exact-leaf regressions.'
      );
    }
  }
...
function classifyQueryReadiness(
  ...
  const acceptableLeafIds = new Set(
  ...
  if (
    (selectedRow.decision_stage === 'selected_family' && familyIds.has(selectedRow.graph_node_id)) ||
...
function summarizeEvidencePresence(value: unknown): {
  exactAlias: boolean;
  foldedAlias: boolean;
  lexicalOrCapability: boolean;
} {
...
function summarizeRankedEvidence(
  expectations: EvaluationExpectationRow[],
  results: SearchRunResultRow[]
): {
  ...
  const bestBroaderBranchHit =
    (bestBroaderBranchKind === 'family' && familyIds.has(bestBroaderBranchNodeId)) ||
...
function emptyCategorySummary(category: string): CategoryReadinessSummary {
  return {
    category,
    queryCount: 0,
    selectedCount: 0,
...
function incrementCategorySummary(
  summary: CategoryReadinessSummary,
  classification: QueryReadinessClassification,
...
function assertMatchingScope(left: ResolvedRunScope, right: ResolvedRunScope): void {
  if (left.sourceName !== right.sourceName) {
    throw new Error(
      `Cannot compare run ${left.searchRunId} and run ${right.searchRunId}: source_name differs (${left.sourceName} vs ${right.sourceName}).`
    );
  }

  if (left.setKey !== right.setKey) {
    throw new Error(
      `Cannot compare run ${left.searchRunId} and run ${right.searchRunId}: set_key differs (${left.setKey} vs ${right.setKey}).`
    );
  }
}
...
function normalizeOptionalRunId(flagName: string, value: number | undefined): number | undefined {
  if (value === undefined) {
    return undefined;
  }
...
function formatCategorySummary(summary: CategoryReadinessSummary): string {
  return [
    `- ${summary.category}:`,
...
function formatCategoryDelta(delta: CategoryReadinessDelta): string {
  return [
    `- ${delta.category}:`,
    `query_count=${formatSignedNumber(delta.queryCount)}`,
    `selected_count=${formatSignedNumber(delta.selectedCount)}`,
