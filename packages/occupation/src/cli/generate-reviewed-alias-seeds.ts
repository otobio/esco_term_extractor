...
type AliasRole = (typeof ALIAS_ROLES)[number];
...
const DEFAULT_ALIAS_ROLE_WEIGHT: Record<AliasRole, number> = {
  locale_primary: 1,
...
async function main(): Promise<void> {
  const authoringSeeds: AuthoringSeed[] = Array.isArray(authoringSeedsJson) ? (authoringSeedsJson as AuthoringSeed[]) : [];
  const sourceNames = [...new Set(authoringSeeds.map((seed) => seed.sourceName))];

  const labelIndexBySource = new Map<string, Map<string, { graphNodeId: number; canonicalLabel: string }[]>>();
  for (const sourceName of sourceNames) {
    const artifact = await loadOccupationSearchMetaArtifactWithDetailsRequired(sourceName);
    const labelIndex = new Map<string, { graphNodeId: number; canonicalLabel: string }[]>();
  ...
  for (const seed of authoringSeeds) {
    ...
    if (matches.length === 0) {
      errors.push(
        `No leaf found for sourceName="${seed.sourceName}" canonicalLabel="${seed.canonicalLabel}" (locale=${seed.locale}, alias="${seed.alias}")`
      );
      continue;
    }
    ...
    if (matches.length > 1) {
      errors.push(
        `Ambiguous canonicalLabel="${seed.canonicalLabel}" for sourceName="${seed.sourceName}" matches ${matches.length} leaves (graphNodeIds: ${matches.map((m) => m.graphNodeId).join(', ')}) -- disambiguate by editing the authoring entry`
      );
      continue;
    }
