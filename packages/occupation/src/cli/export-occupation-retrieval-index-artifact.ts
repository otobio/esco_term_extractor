...
import {
  ...
  defaultOccupationRetrievalIndexManifestPath,
  writeFixedTable,
  writeStringTable,
  writeUint32Rows,
...
async function main(): Promise<void> {
  ...
  const manifest: OccupationRetrievalIndexManifest = {
...
function buildTextRecord(record: RuntimeSearchMetaRecord): TextRecordDraft {
...
function collectStrings(locales: string[], aliasRows: AliasRowDraft[], textRecords: TextRecordDraft[]): string[] {
  const values = new Set<string>(locales);
...
function buildAliasKeyIndex(
  rows: AliasRowDraft[],
  stringIdByValue: Map<string, number>,
  localeIdByCode: Map<string, number>,
  keyForRow: (row: AliasRowDraft) => string
): RangeIndexBuild {
...
function buildAliasTokenPostings(
  rows: AliasRowDraft[],
  stringIdByValue: Map<string, number>,
  localeIdByCode: Map<string, number>
): RangeIndexBuild {
...
function buildTextFieldPostings(
  records: TextRecordDraft[],
  stringIdByValue: Map<string, number>,
  localeIdByCode: Map<string, number>
): RangeIndexBuild {
...
function buildRangeIndex(grouped: Map<string, number[]>, keyWidth: number): RangeIndexBuild {
...
function localeId(localeIdByCode: Map<string, number>, locale: string): number {
...
function aliasRoleId(role: AliasRole): number {
