...
const QUERY_KIND_WEIGHT: Record<QueryTermKind, number> = {
  support: 1,
  role: 1.75,
  role_head: 2.75,
  domain: 0.8,
...
export function rankFamilyTop2(options: FamilyTop2ClassifierOptions): FamilyTop2ClassifierResult {
  const queryTerms = buildQueryTerms(options.query.preparedQuery);
  const hits: FamilyTop2FamilyHit[] = [];
  ...
  for (let rowId = 0; rowId < options.familyProfileArtifact.profileRows.count; rowId += 1) {
    ...
    const hit = scoreFamily(
      options.familyProfileArtifact,
      options.familyTokenRelevanceArtifact,
      profile,
      localeProfile,
      queryTerms,
      options.query.preparedQuery,
...
function buildQueryTerms(preparedQuery: PreparedQuery): QueryTerm[] {
  const terms = new Map<string, QueryTerm>();
  const roleTokens = uniqueTokenSequence(
    preparedQuery.intent.roleTokens.length > 0
  ...
  addTokens(
    terms,
    uniqueTokenSequence(preparedQuery.intent.authoritativeRoleHeadTokens.map((token) => foldSearchText(token))),
    'role_head',
    2.75
  );
  addTokens(
...
function addTokens(terms: Map<string, QueryTerm>, tokens: readonly string[], kind: QueryTermKind, baseWeight: number): void {
...
function scoreFamily(
  ...
  const exactFamilyLabelPhrase =
    familyLabelFolded === exactQueryFolded || foldWeakPunctuationLookupText(profile.familyLabel) === exactQueryWeakPunctuation;
  const familyLabelUsefulTokens = usefulFamilyLabelTokens(profile.familyLabel, preparedQuery.locale);
  const queryUsefulTokens = uniqueTokenSequence(
    preparedQueryFamilyScopedFoldedTokens(preparedQuery).filter(
      (token) =>
  ...
  const scoreBreakdown: FamilyTop2ScoreBreakdown = {
    labelAlias: 0,
    leafLabel: 0,
    capability: 0,
    phrase: 0,
...
function exactPhraseScoreForSource(
  familyProfileArtifact: FamilyProfileArtifactCacheEntry,
...
function compareFamilyHits(left: FamilyTop2FamilyHit, right: FamilyTop2FamilyHit): number {
  return (
...
function sourceHasToken(familyProfileArtifact: FamilyProfileArtifactCacheEntry, source: FamilyProfileSourceRef, tokenId: number): boolean {
  return familyProfileArtifact.sourceHasToken(source, tokenId);
}

function queryKindPriority(kind: QueryTermKind): number {
  switch (kind) {
    case 'role_head':
      return 5;
    case 'role':
