...
type AliasSource = 'mysql' | 'artifact';
...
async function main(): Promise<void> {
  ...
  if (options.format === 'text') {
    console.log(
      `Alias ngram batch source=${options.sourceName} locale=${options.locale} aliases=${index.aliasCount} family_supporting=${index.includeFamilySupportingAliases} index_load=${indexLoadMs}ms`
    );
  }
  ...
  for (const input of queries) {
    ...
    for (const [hitIndex, hit] of hits.entries()) {
      console.log(
        `${hitIndex + 1}. ${hit.canonicalLabel} score=${hit.score} cosine=${hit.cosine} coverage=${hit.usefulTokenCoverage} alias="${hit.alias}" role=${hit.aliasRole} family="${hit.familyLabel ?? 'n/a'}"`
      );
    }
  }
}
...
function toAliasNgramSourceRow(row: AliasRow): AliasNgramSourceRow {
  return {
...
async function loadQueries(options: CliOptions): Promise<QueryInput[]> {
...
function tryParseJsonLine(line: string): QueryInput | null {
  if (!line.startsWith('{')) {
    return null;
  }
...
function parseAliasSource(value: string): AliasSource {
  const normalized = value.trim().toLowerCase();
