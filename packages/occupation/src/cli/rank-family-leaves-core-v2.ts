...
} from '../runtime/occupation-leaf-structure-rules-v2.js';
import type {
  RuntimeAliasRecord,
...
//   contradictoryLevel         -10 if the query and canonical leaf label assert explicitly
//                               incompatible level values such as junior vs senior.
//   contradictorySpecialization -10 if the query and canonical leaf label assert explicitly
//                               incompatible visible specialization values, per the dimensional
//                               profile diff (canonicalLeafSpecializationSlotComparison) over the
//                               curated contradiction groups in occupation-leaf-structure-rules-v2.ts.
//   alignedSpecializationValue   5 if the query and canonical leaf profiles share the SAME value on
//                               a specialization dimension (e.g. both resolve "frontend" on the
//                               software_surface slot) -- rewards an explicit match instead of
//                               scoring it identically to "leaf has no opinion on this dimension".
//                               Zero whenever contradictorySpecialization already fired.
...
export function scoreLeaf(
  ...
  if (exactCanonicalMatch > 0) {
    return {
      ...
      contradictoryLevel: 0,
      contradictorySpecialization: 0,
      alignedSpecializationValue: 0,
  ...
  const contradictoryLevel = hasLeafRelationship && leafLevelKindsContradict(queryContext.queryLevelKind, leafLevelKind) ? -10 : 0;
  ...
  const specializationSlotComparison = hasLeafRelationship
    ? canonicalLeafSpecializationSlotComparison(canonicalTokens, preparedQuery)
    : { contradictionCount: 0, alignmentCount: 0 };
  const contradictorySpecialization = specializationSlotComparison.contradictionCount > 0 ? -10 : 0;
  // Reward a leaf and query naming the SAME contradiction-slot value explicitly (e.g. both say
  // "frontend") -- without this, that case scores identically to the leaf having no opinion on the
  // slot at all, even though it is real, confirming evidence the leaf is the right specialization.
  const alignedSpecializationValue = contradictorySpecialization === 0 && specializationSlotComparison.alignmentCount > 0 ? 5 : 0;
  ...
  return {
    ...
    contradictoryLevel,
    contradictorySpecialization,
    alignedSpecializationValue,
...
// Authority-tier levels (manager/director/chief) name a distinct, higher-authority occupation --
// unlike assistant/junior/senior/lead/supervisor, which are experience bands on the SAME role, a
// manager-flavored leaf is a different job than its non-manager sibling. Promoting one when the
// query never asked for authority at all (e.g. "customer care specialist" landing on "customer
// care manager") is a real mismatch, not a neutral non-signal, so it costs points rather than 0.
const UNREQUESTED_AUTHORITY_LEVEL_PENALTY = -3;
const LEVEL_AUTHORITY_KINDS = new Set<LeafLevelKind>(['manager', 'director', 'chief']);
...
function levelMatchScore(queryLevelKind: LeafLevelKind, leafLevelKind: LeafLevelKind): number {
  ...
  if (queryLevelKind === 'none') {
    return LEVEL_AUTHORITY_KINDS.has(leafLevelKind) ? UNREQUESTED_AUTHORITY_LEVEL_PENALTY : 0;
  }
...
export function sumScoreBreakdown(breakdown: LeafScoreBreakdown): number {
  return (
    ...
    breakdown.contradictoryLevel +
    breakdown.contradictorySpecialization +
    breakdown.alignedSpecializationValue +
