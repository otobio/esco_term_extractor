...
import type { OccupationIntentVocabularyLocale } from '../query/query-intent.js';
import { loadOccupationSearchMetaArtifactRequired } from '../runtime/occupation-search-meta-artifact.js';
import {
  buildOccupationIntentVocabularyInputs,
  buildOccupationIntentVocabularyRecords,
  classifyIntentVocabularyTerm,
...
type TermAuditRow = {
  ...
  suggestedClass: IntentVocabularyTermClass;
  englishBucket: string | null;
  totalCount: number;
  headCount: number;
  prefixCount: number;
  familyCount: number;
...
type LocaleAudit = {
  ...
  domainCandidates: TermAuditRow[];
  ambiguousCandidates: TermAuditRow[];
  englishCarryoverCandidates: TermAuditRow[];
};
...
async function main(): Promise<void> {
  ...
  const audits = options.locales.map((locale) =>
    buildLocaleAudit(
      locale,
      requiredLocaleRecord(records, locale),
      inputs.statsByLocale.get(locale) ?? new Map(),
      englishRecord,
      options.sampleLimit
    )
  );
...
function buildLocaleAudit(
  locale: SupportedQueryLocale,
  record: OccupationIntentVocabularyLocale,
  stats: Map<string, TermStats>,
  englishRecord: OccupationIntentVocabularyLocale,
  sampleLimit: number
): LocaleAudit {
  ...
  return {
    ...
    domainCandidates: rows
      .filter(
        (row) =>
          row.currentBucket !== 'domainModifierTerms' &&
...
function bucketForTerm(record: OccupationIntentVocabularyLocale, term: string): string {
  if (record.roleHeadTerms.includes(term)) return 'roleHeadTerms';
  if (record.roleModifierTerms.includes(term)) return 'roleModifierTerms';
  if (record.domainModifierTerms.includes(term)) return 'domainModifierTerms';
  if (record.credentialModifierTerms.includes(term)) return 'credentialModifierTerms';
  if (record.ambiguousModifierTerms.includes(term)) return 'ambiguousModifierTerms';
  if (record.rolePhrases.includes(term)) return 'rolePhrases';
  if (record.domainPhrases.includes(term)) return 'domainPhrases';
  return 'unclassified';
}

function compareAuditRows(left: TermAuditRow, right: TermAuditRow): number {
  return (
...
function requiredLocaleRecord(records: OccupationIntentVocabularyLocale[], locale: SupportedQueryLocale): OccupationIntentVocabularyLocale {
...
function formatText(audits: LocaleAudit[]): string {
  const lines: string[] = [];
...
function formatRows(label: string, rows: TermAuditRow[]): string {
  if (rows.length === 0) {
    return `${label}=none`;
  }
  ...
  return [
    ...
    ...rows.map(
      (row) =>
        `  - ${row.term} current=${row.currentBucket} suggested=${row.suggestedClass} english=${row.englishBucket ?? 'none'} counts(total=${row.totalCount},head=${row.headCount},prefix=${row.prefixCount},family=${row.familyCount})`
    )
  ].join('\n');
}
...
function isSupportedLocale(value: string): value is SupportedQueryLocale {
