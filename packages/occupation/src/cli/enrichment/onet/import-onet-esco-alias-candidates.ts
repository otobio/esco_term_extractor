...
export async function importOnetEscoAliasCandidates(
  connection: Connection,
  options: ImportOnetEscoAliasCandidatesOptions = {}
): Promise<ImportOnetEscoAliasCandidatesResult> {
  ...
  const rawAliasesByKey = new Map<string, RawAliasRow>();
  ...
  const candidates: OnetAliasCandidate[] = [];
  ...
  const result: ImportOnetEscoAliasCandidatesResult = {
...
async function downloadInputFiles(downloadDir: string): Promise<Record<keyof typeof FILES, string>> {
  const filePaths = {
...
async function loadTargetNodes(connection: Connection, sourceName: string): Promise<Map<string, TargetNode>> {
...
function buildCandidate(
  rawAlias: RawAliasRow,
  target: TargetNode | null,
  targetCount: number | null,
  mappedRows: OnetCrosswalkRow[],
  existingAliasesByNormalizedAlias: Map<string, ExistingAliasTarget[]>
): OnetAliasCandidate {
  ...
  const candidateMode: CandidateMode = excludeReasons.length > 0 ? 'exclude' : reviewReasons.length > 0 ? 'review' : 'generate';
...
function readFirstWorksheet(filePath: string): WorksheetRows {
...
function parseCrosswalkRows(sheet: WorksheetRows): OnetCrosswalkRow[] {
...
function parseJobTitleRows(sheet: WorksheetRows): RawAliasRow[] {
...
function parseReportedTitleRows(sheet: WorksheetRows): RawAliasRow[] {
  ...
  return sheet.rows
    .map((row) => ({
      ...
      sourceFileRole: 'reported_title' as const
    }))
...
function findHeaderedRows(sheet: WorksheetRows, requiredHeaders: string[]): WorksheetRows {
...
function readNullableCell(row: string[], index: number): string | null {
  const value = readCell(row, index);
  return value || null;
}

function uniqueTargets(targets: TargetNode[]): TargetNode[] {
...
function calculateOverlapScore(aliasTokens: string[], targetTokens: string[]): number {
...
function isGenericOneWordAlias(aliasTokens: string[]): boolean {
  return aliasTokens.length === 1 && (GENERIC_ONE_WORD_ALIASES.has(aliasTokens[0]) || isGenericQueryToken(aliasTokens[0], 'en'));
}

function compareCandidates(left: OnetAliasCandidate, right: OnetAliasCandidate): number {
