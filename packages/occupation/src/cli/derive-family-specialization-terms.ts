...
function sharesIdentityPrefix(token: string, labelToken: string): boolean {
...
type FamilyAccumulator = {
  familyNodeId: number;
  familyLabel: string;
  broaderFamilyNodeId: number | null;
  broaderFamilyLabel: string | null;
  leafCount: number;
  tokenLeafCounts: Map<string, number>;
};

async function main(): Promise<void> {
  const families = new Map<number, FamilyAccumulator>();
  ...
  for await (const line of rl) {
    ...
    const record = JSON.parse(line) as LeafRecord;
  ...
  const siblingsByBroader = new Map<number, FamilyAccumulator[]>();
  ...
  const output = familyList
    .map((family) => {
      ...
      const terms: Array<{ token: string; familyDocFreq: number; leafCount: number; siblingMaxDocFreq: number }> = [];
...
function closestBroaderFamily(ancestors: AncestorEntry[]): AncestorEntry | undefined {
  return ancestors
