...
} from './rank-family-leaves-core.js';
...
import type { SearchMetaArtifactCacheEntry, RuntimeCapabilityRecord } from '../runtime/occupation-search-meta-artifact.js';
...
async function main(): Promise<void> {
  ...
  console.log(
    formatResult(
      ...
      effectiveQuery,
      preparedQuery,
      rankedLeaves.slice(0, options.limit),
      familyLabelByNodeId,
      runtime.searchMetaArtifact
    )
  );
...
function formatResult(
  options: CliOptions,
  families: Array<{ familyNodeId: number; familyLabel: string }>,
  effectiveQuery: string,
  preparedQuery: PreparedQuery,
  leaves: RankedFamilyLeaf[],
  familyLabelByNodeId: Map<number, string>,
  ...
  lines.push(
    `families=${families.map((family) => `"${family.familyLabel}" #${family.familyNodeId}`).join(' | ')}  locale=${options.locale}  source=${options.sourceName}`
  );
  ...
  lines.push(
    `${selectedLeaf.rank}. "${selectedLeaf.canonicalLabel}" #${selectedLeaf.graphNodeId}  family="${familyLabelByNodeId.get(selectedLeaf.graphNodeId) ?? 'unknown family'}"  score=${selectedLeaf.totalScore}  closeness=${formatPercent(selectedLeaf.closeness.usefulQueryCoverage)}`
  );
  lines.push('');
...
function formatLeaf(
  ...
  const capabilityLabels: RuntimeCapabilityRecord[] = artifact.getCapabilityLabels(leaf.graphNodeId);
  const debug = debugComputeSpecialization(
    leaf.closeness,
    leaf.structure,
    preparedQuery,
    canonicalTokens,
...
function printHelp(): void {
  console.log(
    'Usage: npm run rank:family-leaves -- --family="Other health professionals" --family="Fishery workers, hunters and trappers" --family="Client information workers" --query="health professional" --locale=en'
  );
  console.log(
    'Pass --family up to 3 times to rank leaves across several families at once (e.g. 2 wrong + 1 right), sorted together by score.'
  );
}
