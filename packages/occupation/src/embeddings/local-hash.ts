...
export function embedTextWithLocalHash(text: string, modelKey: string, dimensions: number): LocalHashEmbedding {
...
export function computeDotProduct(left: number[], right: number[]): number {
...
export function computeCosineSimilarity(
  left: number[],
  right: number[],
  leftNorm = computeVectorNorm(left),
  rightNorm = computeVectorNorm(right)
): number {
...
export function computeVectorNorm(vector: number[]): number {
