 ...
 */
function getLongestTokenCombinations(input: string, minLen: number = 0): string[][] {
    ...
    function getCombinationsOfSize(arr: string[], size: number): string[][] {
        if (size === 0) return [[]];
...
export function buildRetrievalRequest(
  ...
  const roleHeadTokens = Array.from(
    new Set([...baseRoleHeadTokens, ...queryProfile.profile.role_head])
  );

  const groupExpansionSeeds = roleHeadTokens.filter(
    (token) =>
  ...
  const englishRoleHeadTokens = Array.from(
    new Set([...roleHeadTokens, ...expandRoleHeadSpellingVariants(roleHeadTokens)])
  );
