...
export const STRICT_SIMILARITY_ROLE_HEAD_GROUPS: Record<string, readonly string[]> = {
...
export const BROAD_SIMILARITY_ROLE_HEAD_META_GROUPS: Record<string, readonly string[]> = {
...
export function inferRoleHeadsFromStructuralContext(input: {
  ...
  for (const rule of input.familyRules) {
    ...
    const inferableRoleHeads: string[] = [];
...
export const ROLE_HEAD_STRICT_GROUP_BY_TOKEN = new Map<string, string>();
...
export function expandRoleHeadGroupTerms(tokens: readonly string[]): string[] {
  const inputSet = new Set(tokens);
  const groups = new Set(tokens.map((token) => ROLE_HEAD_STRICT_GROUP_BY_TOKEN.get(token)).filter((group): group is string => !!group));

  const expanded = new Set<string>();
  for (const group of groups) {
    for (const term of STRICT_SIMILARITY_ROLE_HEAD_GROUPS[group as keyof typeof STRICT_SIMILARITY_ROLE_HEAD_GROUPS]) {
