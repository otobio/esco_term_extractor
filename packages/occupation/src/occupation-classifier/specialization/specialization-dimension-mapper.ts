...
type PreparedConcept = {
  canonicalTokens: string[];
  defaultDimension: SpecializationConceptDimension | null;
  rules: SpecializationConceptRule[];
};
...
type RawPreparedConceptEntry = PreparedConceptEntry & {
  ...
  staticDimension: SpecializationConceptDimension | null;
};
...
type PreparedSchema = {
  ...
  structuralCombinations: StructuralCombinationRule[];
};
...
type EvidenceConceptAssignment = MatchedConceptAssignment & {
  dimension: SpecializationEvidenceDimension;
};
...
type MatchCandidate = {
  ...
  canonicalTokens: string[];
  conceptId?: string;
  dimension: SpecializationDimension;
  end: number;
  parts: string[];
  priority: number;
  roleHeadPhrase?: MatchedRoleHeadPhrase;
  start: number;
};
...
type ResolvedStructuralMatches = {
  concepts: ResolvedSpecializationConcept[];
  roleHeadPhrases: MatchedRoleHeadPhrase[];
};

type AcceptedMatchCandidates = {
  candidates: MatchCandidate[];
  concepts: MatchedConceptAssignment[];
  roleHeadPhrases: MatchedRoleHeadPhrase[];
};

type CsvRow = Record<string, string>;

type StructuralCombinationRule = {
  conceptIds: string[];
  derivedRoleHeads: string[];
  id: string;
  roleHeads: string[];
...
export const SPECIALIZATION_EVIDENCE_DIMENSIONS: SpecializationEvidenceDimension[] = [
  'venue',
  'channel',
  'product',
  'population',
...
const DIMENSION_PREFERENCE: Record<SpecializationEvidenceDimension, number> = {
  venue: 0,
  channel: 1,
  population: 2,
  knowledge_domain: 3,
...
const INDIRECT_ALIAS_DIMENSION_PREFERENCE: Record<SpecializationEvidenceDimension, number> = {
  venue: 0,
  channel: 1,
  population: 2,
  knowledge_domain: 3,
...
const EXPLICIT_LITERAL_DIMENSIONS: Record<SpecializationEvidenceDimension, Set<string>> = {
...
export function classifySpecializationQuery(title: string, options: ClassifierOptions = {}): QuerySpecializationClassification {
  ...
  const { concepts: conceptMatches, roleHeadPhrases: acceptedRoleHeadPhraseMatches } = resolveStructuralMatches(
    ...
    lowers,
    prepared,
    initialRoleHeads.roleSet,
    initialRoleHeads.roleIndexes,
    roleHeadPhraseMatches
  );
  ...
  for (const match of acceptedRoleHeadPhraseMatches) {
    ...
    pushLocaleRoleHeadAlternates(result.locale_role_head, match.alias, prepared);
  }

  for (const match of committedConceptMatches) {
    for (const canonicalToken of match.canonicalTokens) {
      pushBucketToken(result, match.dimension, canonicalToken);
      pushBucketToken(result.concept, match.dimension, canonicalToken);
    }
  }
  ...
  for (let index = 0; index < lowers.length; index += 1) {
    ...
    if (literalDimension && literalDimension !== 'role_head') {
      ...
      if (literalConceptAssignment) {
        ...
        for (const canonicalToken of literalConceptAssignment.canonicalTokens) {
          pushBucketToken(result, literalDimension, canonicalToken);
          pushBucketToken(result.concept, literalDimension, canonicalToken);
        }
        continue;
      }
    ...
    if (acronymDimension) {
      pushBucketToken(result, acronymDimension, token);
      continue;
    }
  ...
  mergeAvailableBuckets(result);

  return result;
}

export function getDefaultIndustryConceptIdsForRoleHeads(roleHeads: string[], options: ClassifierOptions = {}) {
  const schema = resolveSchemaForOptions(options);
...
export function classifySpecializationTitleDetailed(title: string, options: ClassifierOptions = {}): DetailedTitleClassification {
  ...
  return collectBuckets(
    ...
    literalAssigned,
    accepted.concepts,
    accepted.roleHeadPhrases,
    matchStructuralCombinations(accepted.concepts, roleSet, prepared),
    prepared
  );
}
...
function prepareSchema(schema: SpecializationSchema): PreparedSchema {
  ...
  const rawConceptEntries = schema.concepts
    .flatMap((concept) => {
      ...
      return concept.aliases.flatMap<RawPreparedConceptEntry>((aliasEntry) => {
...
function normalizePhraseAlias(alias: string): string[] {
...
function buildRoleHeadAliasAlternatesByAlias(schema: SpecializationSchema) {
  const alternatesByAlias = new Map<string, string[]>();
  const roleHeadSet = new Set(schema.roleHeads.map((roleHead) => normalizeTokenForMatch(roleHead)));

  for (const entry of schema.roleHeadAliasAlternates ?? []) {
    const normalizedAlias = normalizeTokenForMatch(entry.alias);
    const normalizedRoleHead = normalizeTokenForMatch(entry.alternateRoleHead);
    if (!normalizedAlias || !normalizedRoleHead || !roleHeadSet.has(normalizedRoleHead)) {
      continue;
    }

    const alternates = alternatesByAlias.get(normalizedAlias) ?? [];
    pushUnique(alternates, normalizedRoleHead);
    alternatesByAlias.set(normalizedAlias, alternates);
  }

  return alternatesByAlias;
}
...
function resolveStructuralMatches(
  tokens: string[],
  folded: string[],
  lowers: string[],
  prepared: PreparedSchema,
  ...
  return {
    concepts: [...matchesByConcept.values()].sort(
      (left, right) =>
...
function acceptNonOverlappingMatchCandidates(candidates: MatchCandidate[]): AcceptedMatchCandidates {
  const accepted: MatchCandidate[] = [];
  const conceptAssignments: MatchedConceptAssignment[] = [];
  const roleHeadPhrases: MatchedRoleHeadPhrase[] = [];
  const coveredIndexes = new Set<number>();
...
function roleHeadPhraseMatchCandidate(match: MatchedRoleHeadPhrase, lowers: string[]): MatchCandidate {
  return {
    alias: match.alias,
    canonicalTokens: [],
    dimension: 'role_head',
...
function resolveConceptDimension(
  ...
  roleSet: Set<string>,
  prepared: PreparedSchema
): SpecializationConceptDimension | null {
...
function collectAcceptedRoleHeads(
  lowers: string[],
  prepared: PreparedSchema,
  roleHeadPhraseMatches: readonly MatchedRoleHeadPhrase[],
...
function collectMatchedIndexes(matches: ReadonlyArray<{ start: number; end: number }>) {
...
function matchConceptPartsAtStart(
  lowers: string[],
  folded: string[],
  start: number,
  parts: string[],
  stopwords: Set<string>
): { end: number; exactParts: string[]; indexes: number[] } | null {
  let partIndex = 0;
...
function conceptCanonicalContainsCanonicalRoleHead(conceptId: string, prepared: PreparedSchema) {
  const concept = prepared.concepts.get(conceptId);
  if (!concept) {
    return false;
  }
...
function collectBuckets(
  ...
  structuralCombinations: StructuralCombinationMatch[],
  prepared: PreparedSchema
  ...
  for (const assignment of [...conceptAssignments].sort((left, right) => left.start - right.start || left.end - right.end)) {
    ...
    for (const canonicalToken of assignment.canonicalTokens) {
      pushBucketToken(result.concept, assignment.dimension, canonicalToken);
    }
  }
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    for (const conceptAssignment of conceptAssignmentsByStart.get(index) ?? []) {
      for (const canonicalToken of conceptAssignment.canonicalTokens) {
        pushBucketToken(result, conceptAssignment.dimension, canonicalToken);
      }
    }
    ...
    for (const roleHeadPhrase of roleHeadPhraseByStart.get(index) ?? []) {
      ...
      pushLocaleRoleHeadAlternates(result.locale_role_head, roleHeadPhrase.alias, prepared);
    }
      ...
      if (dimension === 'role_head') {
        ...
        if (canonicalTokens) {
          for (const canonicalToken of canonicalTokens) {
            pushBucketToken(result, dimension, canonicalToken);
            pushBucketToken(result.concept, dimension, canonicalToken);
          }
        } else {
          pushBucketToken(result, dimension, token);
        }
      }
    }
    const literalDimension = literalAssigned[index];
    if (literalDimension && !roleHeadPhraseCovered.has(index)) {
      pushBucketToken(result.literal, literalDimension, token);
    }
    result.assignments.push({
      token,
      normalized: lower,
...
function resolveLiteralTokenDimension(
  ...
  for (const [dimension, values] of Object.entries(FORCED_LITERAL_DIMENSIONS) as Array<
    [SpecializationEvidenceDimension, Set<string>]
  >) {
    if (values.has(lower)) {
      return dimension;
    }
  ...
  if (preferredEntry) {
    const dimension = resolveConceptDimension(preferredEntry.conceptId, roleSet, prepared);
    if (dimension !== null && isSpecializationEvidenceDimension(dimension)) {
      return dimension;
    }
    if (dimension === 'noop') {
      return null;
    }
    return inferContextualCommodityDimension(tokens, index, roleSet, prepared);
  }
...
function inferContextualCommodityDimension(
  ...
  roleSet: Set<string>,
  prepared: PreparedSchema
): SpecializationEvidenceDimension | null {
...
function pushLocaleRoleHeadAlternates(target: string[], alias: string, prepared: PreparedSchema) {
  for (const alternateRoleHead of prepared.roleHeadAliasAlternatesByAlias.get(normalizeTokenForMatch(alias)) ?? []) {
    pushUnique(target, alternateRoleHead);
  }
}

function pushBucketToken(
  buckets: Partial<Record<SpecializationDimension, string[]>>,
  dimension: SpecializationDimension | null | undefined,
  token: string
) {
  const bucket = dimension ? buckets[dimension] : undefined;
  if (bucket) {
    pushUnique(bucket, token);
  }
}

function mergeAvailableBuckets(result: Pick<TitleClassification, 'available' | 'concept' | 'literal'>) {
  for (const dimension of SPECIALIZATION_DIMENSIONS) {
    for (const token of result.literal[dimension] ?? []) {
      pushBucketToken(result.available, dimension, token);
    }
...
function appendConceptMatch(matches: ResolvedSpecializationConcept[], assignment: MatchedConceptAssignment) {
...
function buildFixedConcepts(dimension: SpecializationEvidenceDimension, aliases: string[]): SpecializationConcept[] {
...
export function loadSpecializationSchemaFromCsv(schemaDir = DEFAULT_SCHEMA_DIR, locale?: string): SpecializationSchema | null {
  ...
  try {
    ...
    const industryConceptIds = new Set(
      concepts.filter((concept) => concept.dimension === 'industry').map((concept) => concept.id)
    );
    const roleHeadDefaultIndustryConcepts: Record<string, string[]> = {};
    ...
    for (const row of roleRows) {
      ...
      const defaultIndustryConcepts = parseSemicolonList(row.default_industry_concepts ?? '').filter((conceptId) =>
        industryConceptIds.has(conceptId)
      );
    ...
    const mergedRoleHeadAliases = new Map<string, { alias: string; priority?: number; roleHead: string }>();
    const roleHeadByAliasParts = new Map<string, string>();
    ...
    const roleHeadAliasAlternates = loadRoleHeadAliasAlternateRows(schemaDir, locale)
      .map((row) => ({
        alias: (row.alias ?? '').trim(),
        alternateRoleHead: (row.alternate_role_head ?? '').trim()
      }))
      .filter((row) => row.alias.length > 0 && row.alternateRoleHead.length > 0 && roleHeadSet.has(normalizeTokenForMatch(row.alternateRoleHead)));
...
function parseOptionalCsv(filePath: string): CsvRow[] {
  try {
...
function loadRoleHeadAliasRows(schemaDir: string, locale?: string) {
...
function loadRoleHeadAliasAlternateRows(schemaDir: string, locale?: string) {
  const normalizedLocale = locale?.trim().toLocaleLowerCase('en-US');
  if (!normalizedLocale) {
    return [];
  }

  const localeAliasPath = resolve(schemaDir, `specialization-role-head-alias-alternates.${normalizedLocale}.csv`);
  try {
    return parseCsv(readFileSync(localeAliasPath, 'utf8'));
  } catch {
    return [];
  }
}
...
function buildStructuralCombinationRules(schema: SpecializationSchema): StructuralCombinationRule[] {
  ...
  const rules: StructuralCombinationRule[] = [];
...
function matchStructuralCombinations(
  concepts: readonly MatchedConceptAssignment[],
  roleSet: ReadonlySet<string>,
  prepared: PreparedSchema
): StructuralCombinationMatch[] {
  const matches: StructuralCombinationMatch[] = [];
  ...
  for (const rule of prepared.structuralCombinations) {
    const matchedConcepts = rule.conceptIds
      ...
      .filter((concept): concept is MatchedConceptAssignment => concept !== undefined);
...
function isEvidenceConceptAssignment(concept: MatchedConceptAssignment): concept is EvidenceConceptAssignment {
  return isSpecializationEvidenceDimension(concept.dimension);
}

function keepMostSpecificStructuralCombinations(matches: StructuralCombinationMatch[]): StructuralCombinationMatch[] {
  return matches.filter((match) => {
    return !matches.some((candidate) => {
      ...
      const candidateIsMoreSpecific =
        candidateConceptIds.length > matchConceptIds.length || candidate.roleHeads.length > match.roleHeads.length;

      return (
        candidateIsMoreSpecific && includesAll(candidateConceptIds, matchConceptIds) && includesAll(candidate.roleHeads, match.roleHeads)
      );
    });
  });
}
...
function includesAll(values: readonly string[], required: readonly string[]) {
...
function getStaticPreferredConceptDimension(concept: PreparedConcept): SpecializationConceptDimension | null {
  if (concept.defaultDimension) {
    return concept.defaultDimension;
  }
...
function inferGenericFallbackDimension(lower: string): SpecializationEvidenceDimension | null {
...
function compareConceptDimensions(left: SpecializationConceptDimension, right: SpecializationConceptDimension) {
  if (left === right) {
    return 0;
  }
  if (left === 'noop') {
...
function isSpecializationConceptDimension(value: string): value is SpecializationConceptDimension {
