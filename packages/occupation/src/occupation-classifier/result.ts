...
export function buildCoreResult(input: BuildCoreResultInput): CoreResult {
  return {
...
export function toRuntimeResult(coreResult: CoreResult): RuntimeResult {
...
function buildAltLeafCanonicalTerms(coreResult: CoreResult, family: SelectedFamily | null): AltLeafCanonicalTerm[] {
  if (!family) {
    return [];
  }
...
export function toDebugResult(coreResult: CoreResult, debugTrace: DebugTrace): DebugResult {
  return {
...
export function coreUnresolved(reason: CoreDecision['reason']): CoreResult {
...
function familyFromLeaf(leaf: SelectedLeaf | null): SelectedFamily | null {
...
function comparableTokenValues(comparisonQuery: CanonicalComparisonQuery | undefined): string[] {
  if (!comparisonQuery) {
    return [];
  }
