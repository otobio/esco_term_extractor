...
async function buildVocabulary(records) {
...
const SPECIALIZATION_SCHEMA_DIR = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'occupation-classifier', 'specialization', 'specialization-schema');
...
async function addSpecializationSchemaVocabularyTerms(tokens, tokenHashes, localeMaskByHash, phrasesByTokenCount, phraseHashesByTokenCount) {
    const add = (value, locale) => addText(value, tokens, tokenHashes, phrasesByTokenCount, phraseHashesByTokenCount, localeMaskByHash, locale);
