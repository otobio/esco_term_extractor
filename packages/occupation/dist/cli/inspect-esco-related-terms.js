...
function printVerbRows(options, rows) {
    console.log(`verb=${options.verb}  locale=${options.locale}  source=${options.sourceName}  count=${rows.length}`);
    for (const row of rows) {
        console.log([`- ${row.relatedVerb}`, `relationship=${row.relationshipType}`, `direction=${row.direction}`, `evidence=${row.evidenceCount}`].join('  '));
    }
}
function printObjectRows(options, rows) {
