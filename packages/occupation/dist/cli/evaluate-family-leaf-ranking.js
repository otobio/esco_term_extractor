...
import { ALL_PIPELINE_GOLDEN_CASES } from '../search-pipeline/golden-suite.js';
...
function pickSiblingFamilies(artifact, expectedFamilyNodeId, count) {
    if (count <= 0) {
        return [];
    }
...
function usableGoldenCases() {
    const LOCALE_ORDER = ['en', 'ro', 'hu', 'et'];
    return ALL_PIPELINE_GOLDEN_CASES.filter((goldenCase) => goldenCase.expectation.decisionType === 'leaf' &&
        Boolean(goldenCase.expectation.selectedLabel) &&
        Boolean(goldenCase.expectation.topFamilyLabel)).sort((a, b) => LOCALE_ORDER.indexOf(a.locale) - LOCALE_ORDER.indexOf(b.locale));
}
async function evaluateCase(runtime, sourceName, goldenCase, siblingFamilyCount) {
    ...
    try {
        ...
        const rankedLeaves = cliRankFamilyLeaves(runtime.searchMetaArtifact, runtime.leafStructureArtifact, leaves, preparedQuery, effectiveQuery, goldenCase.locale);
...
function printReport(results, options) {
    for (const result of results) {
        ...
        if (result.error) {
            ...
            lines.push(`  actual="${result.actualLeaf ?? 'none'}"${familyNote}  score=${result.actualScore ?? 'n/a'}  breakdown=${result.actualBreakdown ?? 'none'}`);
        }
