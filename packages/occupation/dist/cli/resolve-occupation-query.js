...
function formatResolutionResult(result, options) {
...
function formatCleanResolutionResult(result, useColor) {
    const lines = [];
...
function formatDebugResolutionResult(result, useColor) {
    const lines = [];
    const color = createColor(useColor);
    const context = result.queryContext;
...
function formatRankedResults(result) {
    const lines = [];
    const bestBranch = result.rankedResults.bestBroaderBranch;
    if (bestBranch) {
        const margin = bestBranch.branchMarginRatio === null ? 'none' : formatScore(bestBranch.branchMarginRatio);
        lines.push(`  best_broader_branch: ${bestBranch.branchKind}="${bestBranch.branchLabel}"#${bestBranch.branchNodeId} score=${formatScore(bestBranch.score)} share=${formatScore(bestBranch.branchShare)} margin=${margin} evidence_tier=${bestBranch.evidenceTier}`);
        for (const leaf of bestBranch.supportingLeaves) {
            lines.push(`    supporting_leaf ${leaf.rank}: "${leaf.canonicalLabel}"#${leaf.graphNodeId} retrieval_score=${formatScore(leaf.retrievalScore)} resolver_score=${formatScore(leaf.score)} evidence_tier=${leaf.evidenceTier}`);
        }
    }
    else {
    ...
    for (const leaf of result.rankedResults.topLeaves) {
        ...
        lines.push(`  top_leaf ${leaf.rank}: "${leaf.canonicalLabel}"#${leaf.graphNodeId} retrieval_score=${formatScore(leaf.retrievalScore)} resolver_score=${formatScore(leaf.score)} evidence_tier=${leaf.evidenceTier} leaf_share=${formatScore(leaf.leafShareWithinBranch)} leaf_margin=${margin} branch="${leaf.branchLabel}"#${leaf.branchNodeId}`);
    }
    return lines.join('\n');
}
function formatOutcome(outcome, useColor) {
    const color = createColor(useColor);
...
function formatBranch(index, branch) {
...
function formatCandidate(index, candidate) {
...
function buildUsefulNotes(result) {
    const notes = [];
    const outcome = result.selectedOutcome;
...
function formatEvidenceTier(evidenceTier, color) {
