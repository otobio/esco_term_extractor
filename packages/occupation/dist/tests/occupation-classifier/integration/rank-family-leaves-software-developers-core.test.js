...
diagnosticTest('bare developer: a leaf with an extra non-family-inherent industry_context cluster (embedded) must rank behind plain software developer', async () => {
    const ranked = await rankQuery('developer');
    ...
    assert.ok(embeddedIndex > softwareDeveloperIndex, `query="developer" expected "embedded systems software developer" (index ${embeddedIndex}) to rank behind "software developer" (index ${softwareDeveloperIndex}); the query gives no evidence for "embedded" so it must not escape decay`);
});
