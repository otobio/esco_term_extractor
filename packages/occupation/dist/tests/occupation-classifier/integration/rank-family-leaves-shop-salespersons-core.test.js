...
function promotableOnly(ranked) {
    return ranked.filter((candidate) => candidate.status === 'promotable');
}
function rejectedLabelsOf(ranked) {
    return ranked.filter((candidate) => candidate.status === 'hard_rejected').map((candidate) => candidate.canonicalLabel);
}
function assertOrder(ranked, expectedOrderPrefix, query) {
    const actualLabels = labelsOf(promotableOnly(ranked));
    const actualPrefix = actualLabels.slice(0, expectedOrderPrefix.length);
    assert.deepEqual(actualPrefix, expectedOrderPrefix, `query="${query}" expected top ${expectedOrderPrefix.length} promotable = ${JSON.stringify(expectedOrderPrefix)}, got ${JSON.stringify(actualPrefix)} (full promotable: ${JSON.stringify(actualLabels.slice(0, 8))})`);
}
...
function assertExcluded(ranked, expectedExcludedLabels, query) {
    ...
    for (const label of expectedExcludedLabels) {
        ...
        assert.ok(rejected.has(label), `query="${query}" expected "${label}" to be hard_rejected, got status=${candidate?.status ?? 'not found'}`);
    }
}
...
diagnosticTest('video recording equipment on sales: audio and video equipment specialised seller beats music and video shop specialised seller', async () => {
    const ranked = await rankQuery('video recording equipment sales');
    assertOrder(ranked, ['audio and video equipment specialised seller', 'music and video shop specialised seller'], 'video recording equipment sales');
});
...
diagnosticTest('motor vehicles parts seller: parts advisor beats generic vehicle seller beats generic seller', async () => {
    ...
    assertOrder(ranked, ['motor vehicles parts advisor', 'motor vehicles specialised seller', 'specialised seller'], 'motor vehicles parts seller');
});
...
diagnosticTest('audio equipment seller: audio/video seller beats generic seller beats music/video shop seller', async () => {
    ...
    assertOrder(ranked, ['audio and video equipment specialised seller', 'specialised seller', 'music and video shop specialised seller'], 'audio equipment seller');
});
diagnosticTest('video equipment seller: audio/video seller beats music/video shop seller beats generic seller', async () => {
    ...
    assertOrder(ranked, ['audio and video equipment specialised seller', 'music and video shop specialised seller', 'specialised seller'], 'video equipment seller');
});
