...
function concept(dimension, conceptId, canonicalTokens = [conceptId]) {
...
test('specializationGate ignores role_head entirely: it is never a judged structural dimension', () => {
    ...
    assert.equal(result.decision, 'pass_partial');
    assert.equal(result.accepted, true);
    assert.deepEqual(SPECIALIZATION_DATA_DIMENSIONS.includes('role_head'), false);
    assert.deepEqual(SPECIALIZATION_DATA_DIMENSIONS.includes('noop'), false);
});
...
test('keeps the concept-equivalence inventory materially populated across dimensions', () => {
    ...
    for (const concept of schema?.concepts ?? []) {
        const dimensions = new Set((concept.rules ?? []).map((rule) => rule.dimension));
