...
function comparisonQuery(overrides = {}) {
    return {
...
function structuralProfile(roleHead) {
    return {
        authority: 'unknown',
        profile: {
            venue: [],
...
test('buildRetrievalRequest adds structurally-derived role heads on top of translation-derived ones', () => {
    ...
    assert.ok(request.englishCanonicalExactKeys.includes('receptionist'), 'derived role head becomes an exact-canonical lookup key so the plain "receptionist" leaf is retrievable');
});
test('buildRetrievalRequest keeps resolved role heads when present and still adds structural ones', () => {
    const request = buildRetrievalRequest('esco_1_2_1', 'ro', surface, comparisonQuery({ resolvedRoleHeadTokens: ['adviser'] }), structuralProfile(['receptionist']));
...
test('buildRetrievalRequest does not duplicate a role head present in both sources', () => {
    const request = buildRetrievalRequest('esco_1_2_1', 'ro', surface, comparisonQuery({ resolvedRoleHeadTokens: ['receptionist'] }), structuralProfile(['receptionist']));
