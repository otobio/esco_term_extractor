...
import { cliRankFamilyLeaves, computeSiblingCompetitionScores, scoreLeaf, sumScoreBreakdown } from '../../src/cli/rank-family-leaves-core.js';
...
const SPECIALIZATION_KIND_MARKER_CASES = {
...
for (const [kind, { canonicalLabel, query }] of Object.entries(SPECIALIZATION_KIND_MARKER_CASES)) {
    test(`scoreLeaf collects the ${kind} specialization kind into alignedBroad when leaf and query share its marker`, async () => {
        ...
        const breakdown = scoreLeaf(closeness, [], genericStructure(900, canonicalLabel, { specializationKinds: [kind] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 900, new Map());
        assert.ok(breakdown.specializationBuckets.alignedBroad.includes(kind), `expected ${kind} in alignedBroad, got ${JSON.stringify(breakdown.specializationBuckets)}`);
    });
}
...
test('scoreLeaf collects a broad contradiction when leaf and query name different product values', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(901, canonicalLabel, { specializationKinds: ['product'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 901, new Map());
    assert.ok(breakdown.specializationBuckets.contradictoryBroad.includes('product'), `expected product in contradictoryBroad, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
test('scoreLeaf collects a broad contradiction from ro locale marker words (industry_context: auto vs sportiv)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(907, canonicalLabel, { specializationKinds: ['industry_context'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'ro', canonicalLabel, query, 907, new Map());
    assert.ok(breakdown.specializationBuckets.contradictoryBroad.includes('industry_context'), `expected industry_context in contradictoryBroad, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
...
test('scoreLeaf collects a narrow alignment when leaf and query share a curated slot value (task_focus: backend)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(902, canonicalLabel, { specializationKinds: ['task_focus'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 902, new Map());
    assert.ok(breakdown.specializationBuckets.alignedNarrow.includes('task_focus'), `expected task_focus in alignedNarrow, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
test('scoreLeaf collects a narrow contradiction when leaf and query name opposing curated slot values (task_focus: backend vs frontend)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(903, canonicalLabel, { specializationKinds: ['task_focus'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 903, new Map());
    assert.ok(breakdown.specializationBuckets.contradictoryNarrow.includes('task_focus'), `expected task_focus in contradictoryNarrow, got ${JSON.stringify(breakdown.specializationBuckets)}`);
...
test('scoreLeaf collects a narrow contradiction from ro locale marker words (population: copil vs tineret)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(908, canonicalLabel, { specializationKinds: ['population'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'ro', canonicalLabel, query, 908, new Map());
    assert.ok(breakdown.specializationBuckets.contradictoryNarrow.includes('population'), `expected population in contradictoryNarrow, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
...
test('scoreLeaf collects a broad alignment from a ro locale marker word (industry_context: auto)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(904, canonicalLabel, { specializationKinds: ['industry_context'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'ro', canonicalLabel, query, 904, new Map());
    assert.ok(breakdown.specializationBuckets.alignedBroad.includes('industry_context'), `expected industry_context in alignedBroad, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
...
test('scoreLeaf collects a broad AND narrow alignment from a ro locale marker word (population: copil)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(905, canonicalLabel, { specializationKinds: ['population'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'ro', canonicalLabel, query, 905, new Map());
    assert.ok(breakdown.specializationBuckets.alignedBroad.includes('population'), `expected population in alignedBroad, got ${JSON.stringify(breakdown.specializationBuckets)}`);
    assert.ok(breakdown.specializationBuckets.alignedNarrow.includes('population'), `expected population in alignedNarrow, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
test('scoreLeaf collects a broad AND narrow alignment from a hu locale marker word (population: gyermek)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(906, canonicalLabel, { specializationKinds: ['population'] }), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'hu', canonicalLabel, query, 906, new Map());
    assert.ok(breakdown.specializationBuckets.alignedBroad.includes('population'), `expected population in alignedBroad, got ${JSON.stringify(breakdown.specializationBuckets)}`);
    assert.ok(breakdown.specializationBuckets.alignedNarrow.includes('population'), `expected population in alignedNarrow, got ${JSON.stringify(breakdown.specializationBuckets)}`);
});
...
for (const [level, markerWord] of Object.entries(LEVEL_KIND_MARKER_WORDS)) {
    test(`scoreLeaf collects an exact level match for ${level}`, async () => {
        ...
        const canonicalTokens = foldedTokenSet(canonicalLabel);
        const closeness = rankCloseness(prepared, canonicalLabel, []);
        const breakdown = scoreLeaf(closeness, [], genericStructure(910, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 910, new Map());
...
const AUTHORITY_TIER_LEVELS = ['supervisor', 'manager', 'director', 'chief'];
const NON_AUTHORITY_TIER_LEVELS = ['assistant', 'junior', 'senior', 'lead'];
test('scoreLeaf collects an authority-level contradiction: non-authority query vs authority-tier leaf', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(920, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 920, new Map());
...
test('scoreLeaf collects an authority-level contradiction: authority-tier query vs non-authority leaf', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(921, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 921, new Map());
    assert.equal(breakdown.authorityLevelContradiction, true);
    assert.equal(breakdown.levelMatch, 0);
});
...
test('scoreLeaf gives same-tier partial credit (not a contradiction) across every authority-tier level pair', async () => {
    for (const queryLevel of AUTHORITY_TIER_LEVELS) {
        for (const leafLevel of AUTHORITY_TIER_LEVELS) {
            ...
            const canonicalTokens = foldedTokenSet(canonicalLabel);
            const closeness = rankCloseness(prepared, canonicalLabel, []);
            const breakdown = scoreLeaf(closeness, [], genericStructure(930, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 930, new Map());
            assert.equal(breakdown.levelMatch, 3, `expected levelMatch=3 for query=${queryLevel} leaf=${leafLevel}, got ${JSON.stringify(breakdown)}`);
            assert.equal(breakdown.authorityLevelContradiction, false);
        }
    }
});
test('scoreLeaf gives same-tier partial credit (not a contradiction) across every non-authority-tier level pair', async () => {
    ...
    for (const [queryLevel, leafLevel] of SAME_BAND_PAIRS) {
        ...
        const canonicalTokens = foldedTokenSet(canonicalLabel);
        const closeness = rankCloseness(prepared, canonicalLabel, []);
        const breakdown = scoreLeaf(closeness, [], genericStructure(931, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, query, 931, new Map());
        assert.equal(breakdown.levelMatch, 3, `expected levelMatch=3 for query=${queryLevel} leaf=${leafLevel}, got ${JSON.stringify(breakdown)}`);
        assert.equal(breakdown.authorityLevelContradiction, false);
    }
});
...
test('scoreLeaf collects an exact level match from a ro locale marker word (supervisor)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(940, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'ro', canonicalLabel, query, 940, new Map());
...
test('scoreLeaf collects an authority-level contradiction from a ro locale marker word (manager leaf vs assistant query)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(941, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'ro', canonicalLabel, query, 941, new Map());
    assert.equal(breakdown.authorityLevelContradiction, true);
    assert.equal(breakdown.levelMatch, 0);
});
test('scoreLeaf collects an exact level match from a hu locale marker word (supervisor: felugyelo)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(942, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'hu', canonicalLabel, query, 942, new Map());
    assert.equal(breakdown.levelMatch, 5);
    assert.equal(breakdown.authorityLevelContradiction, false);
});
test('scoreLeaf collects an authority-level contradiction from a hu locale marker word (manager leaf vs assistant query: segito)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(943, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'hu', canonicalLabel, query, 943, new Map());
    assert.equal(breakdown.authorityLevelContradiction, true);
    assert.equal(breakdown.levelMatch, 0);
});
...
test('scoreLeaf penalizes a leaf specialization (e.g. product) the query never asked for', async () => {
    ...
    const breakdown = scoreLeaf(closeness, [], structure, prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, 'shop manager', 501, new Map());
...
test('scoreLeaf awards the token-level canonical exact match (50) for a grammatical restatement of the query', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(601, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, 'certified electrician', 601, new Map());
...
test('scoreLeaf penalizes a leaf with no token or role-head relationship to the query', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(602, canonicalLabel), prepared, canonicalTokens, canonicalTokens, new Set(), [], null, 'en', canonicalLabel, 'electrician', 602, new Map());
...
test('scoreLeaf does not penalize a leaf whose English role head is the locale-specific role head via the role-head equivalence artifact', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(699, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'hu', canonicalLabel, 'szallodai recepcios', 699, new Map());
...
test("scoreLeaf penalizes a leaf missing the query's context-dependent required role head", async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(603, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'project manager', 603, new Map());
...
test('scoreLeaf awards the top role-head tier (15), venue support, and specialization for a venue-qualified match', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(604, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'shop electrician', 604, new Map());
...
test('scoreLeaf awards tier 10 (full useful-token canonical coverage) and the no-specialization-needed bonus', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(605, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'electrician', 605, new Map());
...
test("scoreLeaf credits a decoy alias that textually equals the query but was not the ranker's chosen match (loose-only aliasMatchBonus=3)", async () => {
    ...
    const breakdown = scoreLeaf(closeness, decoyAliases, genericStructure(606, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'electrician', 606, new Map());
...
test("scoreLeaf credits the ranked full alias-coverage bonus (20) when the closeness ranker's own winner is an alias", async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, aliases);
    const breakdown = scoreLeaf(closeness, aliases, genericStructure(607, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'electrician', 607, new Map());
...
test('scoreLeaf awards tier 5 (bare role-head match, no supporting evidence) for a role head with an unmatched extra token', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(608, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'electrician automotive', 608, new Map());
...
test('scoreLeaf rewards a same-band seniority match (levelMatch=3)', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(609, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'senior electrician', 609, new Map());
...
test('scoreLeaf does not credit a mismatched non-management seniority level match', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(610, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'senior electrician', 610, new Map());
    assert.equal(breakdown.authorityLevelContradiction, false);
    assert.equal(breakdown.levelMatch, 0);
    assert.equal(breakdown.roleHeadOrUsefulCanonical, 10);
    assert.equal(breakdown.usefulMatchNoSpecialization, 5);
...
test('scoreLeaf does not contradict an unrequested authority level (levelMatch=0) when the query asked for no level at all', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(611, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'electrician', 611, new Map());
...
test('scoreLeaf hard-gates a requested authority level the leaf never confirms, even when an alias contains the level word', async () => {
    ...
    const closeness = rankCloseness(prepared, canonicalLabel, aliases);
    const breakdown = scoreLeaf(closeness, aliases, genericStructure(618, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'risk manager', 618, new Map());
    assert.equal(closeness.matchedLabelSource, 'alias');
    assert.equal(breakdown.levelMatch, 0);
    assert.equal(breakdown.authorityLevelContradiction, true);
});
test('scoreLeaf rewards an exact level match, a supported specialization, and its aligned specialization value together', async () => {
    ...
    const breakdown = scoreLeaf(closeness, [], structure, prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'clothing accessories shop manager', 612, new Map());
    assert.equal(breakdown.levelMatch, 5);
    assert.equal(breakdown.roleHeadOrUsefulCanonical, 15);
    assert.equal(breakdown.specializationScore, 8);
...
test('scoreLeaf penalizes a contradictory specialization (contradictorySpecialization=-10) between sibling leaves under the same generic role head', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(613, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'frontend developer', 613, new Map());
...
test('scoreLeaf rewards useful domain-token support (usefulDomainSupport=2) when the canonical label covers a known domain modifier', async () => {
    ...
    const canonicalTokens = foldedTokenSet(canonicalLabel);
    const closeness = rankCloseness(prepared, canonicalLabel, []);
    const breakdown = scoreLeaf(closeness, [], genericStructure(614, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'en', canonicalLabel, 'banking electrician', 614, new Map());
...
test('scoreLeaf does not award usefulVenueSupport for a specialization kind it already penalized as unsupported', async () => {
    ...
    const breakdown = scoreLeaf(closeness, aliases, structure, prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), [], null, 'ro', canonicalLabel, 'retail wholesale trade managers', 617, new Map());
    // Guard against the penalty being silently suppressed by the zero-lexical-overlap
    // optimization -- if this ever goes to 0, the assertion below is testing nothing.
    assert.ok(closeness.matchedUsefulTokens.length > 0);
    // assert.equal(breakdown.unsupportedSpecialization, -5);
...
test('scoreLeaf rewards a strong capability-label fit (usefulCapabilityFit=4) when every capability-verb token is covered', async () => {
    ...
    const breakdown = scoreLeaf(closeness, [], genericStructure(615, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), capabilityLabels, null, 'en', canonicalLabel, 'shop worker', 615, new Map());
...
test('scoreLeaf rewards a partial capability-label fit (usefulCapabilityFit=2) when only some capability-verb tokens are covered', async () => {
    ...
    const breakdown = scoreLeaf(closeness, [], genericStructure(616, canonicalLabel), prepared, canonicalTokens, foldedTokenSet(closeness.matchedLabel), new Set(), capabilityLabels, null, 'en', canonicalLabel, 'shop worker', 616, new Map());
...
function alias(localeCode, value, aliasRole = 'locale_primary') {
    return {
        localeCode,
        alias: value,
...
function capabilityLabel(label) {
    return {
        capabilityId: 1,
...
function genericStructure(graphNodeId, canonicalLabel, overrides = {}) {
    return {
        graphNodeId,
        canonicalLabel,
        familyNodeId: 1,
