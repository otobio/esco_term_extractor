...
function assertPeelingCase(options) {
    ...
    for (const kind of options.noiseKinds ?? []) {
        assert.ok(result.noiseChunks.some((chunk) => chunk.kind === kind), `${options.locale}: ${options.title} missing ${kind}`);
    }
}
function assertNoNoiseCase(title, locale, peeledTitle = title) {
...
function assertNoiseOrigin(title, locale, kind, origin) {
    const result = peelOccupationTitleNoise(title, locale);
...
test('separator and no-separator forms both peel common noise surfaces', () => {
    ...
    for (const testCase of cases) {
        assert.equal(peelOccupationTitleNoise(testCase.title, testCase.locale).peeledTitle, testCase.peeledTitle, `${testCase.locale}: ${testCase.title}`);
    }
});
