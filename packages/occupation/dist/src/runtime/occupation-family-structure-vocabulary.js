...
const FAMILY_STRUCTURE_VOCABULARY_DIMENSIONS = new Set([
    'occupation_level',
    'role_heads',
    'knowledge_domains',
    'work_objects',
...
const FAMILY_STRUCTURE_VOCABULARY_LOCALES = new Set(['en', 'ro', 'hu', 'et']);
...
const BASE_FAMILY_STRUCTURE_QUERY_VOCABULARY = [
...
export const FAMILY_STRUCTURE_QUERY_VOCABULARY = [
...
export function familyStructureVocabularyMatchesForToken(token, locale = 'en', options = {}) {
    const matches = [];
    const seen = new Set();
    const keys = options.expandLocaleVariants === false
...
export function familyStructureVocabularyMatchesForTokens(tokens, locale = 'en', options = {}) {
    const matches = [];
    const seen = new Set();
...
export function familyStructureVocabularyValuesByDimension(tokens, locale = 'en', options = {}) {
...
function entry(dimension, value, aliases, locales) {
    return locales ? { dimension, value, aliases, locales } : { dimension, value, aliases };
}
function reviewedFamilyStructureVocabularyEntries() {
...
function parseReviewedFamilyStructureVocabularySeedRow(row, index) {
    ...
    if (typeof dimension !== 'string' || !FAMILY_STRUCTURE_VOCABULARY_DIMENSIONS.has(dimension)) {
    ...
    if (locale !== undefined && (typeof locale !== 'string' || !FAMILY_STRUCTURE_VOCABULARY_LOCALES.has(locale))) {
        throw new Error(`Invalid reviewed family structure vocabulary row ${index}: invalid locale.`);
    }
    if (locales !== undefined &&
        (!Array.isArray(locales) ||
            locales.length === 0 ||
            !locales.every((item) => typeof item === 'string' && FAMILY_STRUCTURE_VOCABULARY_LOCALES.has(item)))) {
...
function derivedLeafStructureVocabularyEntries() {
    const entries = [];
...
function buildVocabularyTokenIndex() {
...
function vocabularyTokenKey(token) {
    return foldSearchText(token).toLocaleLowerCase('en-US');
}
function vocabularyLookupKeys(token, locale) {
