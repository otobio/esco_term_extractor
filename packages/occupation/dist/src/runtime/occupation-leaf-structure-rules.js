...
export const LEAF_STRUCTURE_AUTHORITY_ORDER = leafStructureSynonymsSeed.leafStructureAuthorityOrder;
export const LEVEL_SPECIALIZATION_SYNONYMS = leafStructureSynonymsSeed.levelSpecializationSynonyms;
export const ATOMIC_SPECIALIZATION_SYNONYMS = leafStructureSynonymsSeed.atomicSpecializationSynonyms;
...
const SPECIALIZATION_MARKER_EQUIVALENCE_GROUPS = leafStructureSynonymsSeed.specializationMarkerEquivalenceGroups;
...
function contradictionValue(label, ...anchorGroups) {
...
const LEAF_CANONICAL_SPECIALIZATION_CONTRADICTION_GROUPS = [
    ...leafStructureSynonymsSeed.otherContradictionGroups,
    {
...
export function leafMarkerTokensForKind(kind, tokens, locale = 'en') {
...
export function canonicalLeafSpecializationSlotComparison(canonicalTokens, preparedQuery) {
    ...
    const contradictionKinds = [];
    const alignmentKinds = [];
...
function matchingContradictionValueIndexes(tokens, values, locale = 'en') {
    const indexes = [];
...
export function specializationKindAlignedWithQueryMatch(kind, canonicalTokens, preparedQuery) {
...
export function specializationKindContradictedByQueryMatch(kind, canonicalTokens, preparedQuery) {
    const keyIndex = SPECIALIZATION_MARKER_KEY_INDEX[kind];
    const markerTokens = leafMarkerTokensForKind(kind, canonicalTokens);
    if (markerTokens.length === 0) {
        return null;
...
export function specializationKindContradictedByQuery(kind, canonicalTokens, preparedQuery) {
...
// query-supported.
const VENUE_IMPLIES_INDUSTRY_CONTEXT = leafStructureSynonymsSeed.venueImpliesIndustryContext;
...
function matchesTokenOrPlural(token, candidates, locale = 'en') {
...
function hasAny(tokens, candidates, locale = 'en') {
    for (const token of tokens) {
