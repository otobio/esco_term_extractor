...
export class OccupationSearchMetaAuditor {
    ...
    async run(options = {}) {
        ...
        const englishBackbone = await this.runSection('englishBackbone', () => this.loadEnglishBackbone(sourceName, weakEnglishBackboneThreshold));
        const qualityFlags = await this.runSection('qualityFlags', () => this.loadQualityFlags(sourceName));
        const missingHierarchySamples = await this.runSection('missingHierarchySamples', () => this.loadMissingHierarchySamples(sourceName, sampleLimit));
        const missingLocaleSamples = await this.runSection('missingLocaleSamples', () => this.loadMissingLocaleSamples(sourceName, locales.length, sampleLimit));
        const highGenericRiskSamples = await this.runSection('highGenericRiskSamples', () => this.loadHighGenericRiskSamples(sourceName, sampleLimit));
        const weakEnglishBackboneSamples = await this.runSection('weakEnglishBackboneSamples', () => this.loadWeakEnglishBackboneSamples(sourceName, weakEnglishBackboneThreshold, sampleLimit));
        ...
        const reviewQueueInsertResult = options.insertReviewQueue
            ? await this.runSection('reviewQueueInsertResult', () => this.insertReviewQueueRows(sourceName, locales.length, weakEnglishBackboneThreshold))
            : null;
        return {
    ...
    async loadGenericRiskDistribution(sourceName) {
        const [rows] = await this.connection.query(`
    ...
    async loadLocaleCoverage(sourceName, locales) {
        const rows = [];
    ...
    async loadAliasSparsity(sourceName) {
        const [rows] = await this.connection.query(`
        SELECT
          CASE
    ...
    async loadTextQuality(sourceName) {
        const [rows] = await this.connection.query(`
        SELECT
    ...
    async loadEnglishBackbone(sourceName, threshold) {
        const [rows] = await this.connection.query(`
        SELECT
    ...
    async loadQualityFlags(sourceName) {
        const [rows] = await this.connection.query(`
    ...
    async loadMissingHierarchySamples(sourceName, sampleLimit) {
        return this.loadSamples(sourceName, `
        meta.has_hierarchy = 0
        OR meta.parent_node_id IS NULL
        OR meta.family_node_id IS NULL
        OR meta.group_node_id IS NULL
      `, 'meta.has_hierarchy ASC, meta.locale_coverage_count ASC, node.canonical_label', [sampleLimit]);
    }
    async loadMissingLocaleSamples(sourceName, expectedLocaleCount, sampleLimit) {
        return this.loadSamples(sourceName, 'meta.locale_coverage_count < ?', 'meta.locale_coverage_count ASC, meta.exact_alias_count ASC, node.canonical_label', [expectedLocaleCount, sampleLimit]);
    }
    async loadHighGenericRiskSamples(sourceName, sampleLimit) {
        return this.loadSamples(sourceName, "meta.generic_risk = 'high'", 'meta.exact_alias_count ASC, meta.active_alias_count DESC, node.canonical_label', [sampleLimit]);
    }
    async loadWeakEnglishBackboneSamples(sourceName, threshold, sampleLimit) {
        return this.loadSamples(sourceName, '(meta.english_backbone_strength IS NULL OR meta.english_backbone_strength < ?)', 'meta.english_backbone_strength IS NULL DESC, meta.english_backbone_strength ASC, node.canonical_label', [threshold, sampleLimit]);
    }
    async loadTextQualitySamples(sourceName, sampleLimit) {
        return this.loadSamples(sourceName, `
      ...
      `, 'node.canonical_label', [sampleLimit]);
    }
    async loadSamples(sourceName, predicateSql, orderBySql, params) {
        const [rows] = await this.connection.query(`
        SELECT
    ...
    async insertReviewQueueRows(sourceName, expectedLocaleCount, weakEnglishBackboneThreshold) {
    ...
    async insertHierarchyGapRows(sourceName) {
    ...
    async insertCrossLocaleGapRows(sourceName, expectedLocaleCount) {
        const [result] = await this.connection.execute(`
        INSERT INTO ose_manual_review_queue (
          review_type,
          subject_text,
    ...
    async insertGenericHeadRows(sourceName, weakEnglishBackboneThreshold) {
        const [result] = await this.connection.execute(`
        INSERT INTO ose_manual_review_queue (
          review_type,
          subject_text,
...
function buildFindings(overview, localeCoverage, textQuality, englishBackbone, genericRiskDistribution, reviewQueueInsertResult) {
    const findings = [
    ...
    for (const localeRow of localeCoverage) {
        findings.push(`${localeRow.localeCode}: ${localeRow.occupationsWithLocaleAliases}/${overview.totalMetaRows} occupations have local aliases; ${localeRow.occupationsMissingLocaleAliases} are missing local alias coverage.`);
    }
    if (reviewQueueInsertResult) {
        const totalInserted = reviewQueueInsertResult.hierarchyGapRowsInserted +
