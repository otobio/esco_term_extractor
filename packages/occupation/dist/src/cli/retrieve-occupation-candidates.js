...
function formatRetrievalResult(cliResult, format) {
    if (format === 'json') {
    ...
    lines.push(`Retrieval candidates for "${retrievalQuery.originalQuery}" (locale=${cliResult.locale}, source_name=${cliResult.sourceName}${evaluationSummary})`);
    ...
    for (const [index, candidate] of result.candidates.entries()) {
        ...
        for (const evidence of candidate.evidence) {
            ...
            if (evidence.channel === 'ngram_alias') {
                lines.push(`   evidence: channel=ngram_alias score=${formatScore(evidence.score)} cosine=${formatScore(evidence.cosine)} matched_tokens=${formatStringArray(evidence.details?.matched_tokens)} useful_token_coverage=${formatUnknownScore(evidence.details?.useful_token_coverage)} alias="${evidence.alias ?? ''}" normalized_alias="${evidence.normalizedAlias ?? ''}" alias_role=${evidence.aliasRole ?? ''} alias_weight=${formatScore(evidence.aliasWeight)}`);
                continue;
            }
            if (evidence.channel === 'capability_task') {
                lines.push(`   evidence: channel=capability_task score=${formatScore(evidence.score)} opensearch_score=${formatUnknownScore(evidence.details?.opensearch_score)} matched_tokens=${formatStringArray(evidence.details?.matched_tokens)} useful_token_coverage=${formatUnknownScore(evidence.details?.max_useful_token_coverage)}`);
                continue;
            }
            const folded = evidence.foldedAlias ? ` folded_alias="${evidence.foldedAlias}"` : '';
            const aliasAuthority = formatAliasAuthority(evidence.details);
            lines.push(`   evidence: channel=${evidence.channel} score=${formatScore(evidence.score)} alias="${evidence.alias ?? ''}" normalized_alias="${evidence.normalizedAlias ?? ''}"${folded} alias_role=${evidence.aliasRole ?? ''} alias_weight=${formatScore(evidence.aliasWeight)}${aliasAuthority}`);
        }
    }
    return lines.join('\n');
}
...
function formatAliasAuthority(details) {
    if (!details) {
        return '';
    }
    const parts = [
...
function formatAliasAuthorityPart(label, value) {
...
function printHelp() {
    console.log(`Usage: node dist/cli/retrieve-occupation-candidates.js --query="software developer" [--locale=${DEFAULT_RETRIEVAL_LOCALE}] [--source-name=${DEFAULT_ESCO_SOURCE_NAME}] [--limit=${DEFAULT_CANDIDATE_LIMIT}] [--format=text|json] [--evaluation-query-id=N]`);
}
