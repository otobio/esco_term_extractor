...
async function main() {
    ...
    const aliasIndexByLocale = new Map();
    ...
    for (const record of artifact.getAllCoreRecords()) {
        ...
        for (const alias of aliases) {
            ...
            const localeIndex = aliasIndexByLocale.get(alias.localeCode);
            if (!localeIndex) {
                continue;
            }
            const folded = foldSearchText(alias.alias);
            const bucket = localeIndex.get(folded) ?? new Set();
    ...
    for (const locale of LOCALES) {
        ...
        for (const entry of entries) {
            ...
            if (confirmedLabels.length > 0) {
                console.log(`[redundant] locale=${locale} surface="${entry.surface}" role=${entry.roleKey} canonicalEnglish="${entry.canonicalEnglish}" -- already exists as a real alias for the same concept: ${confirmedLabels.join(' | ')}`);
            }
            else {
                console.log(`[mismatch]  locale=${locale} surface="${entry.surface}" role=${entry.roleKey} canonicalEnglish="${entry.canonicalEnglish}" -- alias exists but attached to a different leaf: ${Array.from(matchingLabels).join(' | ')}`);
            }
        }
    }
}
