...
async function runWithRawQuery(options, sourceName) {
    if (!options.query) {
...
async function runWithEvaluationQuery(connection, options, sourceName) {
    const evaluationQueryId = options.evaluationQueryId;
    const [rows] = await connection.query('SELECT id, locale_code, query_text FROM ose_evaluation_queries WHERE id = ? LIMIT 1', [evaluationQueryId]);
...
async function buildRetrievalQuery(sourceName, locale, originalQuery) {
...
function formatBranchExpansionResult(cliResult, format) {
    if (format === 'json') {
    ...
    lines.push(`normalized_query="${preparedQuery.normalized}", folded_query="${preparedQuery.folded}", retrieval_profile=${result.retrievalProfile}`);
    lines.push(`scanned alias hits=${result.scannedAliasHitCount}, scanned lexical hits=${result.scannedOpenSearchHitCount}, candidates=${result.candidates.length}, branches=${result.branches.length}, sibling_limit=${result.siblingLimit}`);
    ...
    for (const [branchIndex, branch] of result.branches.entries()) {
        ...
        for (const [candidateIndex, candidate] of branch.candidates.entries()) {
            ...
            lines.push(`      hierarchy: family=${formatNode(candidate.familyNodeId, candidate.familyLabel)}, group=${formatNode(candidate.groupNodeId, candidate.groupLabel)}, parent=${formatNode(candidate.parentNodeId, candidate.parentLabel)}`);
            if (candidate.ancestors.length > 0) {
                lines.push(`      ancestors: ${candidate.ancestors
            ...
            if (candidate.siblings.length > 0) {
                lines.push(`      siblings: ${candidate.siblings
            ...
            for (const evidence of candidate.evidence) {
                ...
                if (evidence.channel === 'capability_task') {
                    lines.push(`      evidence: channel=capability_task score=${formatScore(evidence.score)} matched_tokens=${formatStringArray(evidence.details?.matched_tokens)} useful_token_coverage=${formatUnknownScore(evidence.details?.max_useful_token_coverage)}`);
                    continue;
                }
                const folded = evidence.foldedAlias ? ` folded_alias="${evidence.foldedAlias}"` : '';
                lines.push(`      evidence: channel=${evidence.channel} score=${formatScore(evidence.score)} alias="${evidence.alias ?? ''}" normalized_alias="${evidence.normalizedAlias ?? ''}"${folded} alias_role=${evidence.aliasRole ?? ''} alias_weight=${formatScore(evidence.aliasWeight)}`);
            }
        }
    }
    return lines.join('\n');
...
function toJsonResult(cliResult) {
    const result = cliResult.result;
...
function toJsonCandidate(candidate) {
    return {
...
function formatNode(graphNodeId, label) {
...
function formatUnknownScore(value) {
    return typeof value === 'number' ? formatScore(value) : '0';
}
function printHelp() {
    console.log(`Usage: node dist/cli/expand-occupation-candidate-branches.js --query="software developer" [--locale=${DEFAULT_RETRIEVAL_LOCALE}] [--source-name=${DEFAULT_ESCO_SOURCE_NAME}] [--limit=${DEFAULT_CANDIDATE_LIMIT}] [--sibling-limit=${DEFAULT_SIBLING_LIMIT}] [--format=text|json] [--evaluation-query-id=N]`);
}
