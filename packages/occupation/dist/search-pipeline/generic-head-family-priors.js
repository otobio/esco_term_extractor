...
export function getGenericHeadFamilyContradiction(roleHeadTokens, roleTokens, venueTokens, hasCuratedRolePhrase, familyNodeId) {
    if (hasCuratedRolePhrase || roleHeadTokens.length === 0) {
        return false;
    }
    const headCarrier = normalizeGenericHeadCarrier(roleHeadTokens[roleHeadTokens.length - 1] ?? '');
    ...
    return profile.rules.some((rule) => rule !== matchedRule && rule.priors.some((prior) => prior.familyNodeId === familyNodeId && prior.strength === 'primary'));
}
