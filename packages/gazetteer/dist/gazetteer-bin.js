...
import { GazetteerIndex } from './gazetteer-index.js';
import { GAZETTEER_CONFIG, GazetteerResolver } from './resolver.js';
const MAGIC = 0x475a4231; // "GZB1"
const VERSION = 1;
const KIND_CODE = { country: 0, admin1: 1, admin2: 2, admin3: 3, settlement: 4 };
const F_CAPITAL = 1, F_SEAT = 2, F_DOMINANT = 4, F_STOPWORD = 8, F_LEAF = 16, F_MAJOR = 32;
...
export function pack(records) {
    ...
    for (const r of records)
        maxDepth.set(r.countryCode, Math.max(maxDepth.get(r.countryCode) ?? 0, r.depth));
    ...
    let keyPos = 0, namePos = 0, ancPos = 0;
    ...
    const distinct = [...surfaceMap.keys()].sort((a, b) => Buffer.compare(Buffer.from(a, 'utf8'), Buffer.from(b, 'utf8')));
    const s = distinct.length;
    const surfOff = Buffer.alloc((s + 1) * 4);
    const postOff = Buffer.alloc((s + 1) * 4);
    const surfParts = [];
    const postingsList = [];
    let surfPos = 0, postPos = 0;
...
export class GazetteerBin {
    ...
    constructor(ab) {
        ...
        if (dv.getUint32(0, true) !== MAGIC)
            throw new Error('not a GZB file');
        if (dv.getUint32(4, true) !== VERSION)
            throw new Error(`GZB version ${dv.getUint32(4, true)} != ${VERSION}`);
        const n = dv.getUint32(8, true);
        const s = dv.getUint32(12, true);
        const dictCount = dv.getUint32(16, true);
        this.size = n;
        const off = (i) => dv.getUint32(20 + i * 4, true);
    ...
    static async load(path) {
        ...
        const ab = buf.buffer.slice(buf.byteOffset, buf.byteOffset + buf.byteLength); // fresh, aligned
    ...
    place(i) {
        return {
            ...
            depth: this.depth[i],
            languageCode: this.countryDict[this.country[i]],
        };
    }
    ...
    exact(normName) {
        ...
        let lo = 0, hi = this.surfOff.length - 2; // s-1
        while (lo <= hi) {
            const mid = (lo + hi) >> 1;
    ...
    indexOfKey(key) {
        if (!this.keyToIdx) {
            ...
            for (let i = 0; i < this.size; i++)
                this.keyToIdx.set(this.key(i), i);
        }
...
export async function openGazetteer(dataDir = DATA_DIR) {
    return timed(async () => {
        try {
...
export function openGazetteerSync(dataDir = DATA_DIR) {
...
function resolverFor(bin) {
    const stopNames = new Set([...GAZETTEER_CONFIG.stopNames, ...bin.stopSurfaces()]);
    return new GazetteerResolver(bin, { ...GAZETTEER_CONFIG, stopNames });
}
function trigrams(norm) {
    const str = ` ${norm} `;
    if (str.length <= 3)
        return [str];
    const out = [];
