...
import { depthFromTermType } from './place.js';
...
export class GazetteerIndex {
    ...
    majorSet;
    majorCities;
    constructor(places, parentsArr, surface, 
    ...
    patterns = {}) {
        ...
        for (const p of places) {
            if (p.depth > (this.maxDepthByLang.get(p.languageCode) ?? 0))
                this.maxDepthByLang.set(p.languageCode, p.depth);
        }
        ...
        for (const [norm, idxs] of this.surfaceMap) {
            for (const idx of idxs)
                this.namesByPlace[idx].push(norm);
            for (const tri of trigrams(norm)) {
                let set = this.trigramIdx.get(tri);
                if (!set)
                    this.trigramIdx.set(tri, (set = new Set()));
                for (const idx of idxs)
                    set.add(idx);
            }
        }
    ...
    computeMajor() {
        const major = new Set();
        const slugOf = (key) => key.slice(key.lastIndexOf(':') + 1);
    ...
    static async load(dir) {
        ...
        if (data.schemaVersion !== GAZETTEER_SCHEMA_VERSION) {
            throw new Error(`Gazetteer schema ${data.schemaVersion} != ${GAZETTEER_SCHEMA_VERSION}; rebuild with 'npm run build:gazetteer'.`);
        }
    ...
    static fromTerms(locationTerms, edges, patterns) {
    ...
    static build(locationTerms, edges) {
        ...
        for (let idx = 0; idx < surfacesByIdx.length; idx++) {
            for (const norm of surfacesByIdx[idx])
                (surface[norm] ??= []).push(idx);
        }
        // Hierarchy: child -> nearest parent, then walk up to collect ancestors.
        const parentKeyOf = new Map();
        for (const e of edges)
            parentKeyOf.set(e.childKey, e.parentKey);
        const parents = places.map((p) => {
            const chain = [];
            let cur = parentKeyOf.get(p.canonicalKey);
            const seen = new Set([p.canonicalKey]);
    ...
    static async save(dir, locationTerms, edges) {
