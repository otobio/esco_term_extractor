...
export class GazetteerResolver {
    gaz;
    cfg;
    constructor(gaz, cfg = GAZETTEER_CONFIG) {
        this.gaz = gaz;
        this.cfg = cfg;
    }
    resolve(clauses, structuredLocation, countryCode) {
        const mentions = [];
        if (structuredLocation)
            this.scanSpans(structuredLocation, 'structured', mentions);
        ...
        if (!scoped.length)
            return [];
        const adminInText = new Set();
        for (const m of scoped) {
            for (const idx of m.idxs) {
                if (!this.gaz.isLeaf(idx))
                    adminInText.add(idx);
            }
        }
        ...
        for (const m of scoped) {
            ...
            const score = m.source === 'structured'
    ...
    scanSpans(text, source, out) {
        ...
        while (i < toks.length) {
            ...
            for (let n = Math.min(MAX_NGRAM, toks.length - i); n >= 1; n--) {
                ...
                if (n === 1 && source === 'text' && this.cfg.stopNames.has(span))
                    continue;
            ...
            if (!matched) {
                if (fuzzyOn && !(source === 'text' && this.cfg.stopNames.has(toks[i]))) {
                    ...
                    if (fz)
                        out.push({ idxs: [fz.index], spanWords: 1, fuzzy: true, source, text: toks[i] });
                }
                i += 1;
            }
        }
    }
    scanSubdivisions(text, out) {
    ...
    accept(idx, m, adminInText) {
        if (m.source === 'structured')
            return true;
        if (m.spanWords >= 2)
            return true;
        if (this.gaz.isMajor(idx))
            return true;
        if (this.isPrimarySeat(idx))
            return true;
    ...
    isPrimarySeat(idx) {
        ...
        for (const pi of this.gaz.parentsOf(idx)) {
            if (this.gaz.isLeaf(pi))
                continue;
            const parent = baseName(this.gaz.place(pi).displayName);
            if (parent === name || parent.startsWith(`${name} `))
                return true;
        }
        return false;
    }
    corroborated(idx, adminInText) {
        return this.gaz.parentsOf(idx).some((q) => adminInText.has(q));
    }
    disambiguate(gated, adminInText) {
        if (gated.length === 1)
            return gated;
