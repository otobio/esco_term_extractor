...
const CONTAINER_KINDS: ReadonlySet<LocationKind> = new Set(['country', 'admin1', 'admin2', 'admin3']);
...
export interface LocationRecord {
  key: string; // canonical, e.g. location:depth2:cluj
  sourceId: string;
  parentKey: string | null;
  name: string;
  kind: LocationKind;
  depth: number; // country=0, coarsest subdivision=1, … leaf deepest
  countryCode: string; // runtime locale gate (ro/ng/hu/et)
  population: number | null;
  isCapital: boolean;
  isAdminSeat: boolean;
  prominence: number;
  dominant: boolean; // clearly dominates same-name siblings in-country
  surfaces: Surface[];
  stopword: boolean;
  lat: number | null;
  lng: number | null;
...
export interface DatasetMeta {
  ...
  generatedAt: string;
  sources: string;
  thresholds: { minPopulation: number; dominanceRatio: number };
  counts: { total: number; byKind: Record<string, number>; surfaces: number; stopwords: number; dominant: number };
  checksum: string;
}
...
const asBool = (v: unknown): boolean => v === true || v === 1 || v === '1' || String(v).toLowerCase() === 'true';
const asList = (v: unknown): string[] => {
  ...
  if (s.startsWith('[')) {
    try {
      ...
      if (Array.isArray(a)) return a.map(asStr).filter(Boolean);
    } catch {
      /* fall through */
    }
  }
...
const isLatinName = (s: string): boolean => /[a-zA-Z]/.test(s) && !/[Ͱ-ϿЀ-ӿ԰-ۿ　-鿿가-힯]/.test(s);
 ...
 */
export async function readGeonames(dir: string, files: GeonamesCountryFile[] = DEFAULT_GEONAMES): Promise<RawRow[]> {
  const out: RawRow[] = [];
  for (const cf of files) {
    ...
    const adm1 = new Map<string, string>(); // iso.a1 -> geonameid
    const kept: string[][] = [];
 ...
 */
export const ALT_LANGS_BY_COUNTRY: Record<string, ReadonlySet<string>> = {
...
export async function loadAlternateNames(
  v2path: string,
  rows: RawRow[],
  allowed: Record<string, ReadonlySet<string>> = ALT_LANGS_BY_COUNTRY,
  perGidCap = 20,
): Promise<Map<string, string[]>> {
  const gidCountry = new Map<string, string>();
  for (const r of rows) {
    const g = asStr(r.geonames_id);
    if (g) gidCountry.set(g, asStr(r.country_code));
  }
  ...
  for await (const line of rl) {
    ...
    if (!langs?.has(t[2]) || !t[3] || !isLatinName(t[3])) continue;
    let arr = out.get(t[1]);
    if (!arr) out.set(t[1], (arr = []));
    if (arr.length < perGidCap && !arr.includes(t[3])) arr.push(t[3]);
  }
  return out;
}
...
export function applyAlternateNames(rows: RawRow[], altMap: Map<string, string[]>): number {
  let hit = 0;
  for (const r of rows) {
...
export const DEFAULT_KIND_MAP: Record<string, LocationKind> = {
  country: 'country',
  admin1: 'admin1',
  state: 'admin1',
  province: 'admin1',
...
export const ADMIN_TYPE_WORDS: Record<string, Partial<Record<LocationKind, string[]>>> = {
...
function stripTypeWord(name: string, words: ReadonlySet<string>): string {
  const toks = normalizeText(name).split(' ').filter(Boolean);
  if (toks.length > 1 && words.has(toks[toks.length - 1])) toks.pop();
  else if (toks.length > 1 && words.has(toks[0])) toks.shift();
  return toks.join(' ');
}

/** Seed common-word collisions per locale (replace with a frequency-derived list). */
export const DEFAULT_COMMON_WORDS: Record<string, ReadonlySet<string>> = {
...
const slugify = (s: string): string => normalizeText(s).replace(/\s+/g, '_') || 'x';
const usableSurface = (norm: string): boolean => norm.length >= 3 && !/^\d+$/.test(norm);

/** Pure: raw rows → validated, pruned, enriched records + a report. */
export function enrich(raw: RawRow[], cfg: EnrichConfig = {}): { records: LocationRecord[]; report: EnrichReport } {
  ...
  const bySource = new Map<string, Interim>(items.map((it) => [it.sourceId, it]));

  // depth via parent walk (cycle-guarded)
  let orphans = 0;
  for (const it of items) {
    let d = 0;
    let cur: Interim | undefined = it;
    const seen = new Set<string>();
    while (cur?.parentSourceId) {
      if (seen.has(cur.sourceId)) break;
  ...
  for (const it of items) {
    ...
    if (CONTAINER_KINDS.has(it.kind) || it.isCapital || it.isSeat || rescued || (it.population ?? 0) >= minPop) {
      ...
      if (rescued && !it.isSeat && !it.isCapital && !CONTAINER_KINDS.has(it.kind)) rescuedUnknownPopulationSeats++;
    } else prunedSettlements++;
  }
  for (const id of [...keep]) {
    ...
    while (cur?.parentSourceId) {
      const p = bySource.get(cur.parentSourceId);
      if (!p || keep.has(p.sourceId)) break;
      keep.add(p.sourceId);
      cur = p;
    }
  }
  ...
  for (const it of kept) {
    ...
    const base =
      CONTAINER_KINDS.has(it.kind) || !parent ? slugify(it.name) : `${slugify(it.name)}_${slugify(parent.name)}`;
  ...
  for (const g of groups.values()) {
    ...
    if (top > 0 && top >= ratio * second) {
      ...
      dominantGroups++;
    } else ambiguousGroups++;
  }

  // surfaces (typed) + stopword + final records
  let stopwords = 0;
  const records: LocationRecord[] = kept
    .map((it) => {
      const seen = new Set<string>();
      const surfaces: Surface[] = [];
      const add = (raw: string, kind: SurfaceKind) => {
...
function buildMeta(
  ...
  sources: string,
  thresholds: DatasetMeta['thresholds'],
  jsonl: string,
): DatasetMeta {
...
export async function loadFile(dir: string): Promise<{ records: LocationRecord[]; meta: DatasetMeta }> {
  ...
  if (createHash('sha256').update(jsonl).digest('hex') !== meta.checksum) throw new Error('dataset checksum mismatch');
  return {
    records: jsonl
      .split('\n')
      .filter(Boolean)
      .map((l) => JSON.parse(l) as LocationRecord),
    meta,
  };
}
...
export function validate(records: LocationRecord[]): Issue[] {
  const issues: Issue[] = [];
  const byKey = new Map<string, LocationRecord>();
  for (const r of records) {
    if (byKey.has(r.key)) issues.push({ key: r.key, problem: 'duplicate key' });
    byKey.set(r.key, r);
  }
  for (const r of records) {
    if (!r.countryCode) issues.push({ key: r.key, problem: 'missing countryCode' });
    if (!r.surfaces.length) issues.push({ key: r.key, problem: 'no surfaces' });
    if (r.parentKey) {
      const p = byKey.get(r.parentKey);
      if (!p) issues.push({ key: r.key, problem: `parentKey ${r.parentKey} not found` });
      else if (p.depth !== r.depth - 1)
        issues.push({ key: r.key, problem: `depth ${r.depth} != parent.depth+1 (${p.depth})` });
    } else if (r.kind !== 'country') issues.push({ key: r.key, problem: 'non-country has no parent' });
  }
  return issues;
}
...
async function connect(cfg: MysqlSink): Promise<any> {
  const spec = 'mysql2/promise';
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const mysql: any = await import(spec).catch(() => {
...
export async function writeMysql(cfg: MysqlSink, records: LocationRecord[], meta: DatasetMeta): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < placeRows.length; i += chunk) {
      await conn.query(
        'INSERT INTO place (`key`,source_id,parent_key,name,kind,depth,country_code,population,is_capital,is_admin_seat,prominence,dominant,stopword,lat,lng,geonames_id) VALUES ?',
        [placeRows.slice(i, i + chunk)],
      );
    }
...
export async function statsMysql(cfg: MysqlSink): Promise<void> {
  const conn = await connect(cfg);
  try {
    ...
    const [byCountry] = await conn.query(
      'SELECT country_code, COUNT(*) n, SUM(dominant) dom, SUM(stopword) stop FROM place GROUP BY country_code',
    );
    ...
    console.log(`by kind: ${byKind.map((r: RawRow) => `${r.kind}=${r.n}`).join(' ')}`);
    console.log(
      `by country: ${byCountry.map((r: RawRow) => `${r.country_code}(${r.n}, dom ${r.dom}, stop ${r.stop})`).join(' ')}`,
    );
    if (run) console.log(`last build ${run.created_at} · checksum ${String(run.checksum).slice(0, 12)}…`);
  } finally {
    await conn.end();
  }
}
...
async function main(): Promise<void> {
  ...
  if (cmd === 'build') {
    ...
    console.log(
      `read ${report.read} · kept ${report.kept} · pruned ${report.prunedSettlements} · orphans ${report.orphans} · rescued (unknown-population admin seats) ${report.rescuedUnknownPopulationSeats}`,
    );
    console.log(
      `surfaces ${meta.counts.surfaces} · stopwords ${report.stopwords} · dominant groups ${report.dominantGroups} · still-ambiguous ${report.ambiguousGroups}`,
    );
  ...
  if (cmd === 'validate') {
    if (!values.file) throw new Error('validate requires --file <dir>');
