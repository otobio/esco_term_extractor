...
const ENGLISH_NON_VERB_SUFFIXES =
    /(ness|ship|ity|ance|ence|ment|tion|sion|ology|ography|ism|ful|ous|ive|ary|able|ible|less)$/i;
 ...
 */
export function isLikelyEnglishVerb(word: string): boolean {
    ...
    if (ENGLISH_NON_VERB_TOKENS.has(headWord)) return false;
    if (ENGLISH_IRREGULAR_VERBS.has(headWord)) return true;
    if (ENGLISH_NON_VERB_SUFFIXES.test(headWord)) return false;
 ...
 */
export function toEnglishVerbRootForm(word: string): string {
    const normalized = word.trim().toLowerCase();
    if (!normalized) return normalized;

    if (/[^aeiou]ies$/.test(normalized)) return normalized.replace(/ies$/, "y");
    if (/[^aeiou]ied$/.test(normalized)) return normalized.replace(/ied$/, "y");
    if (/(ss|sh|ch|x|z|o)es$/.test(normalized)) return normalized.replace(/es$/, "");
    if (/[a-z]s$/.test(normalized) && !/(ss|us)$/.test(normalized)) return normalized.replace(/s$/, "");
...
export function americanToBritishOrthography(text: string): string {
    ...
    const rules: Rule[] = [
        ...
        // -----------------------------
        {
            ...
            replace: (word) =>
                word.replace(/se$/i, "ce"),
        },
    ];

    return text.replace(/\b[A-Za-z]+\b/g, (word) => {
        return rules.reduce(
            (result, rule) => result.replace(rule.pattern, rule.replace as any),
            word,
        );
    });
}