...
export function defaultOccupationVectorManifestPath(sourceName, modelKey) {
...
export function defaultOccupationVectorMetadataPath(sourceName, modelKey) {
...
export function defaultOccupationVectorValuesPath(sourceName, modelKey) {
...
export async function loadOccupationVectorArtifactIfAvailable(sourceName, modelKey) {
...
export async function loadOccupationVectorArtifactRequired(sourceName, modelKey) {
...
export function scoreOccupationVectorArtifact(artifact, queryVector, queryVectorNorm, options) {
    ...
    for (const record of artifact.vectors) {
        ...
        const cosine = computeCosineSimilarityForRecord(artifact.vectorValues, record.index, artifact.dimensions, queryVector, queryVectorNorm, vectorNorm);
        if (cosine <= 0) {
            continue;
        }
        scored.push({
...
export function scoreOccupationVectorRecords(records, vectorValues, dimensions, queryVector, queryVectorNorm, options) {
    const scored = [];
    for (const record of records) {
        const vectorNorm = record.vectorNorm ?? undefined;
        const cosine = computeCosineSimilarityForRecord(vectorValues, record.index, dimensions, queryVector, queryVectorNorm, vectorNorm);
        if (cosine <= 0) {
            continue;
        }
        scored.push({
...
async function loadArtifact(manifestPath, sourceName, modelKey) {
    try {
...
function buildVectorsByFamilyNodeId(records) {
    const vectorsByFamilyId = new Map();
    for (const record of records) {
...
async function loadVectorMetadataRecords(metadataPath) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        records.push(validateVectorMetadataRecord(JSON.parse(line), metadataPath, index + 1, records.length));
    }
    return records;
}
async function loadVectorValues(vectorsPath, count, dimensions) {
...
function validateVectorMetadataRecord(value, metadataPath, lineNumber, index) {
    if (!isRecord(value)) {
...
function computeCosineSimilarityForRecord(vectorValues, recordIndex, dimensions, queryVector, queryVectorNorm, vectorNorm) {
