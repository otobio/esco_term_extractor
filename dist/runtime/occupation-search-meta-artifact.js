...
export function buildOccupationSearchMetaBinaryFiles(records, prefix) {
    ...
    const capabilityLocaleLabelRows = [];
    const detailRows = [];
...
function groupCapabilitiesByCanonicalKey(capabilities) {
...
async function loadArtifact(manifestPath, sourceName) {
    ...
    let capabilityLocaleLabelRows = null;
    const entryBase = {
        manifestPath,
        manifest,
        artifact: manifest,
    ...
    const entry = {
        ...
        get capabilityLocaleLabelRows() {
            capabilityLocaleLabelRows ??= readFileBackedFixedTableSync(capabilityLocaleLabelRowsPath, CAPABILITY_LOCALE_LABEL_ROW_WIDTH, manifest.capabilityLocaleLabelCount, DETAIL_ROW_PAGING);
 ...
 */
const INTERNED_STRINGS_BY_TABLE = new WeakMap();
function internedStringAt(strings, stringId) {
