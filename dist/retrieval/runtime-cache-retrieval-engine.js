...
export function createRuntimeCacheRetrievalEngine() {
...
export class RuntimeCacheAliasRetriever {
    async retrieve(options) {
        ...
        const exactRows = firstMatchingAliasRows(uniqueNonEmpty(options.exactAliasQueries), (value) => index.exactAliasesByLocaleAndValue.get(localeValueKey(options.locale, value)) ?? [])
            .sort(compareAliasRowsWithQueryPriority)
            .slice(0, size);
        const foldedRows = firstMatchingAliasRows(uniqueNonEmpty(options.foldedAliasQueries), (value) => index.foldedAliasesByLocaleAndValue.get(localeValueKey(options.locale, value)) ?? [])
            .sort(compareAliasRowsWithQueryPriority)
            .slice(0, size);
        ...
        const subphraseRows = phraseWindows.length === 0
            ...
            : subphraseCandidateRows
                .filter((row) => SEARCH_ALIAS_ROLES.has(row.alias_role) &&
...
function firstMatchingAliasRows(queries, rowsForQuery) {
...
export class RuntimeCacheOccupationRetriever {
    async retrieve(options) {
        const index = await loadRuntimeCacheIndex(options.sourceName);
...
async function loadRuntimeCacheIndex(sourceName) {
...
async function buildRuntimeCacheIndex(sourceName) {
    ...
    const exactAliasesByLocaleAndValue = new Map();
    const foldedAliasesByLocaleAndValue = new Map();
    const canonicalByLocaleAndFoldedLabel = new Map();
    for (const row of aliasRows) {
        if (SEARCH_ALIAS_ROLES.has(row.alias_role)) {
...
async function retrieveTextHits(index, options, familyNodeId) {
    ...
    const candidateRecords = candidateTextRecords(index, options.locale, preparedQuery.foldedTokens, familyNodeId).filter((record) => record.localeCodes.has(options.locale) &&
...
function buildTextRecord(record, index = 0) {
...
function mapTextFields(callback) {
    return Object.fromEntries(TEXT_FIELDS.map((field) => [field, callback(field)]));
}
...
function buildTokenPrefixMap(tokens) {
    const map = new Map();
...
function buildAliasTokenPostings(aliasRows) {
...
function buildTextFieldTokenPostings(records) {
    const postings = new Map();
    for (const record of records) {
        for (const locale of record.localeCodes) {
...
function candidateAliasRows(index, locale, phraseWindowTokens) {
    const rows = new Map();
...
function candidateTextRecords(index, locale, queryTokens, familyNodeId) {
    return tieredCandidateTextRecords(index, locale, queryTokens, familyNodeId);
}
function tieredCandidateTextRecords(index, locale, queryTokens, familyNodeId) {
    ...
    const broadFields = ['search_text', 'capability_text', 'ancestor_text'];
...
function appendAllTermFieldCandidates(index, selectedIndexes, locale, field, queryTokens, familyNodeId, limit) {
...
function appendUnionFieldCandidates(index, selectedIndexes, locale, field, queryTokens, familyNodeId, limit) {
...
function appendCandidateIndex(index, selectedIndexes, recordIndex, familyNodeId, limit) {
...
function buildAuthorityMatches(rawQuery, preparedQuery) {
    ...
    for (const query of rawQueries) {
        matches.push(authorityMatch('authority_060_raw_primary_phrase', OPENSEARCH_AUTHORITY_SCORE.RAW_PRIMARY_OR_CANONICAL_PHRASE, query, 'phrase', ['locale_primary_aliases_text', 'canonical_label']), authorityMatch('authority_070_raw_supporting_phrase', OPENSEARCH_AUTHORITY_SCORE.RAW_SUPPORTING_OR_REVIEWED_PHRASE, query, 'phrase', ['locale_supporting_aliases_text', 'reviewed_crosswalk_aliases_text', 'family_supporting_aliases_text', 'english_backbone_aliases_text']));
    }
    for (const query of preparedQueries) {
        matches.push(authorityMatch('authority_080_prepared_all_terms', OPENSEARCH_AUTHORITY_SCORE.PREPARED_ALL_TERMS, query, 'all_terms', ['locale_primary_aliases_text', 'canonical_label', 'locale_supporting_aliases_text', 'reviewed_crosswalk_aliases_text', 'family_supporting_aliases_text', 'english_backbone_aliases_text', 'aliases_text', 'search_text', 'capability_text']), authorityMatch('authority_090_strict_fuzzy', OPENSEARCH_AUTHORITY_SCORE.STRICT_FUZZY, query, 'fuzzy', ['locale_primary_aliases_text', 'canonical_label', 'locale_supporting_aliases_text', 'reviewed_crosswalk_aliases_text', 'family_supporting_aliases_text', 'english_backbone_aliases_text', 'aliases_text']));
    }
    for (const query of rawQueries) {
        matches.push(authorityMatch('authority_100_raw_all_terms', OPENSEARCH_AUTHORITY_SCORE.RAW_ALL_TERMS, query, 'all_terms', ['aliases_text', 'search_text', 'capability_text', 'ancestor_text']));
    }
    return matches;
}
...
function scoreTextRecord(record, authorityMatches, queryTokens, locale) {
    let rawScore = 0;
    ...
    for (const match of authorityMatches) {
        ...
        const matchedMatchFields = match.fields.filter((field) => fieldMatches(record.fieldTokens[field], record.fieldTokenText[field], record.fieldTokenSets[field], record.fieldTokensByPrefix[field], match.queryTokens, match.type, locale));
...
function fieldMatches(fieldTokens, fieldTokenText, fieldTokenSet, fieldTokensByPrefix, queryTokens, type, locale) {
    ...
    return queryTokens.every((queryToken) => (fieldTokensByPrefix.get(queryToken.slice(0, 3)) ?? []).some((fieldToken) => editDistanceAtMostOne(fieldToken, queryToken)));
}
function buildFieldSignals(record, queryTokens, locale) {
    return TEXT_FIELDS
        .map((field) => buildFieldSignal(field, record.fieldTokens[field], record.fieldTokenText[field], record.fieldTokenSets[field], queryTokens, locale))
        .filter((signal) => signal !== null);
}
function buildFieldSignal(field, fieldTokens, fieldTokenText, fieldTokenSet, queryTokens, locale) {
...
function maxTokenCoverage(signals) {
...
function compareAliasRowsWithQueryPriority(left, right) {
    return ((left.queryPriority ?? Number.POSITIVE_INFINITY) - (right.queryPriority ?? Number.POSITIVE_INFINITY) ||
        compareAliasRows(left, right));
}
function localeValueKey(locale, value) {
    return `${locale}\0${value}`;
}
function localeFieldValueKey(locale, field, value) {
...
function sortMapValues(map, compare) {
