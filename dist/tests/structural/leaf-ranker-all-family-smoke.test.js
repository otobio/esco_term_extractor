...
test('all-family leaf ranker preserves stable top-leaf smoke cases', async () => {
    for (const [jobTitle, expectedLeaf] of ALL_FAMILY_RANKER_SMOKE_CASES) {
        ...
        assert.equal(foldSearchText(result.topLeaf), foldSearchText(expectedLeaf), `${jobTitle} ranked "${result.topLeaf}" (${result.topFamily}) score=${result.topScore} leaves=${result.leafCount}`);
    }
});
