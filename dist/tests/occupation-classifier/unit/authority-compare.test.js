...
test('isAuthorityTier classifies none/assistant/junior/senior/lead as non-authority tiers', () => {
    for (const tier of ['none', ...NON_AUTHORITY_TIERS]) {
...
test('leafAuthorityLevelKindsContradict does not contradict an unspecified query level against a non-authority leaf', () => {
    for (const leafLevel of ['none', ...NON_AUTHORITY_TIERS]) {
...
test('leafAuthorityLevelKindsContradict contradicts an unspecified query level against an authority-tier leaf', () => {
    for (const leafLevel of AUTHORITY_TIERS) {
        assert.equal(leafAuthorityLevelKindsContradict('none', leafLevel), true, `query "none" vs leaf "${leafLevel}" should contradict (authority tier must be explicitly requested)`);
    }
});
...
test('leafAuthorityLevelKindsContradict does not contradict a non-authority-tier query against a leaf with no level at all', () => {
    for (const queryLevel of NON_AUTHORITY_TIERS) {
        assert.equal(leafAuthorityLevelKindsContradict(queryLevel, 'none'), false, `query "${queryLevel}" vs leaf "none" should not contradict (both non-authority tier)`);
    }
});
...
test('leafAuthorityLevelKindsContradict does not contradict when both sides are authority tiers', () => {
    for (const queryLevel of AUTHORITY_TIERS) {
        for (const leafLevel of AUTHORITY_TIERS) {
            assert.equal(leafAuthorityLevelKindsContradict(queryLevel, leafLevel), false, `query "${queryLevel}" vs leaf "${leafLevel}" should not contradict (both authority tiers)`);
        }
    }
});
test('leafAuthorityLevelKindsContradict does not contradict when both sides are non-authority tiers', () => {
    for (const queryLevel of NON_AUTHORITY_TIERS) {
        for (const leafLevel of NON_AUTHORITY_TIERS) {
            assert.equal(leafAuthorityLevelKindsContradict(queryLevel, leafLevel), false, `query "${queryLevel}" vs leaf "${leafLevel}" should not contradict (both non-authority tiers)`);
        }
    }
});
...
test('leafAuthorityLevelKindsContradict contradicts when the query names an authority tier the leaf does not have', () => {
    for (const queryLevel of AUTHORITY_TIERS) {
        for (const leafLevel of NON_AUTHORITY_TIERS) {
            assert.equal(leafAuthorityLevelKindsContradict(queryLevel, leafLevel), true, `query "${queryLevel}" vs leaf "${leafLevel}" should contradict (authority vs non-authority)`);
        }
    }
});
test('leafAuthorityLevelKindsContradict contradicts when the query names a non-authority tier but the leaf is an authority tier', () => {
    for (const queryLevel of NON_AUTHORITY_TIERS) {
        for (const leafLevel of AUTHORITY_TIERS) {
            assert.equal(leafAuthorityLevelKindsContradict(queryLevel, leafLevel), true, `query "${queryLevel}" vs leaf "${leafLevel}" should contradict (non-authority vs authority)`);
        }
    }
});
...
test('leafAuthorityLevelKindsContradict matches isAuthorityTier(query) !== isAuthorityTier(leaf) for every non-"none" query level', () => {
    for (const queryLevel of LEAF_LEVEL_KINDS) {
        ...
        for (const leafLevel of LEAF_LEVEL_KINDS) {
            const expectedContradiction = isAuthorityTier(queryLevel) !== isAuthorityTier(leafLevel);
            assert.equal(leafAuthorityLevelKindsContradict(queryLevel, leafLevel), expectedContradiction, `query "${queryLevel}" vs leaf "${leafLevel}" should be ${expectedContradiction}`);
        }
    }
});
