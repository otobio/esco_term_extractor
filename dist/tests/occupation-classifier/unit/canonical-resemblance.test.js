...
function candidate(canonicalLabel, evidence = {}) {
    return {
        graphNodeId: 1,
        canonicalLabel,
...
function comparisonQuery(canonicalExactKeys = [], translationUnits = []) {
    return {
        englishTokens: [],
        modifierTokens: translationUnits.flatMap((unit) => unit.alternatives.filter((alternative) => alternative.kind !== 'role_head').map((alternative) => alternative.token)),
        unresolvedTokens: [],
...
function retrievalRequest(canonicalExactKeys) {
    return {
...
function runtimeWithLeaf(canonicalLabel) {
    return {
        searchMetaArtifact: {
            getCoreRecord: () => ({
                graphNodeId: 1,
...
function structuralGateFor(queryProfile, canonicalProfile) {
...
function resemblance(query, canonicalLabel, evidence = {}) {
    ...
    return computeCanonicalResemblance(candidate(canonicalLabel, evidence), comparisonQuery(), queryProfile, canonicalProfile, structuralGateFor(queryProfile, canonicalProfile));
}
...
test('computeCanonicalResemblance marks exact canonical evidence as first-order authority', () => {
    ...
    const result = computeCanonicalResemblance(exactCanonicalCandidate, comparisonQuery(['electrician']), queryProfile, canonicalProfile, structuralGateFor(queryProfile, canonicalProfile));
...
test('computeCanonicalResemblance treats same-source translated concept aliases as alternatives', () => {
    ...
    const result = computeCanonicalResemblance(candidate('business consultant'), comparisonQuery(['business consultant'], units), queryProfile, canonicalProfile, structuralGateFor(queryProfile, canonicalProfile));
    assert.equal(result.hasSharedModifierToken, true);
...
test('selectUniqueExactCanonicalLeaf does not shortcut when exact key misses concept-backed query tokens', () => {
    const result = selectUniqueExactCanonicalLeaf([{ graphNodeId: 1, weakFoldedCanonicalLabel: 'electrician' }], runtimeWithLeaf('electrician'), retrievalRequest(['electrician']), buildQueryStructuralProfile('solar system panel', 'en'));
    assert.equal(result, null);
});
test('selectUniqueExactCanonicalLeaf shortcuts when exact key covers a strong role head and concept-backed tokens', () => {
    const result = selectUniqueExactCanonicalLeaf([{ graphNodeId: 1, weakFoldedCanonicalLabel: 'technical receptionist' }], runtimeWithLeaf('technical receptionist'), retrievalRequest(['technical receptionist']), buildQueryStructuralProfile('technical receptionist', 'en'));
...
test('selectUniqueExactCanonicalLeaf treats translated concept alternatives as one required source unit', () => {
    ...
    const result = selectUniqueExactCanonicalLeaf([{ graphNodeId: 1, weakFoldedCanonicalLabel: 'business consultant' }], runtimeWithLeaf('business consultant'), retrievalRequest(['business consultant']), buildQueryStructuralProfile('business sales consultant', 'en'), comparisonQuery(['business consultant'], units));
