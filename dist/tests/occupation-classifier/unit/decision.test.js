...
test('selectDecision recovers the shared family when only leaves within the selection margin of the top score agree on it', () => {
    ...
    const family = buildFamilyAssessment({ familyNodeId: 100, familyLabel: 'Sales, marketing and public relations professionals', confidence: 0.6675 });
    const otherFamilyA = buildFamilyAssessment({ familyNodeId: 200, familyLabel: 'Sales and purchasing agents and brokers', confidence: 0.5425 });
    const otherFamilyB = buildFamilyAssessment({ familyNodeId: 300, familyLabel: 'Business services agents', confidence: 0.4225 });
    const rankedLeaves = [
        buildRankedLeaf({ graphNodeId: 1, canonicalLabel: 'technical sales representative in machinery and industrial equipment', familyNodeId: 100, familyLabel: family.familyLabel, score: 0.67 }),
        buildRankedLeaf({ graphNodeId: 2, canonicalLabel: 'technical sales representative in agricultural machinery and equipment', familyNodeId: 100, familyLabel: family.familyLabel, score: 0.65 }),
        buildRankedLeaf({ graphNodeId: 3, canonicalLabel: 'technical sales representative in office machinery and equipment', familyNodeId: 100, familyLabel: family.familyLabel, score: 0.65 }),
        buildRankedLeaf({ graphNodeId: 4, canonicalLabel: 'technical sales representative in electronic and telecommunications equipment', familyNodeId: 100, familyLabel: family.familyLabel, score: 0.59 }),
        buildRankedLeaf({ graphNodeId: 5, canonicalLabel: 'commercial sales representative', familyNodeId: 200, familyLabel: otherFamilyA.familyLabel, score: 0.54 }),
        buildRankedLeaf({ graphNodeId: 6, canonicalLabel: 'advertising sales agent', familyNodeId: 300, familyLabel: otherFamilyB.familyLabel, score: 0.42 })
    ];
    const outcome = selectDecision(rankedLeaves, [family, otherFamilyA, otherFamilyB], new Map(), emptyComparisonQuery());
...
test('selectDecision stays unresolved when the contending leaves within the margin disagree on family', () => {
    ...
    const outcome = selectDecision(rankedLeaves, [familyA, familyB], new Map(), emptyComparisonQuery());
