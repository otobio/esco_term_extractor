...
import { BASE_SPECIALIZATION_SCHEMA, DEFAULT_ROLE_HEAD_GROUPS, DEFAULT_SPECIALIZATION_SCHEMA, classifySpecializationQuery, classifySpecializationTitle, classifySpecializationTitleDetailed, getDefaultIndustryConceptIdsForRoleHeads, loadSpecializationSchemaFromCsv, tokenizeTitle } from '../../../src/occupation-classifier/specialization/specialization-dimension-mapper.js';
...
function readLeafStructureArtifact() {
    return JSON.parse(readFileSync(LEAF_STRUCTURE_PATH, 'utf8'));
}
...
test('classifySpecializationQuery maps shift and locale aliases to noop', () => {
    ...
    for (const testCase of cases) {
        ...
        assert.deepEqual(result.concepts.map((concept) => ({ conceptId: concept.conceptId, dimension: concept.dimension })), [{ conceptId: 'shift_noop', dimension: 'noop' }], testCase.query);
    }
});
...
test('classifySpecializationQuery keeps the most specific structural combination', () => {
    ...
    assert.deepEqual(contextOnly.structural_combination.map((match) => match.id), ['front_desk_context']);
    const withRoleHead = classifySpecializationQuery('front desk receptionist', { schema });
    assert.deepEqual(withRoleHead.structural_combination.map((match) => match.id), ['front_desk_receptionist']);
});
test('classifySpecializationQuery requires all structural-combination concepts', () => {
    ...
    assert.deepEqual(classifySpecializationQuery('front desk customer service', { schema }).structural_combination.map((match) => match.id), ['front_desk_customer_service']);
});
test('classifySpecializationQuery matches concept phrases across stopwords', () => {
    ...
    assert.deepEqual(speechWithStopword.concepts.map((concept) => concept.conceptId), ['speech_and_language']);
    ...
    assert.deepEqual(speechWithStopword.concepts.map((concept) => concept.conceptId), speechWithoutStopword.concepts.map((concept) => concept.conceptId));
    ...
    assert.deepEqual(machineryWithStopword.concepts.map((concept) => concept.conceptId), ['land_based_machinery_work_object']);
    ...
    assert.deepEqual(machineryWithStopword.concepts.map((concept) => concept.conceptId), machineryWithoutStopword.concepts.map((concept) => concept.conceptId));
...
test('classifySpecializationQuery, title, and detailed title agree on structural combinations', () => {
    ...
    assert.deepEqual(query.structural_combination.map((match) => match.id), ['front_desk_receptionist']);
});
test('every dumped structural combination names known concepts and role heads', () => {
    const conceptIds = new Set(readFixtureRows('specialization-concept-rules.csv')
        .map((row) => row.concept_id?.trim())
        .filter(Boolean));
    const roleHeads = new Set(readFixtureRows('specialization-role-heads.csv')
...
test('every role-head default industry concept names a known industry concept', () => {
    const industryConceptIds = new Set(readFixtureRows('specialization-concept-rules.csv')
...
test('reviewed stopword-compressed concept aliases resolve through the full mapper surface', () => {
    ...
    for (const [fileName, locale] of conceptFilesByLocale) {
        for (const row of readFixtureRows(fileName)) {
            ...
            const compressedClean = compressed.concepts.length === 1 && compressed.concepts.some((concept) => concept.conceptId === conceptId);
            if (!fullSurfaceOk) {
                fullSurfaceMisses += 1;
            }
...
test('esco leaf canonical labels keep specialization concept extraction stable', () => {
    ...
    const conceptWitnesses = new Map();
...
test('classifySpecializationQuery resolves every dumped role-head alias through the full mapper', () => {
    ...
    for (const row of readRoleHeadAliasFixtureRows()) {
        ...
        const aliasSignature = tokenizeTitle(alias).map(foldRoleHeadAliasToken).join('\u0001');
        const expectedRoleHeads = expectedRoleHeadsByAliasSignature.get(aliasSignature) ?? new Set();
        const variants = [{}, { locale: 'ro' }, { locale: 'hu' }];
...
test('classifySpecializationQuery lets dumped role-head phrases beat shorter dumped concept aliases', () => {
    ...
    for (const row of readRoleHeadAliasFixtureRows()) {
        ...
        const passed = variants.some((options) => {
            ...
            return (query.role_head.includes(roleHead) &&
...
test('classifySpecializationQuery uses locale-specific role-head phrase aliases without token alias leakage', () => {
    ...
    assert.deepEqual(detailed.assignments.map((assignment) => ({
...
test('classifySpecializationQuery keeps fallback-resolved literal concepts in the concept match list', () => {
    ...
    assert.deepEqual(result.concepts.map((concept) => [concept.conceptId, concept.dimension]), [
...
test('classifySpecializationQuery keeps the runtime role-head alias set unique', () => {
    const aliasPairs = (DEFAULT_SPECIALIZATION_SCHEMA.roleHeadAliases ?? []).map((row) => `${row.roleHead.toLowerCase()}${row.alias.toLowerCase()}`);
