...
export class OccupationAliasOpenSearchBulkIndexer {
    ...
    async countIndexableAliases(sourceName, limit) {
    ...
    async loadAliasRows(sourceName, lastGraphNodeId, lastAliasSortKey, limit) {
        const [rows] = await this.connection.query(`
        SELECT
          meta.graph_node_id,
    ...
    async bulkIndexDocuments(indexName, documents) {
        ...
        if ((body.errors ?? false) || failedDocumentCount > 0) {
            throw new Error(`OpenSearch alias bulk indexing failed for ${failedDocumentCount} documents.${failureExamples.length > 0 ? ` Examples: ${failureExamples.join(' | ')}` : ''}`);
        }
        return {
            indexedDocumentCount,
            failedDocumentCount
...
function buildOccupationAliasIndexProperties() {
    return {
...
function aliasDocumentId(document) {
...
function normalizePositiveInteger(label, value, fallback) {
...
function normalizeOptionalPositiveInteger(label, value) {
    if (value === undefined) {
        return undefined;
    }
