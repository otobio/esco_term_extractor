...
import { listEscoCsvFiles, locateEscoPack, readEscoFile } from '../../../utils/csv/read-esco-file.js';
...
export class EscoSourceImporter {
    ...
    async importFile(localeCode, pack, fileName) {
    ...
    async importConceptRow(sourceFileId, localeCode, fileFamily, row) {
    ...
    async importConceptSchemeRow(sourceFileId, localeCode, row) {
    ...
    async importRelationRow(sourceFileId, localeCode, fileFamily, row) {
    ...
    async markImportRun(status, notes) {
    ...
    async updateSourceFileRowCount(sourceFileId, rowCount) {
        await this.connection.execute(`
    ...
    async insertRawRow(sourceFileId, rowNumber, rowKey, externalUri, payload) {
        await this.connection.execute(`
    ...
    async upsertStubConcept(sourceFileId, localeCode, externalUri, conceptType, preferredLabel) {
    ...
    async upsertAlias(sourceConceptId, sourceFileId, localeCode, alias, aliasType, isPreferred, sourcePayload) {
    ...
    async insertMembership(sourceConceptId, collectionName, membershipType) {
        await this.connection.execute(`
...
function detectFileFamily(fileName, localeCode) {
...
function detectFileRole(fileFamily) {
    if (CONCEPT_FAMILIES.has(fileFamily)) {
...
function extractExternalUri(fileFamily, row) {
    switch (fileFamily) {
        case 'conceptSchemes':
...
function detectEntityKind(fileFamily, row) {
    switch (fileFamily) {
        case 'occupations':
...
function guessEntityKind(conceptType, externalUri) {
...
function buildRowKey(fileFamily, localeCode, rowNumber, row) {
...
function splitLineSeparatedValues(value) {
    if (!value) {
        return [];
    }
    return value
...
function firstFromMultiValue(value) {
...
function clipNullable(value, maxLength) {
    if (!value) {
        return null;
    }
    const trimmed = value.trim();
...
function buildCollectionName(externalUri, preferredLabel) {
    if (preferredLabel) {
