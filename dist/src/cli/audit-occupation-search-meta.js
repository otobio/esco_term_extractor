...
function parseCliOptions(args) {
    ...
    for (const arg of args) {
        ...
        if (arg.startsWith('--weak-english-backbone-threshold=')) {
            options.weakEnglishBackboneThreshold = parseThreshold('weak-english-backbone-threshold', arg.slice('--weak-english-backbone-threshold='.length));
            continue;
        }
...
function parseThreshold(flagName, value) {
...
function printHelp() {
    console.log(`Usage: node dist/cli/audit-occupation-search-meta.js [--source-name=${DEFAULT_ESCO_SOURCE_NAME}] [--locales=en,ro] [--format=text|json] [--sample-limit=10] [--weak-english-backbone-threshold=0.5] [--insert-review-queue]`);
}
main().catch((error) => {
