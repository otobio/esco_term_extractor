...
export async function importEuresEscoAliasCandidates(connection, options = {}) {
...
function buildCandidateRows(config, mappingRows, targetNodesByUri, existingAliasesByKey) {
...
function buildCandidate(config, row, alias, target, ownersByAlias, existingAliasesByKey) {
    const normalizedAlias = normalizeSearchText(alias);
    ...
    const importOwners = ownersByAlias.get(buildAliasKey(config.locale, normalizedAlias)) ?? new Set();
    const reasons = [];
    if (!normalizedAlias) {
...
async function loadTargetNodesByUri(connection, sourceName) {
...
function expandNationalLabels(value) {
...
function splitOnCommasOutsideParentheses(value) {
    const parts = [];
    let depth = 0;
    let current = '';
...
function scoreCandidateMode(mode) {
...
function buildAliasKey(localeCode, normalizedAlias) {
