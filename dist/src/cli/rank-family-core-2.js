...
import { familyCapabilityRelevanceMultiplier, tryLoadOccupationFamilyCapabilityRelevanceLookup } from '../query/occupation-family-capability-relevance.js';
...
import { getGenericHeadFamilyPriors, getGenericHeadFamilyContradiction, hasGenericHeadVenueContext } from '../search-pipeline/generic-head-family-priors.js';
...
import { compareFamilyStructureToQuery, getFamilyStructureRule, familyStructureSupportScore } from '../runtime/occupation-family-structure-rules.js';
import { BRANCH_MARGIN_POLICY, EVIDENCE_NORMALIZATION_POLICY, FAMILY_SCORING_POLICY, GENERIC_RISK_PENALTY, PIPELINE_DECISION_GATE } from '../scoring/scoring-policy.js';
import { clampScore, roundScore } from '../utils/operators.js';
import { foldSearchText, tokenizeNormalizedText } from '../utils/texts.js';
export function exactRoleMatchThreshold(preparedQuery) {
    if (preparedQuery.intent.roleTokens.length === 0) {
        return 0;
    }
    return preparedQuery.intent.roleTokens.length >= 2 ? 2 : 1;
}
export function rankFamilyCandidatesForRecovery(preparedQuery, sourceName, topFamilyLimit, candidateFamilies, candidateLeavesByFamilyKey, jobFunction = null) {
    ...
    if (hasGenericHeadVenueContext(preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens)) {
        ...
        const priors = getGenericHeadFamilyPriors(authoritativeRoleHeadTokens(preparedQuery), preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens, Boolean(preparedQuery.commonRolePhraseMatch || preparedQuery.familyAliasMatch)) ?? [];
...
export function rankFamilyCandidatesForSelection(preparedQuery, families, recoverAuthority) {
    return families
        .map((family) => {
...
function scoreFamily(input) {
    ...
    const existingScore = 'rankingScore' in family ? family.rankingScore : undefined;
    ...
    if (preparedQuery) {
        ...
        const branch = Math.max(family.branchShare, ratioToScore(family.branchMarginRatio, BRANCH_MARGIN_POLICY.WEAK_RATIO, BRANCH_MARGIN_POLICY.STRONG_RATIO));
        const exactOccupation = Math.max(maxEvidenceScore(family.evidence, ['exact_canonical']), maxEvidenceScore(family.evidence, ['exact_alias']));
        ...
        retrieval = clampScore(exactOccupation *
        ...
        semantic = clampScore(familyRoleCoverage(family.evidence, preparedQuery) * FAMILY_SCORING_POLICY.ROLE_COVERAGE_WEIGHT +
        ...
        prior = clampScore(maxEvidenceScore(family.evidence, ['reviewed_family_signal']) * FAMILY_SCORING_POLICY.REVIEWED_SIGNAL_WEIGHT +
        ...
        corroboration = clampScore((Math.min(supportingLeaves.length, FAMILY_SCORING_POLICY.MAX_BREADTH_LEAVES) / FAMILY_SCORING_POLICY.MAX_BREADTH_LEAVES) *
            FAMILY_SCORING_POLICY.HYBRID_SUPPORT_BREADTH_WEIGHT);
        contradiction = clampScore(familyGroupMismatch(family.familyNodeId, preparedQuery) * FAMILY_SCORING_POLICY.GROUP_MISMATCH_PENALTY_WEIGHT +
    ...
    if (authority) {
        retrieval = clampScore(Math.max(retrieval, authority.exactFamilyCanonical, authority.usefulExact) +
            Math.min(authority.exactAliasCount, 5) * 0.03 +
            Math.min(authority.foldedAliasCount, 5) * 0.02 +
            authority.branchShare * 0.06);
        semantic = clampScore(Math.max(semantic, authority.roleCoverage * FAMILY_SCORING_POLICY.ROLE_COVERAGE_WEIGHT +
        ...
        recovery = clampScore(Math.min(authority.exactRoleLeafCount, 5) * 0.05 +
        ...
        corroboration = clampScore(corroboration + Math.min(authority.exactEvidenceCount, 5) * 0.015 + Math.min(authority.bestRoleTokenMatchCount, 5) * 0.015);
        ...
        const usesRoleAgreementOrdering = preparedQuery !== undefined &&
...
function compareFamilyScores(left, right) {
    ...
    const reviewedSignalDelta = maxEvidenceScore(right.evidence, ['reviewed_family_signal']) - maxEvidenceScore(left.evidence, ['reviewed_family_signal']);
    const genericHeadPriorDelta = maxEvidenceScore(right.evidence, ['generic_head_family_prior']) - maxEvidenceScore(left.evidence, ['generic_head_family_prior']);
    ...
    return (leftContradicted - rightContradicted ||
...
function structuralNetScore(score) {
    return score.structuralRejected ? -1 : score.structuralSupport - score.structuralContradiction;
}
function applyGenericHeadPriorEvidence(family, preparedQuery) {
...
function hasPrimaryGenericHeadFamilyPrior(family) {
    return family.evidence.some((record) => record.channel === 'generic_head_family_prior' &&
...
function familyStructureComparison(familyNodeId, preparedQuery) {
    ...
    const support = clampScore((Math.min(rawSupport, FAMILY_STRUCTURE_MAX_EXPECTED_SCORE) / FAMILY_STRUCTURE_MAX_EXPECTED_SCORE) * FAMILY_STRUCTURE_SUPPORT_WEIGHT);
    const softContradictionCount = comparison.dimensions.filter((dimension) => dimension.contradicted && !comparison.reasons.some((reason) => reason.startsWith(`${dimension.dimension} contradiction:`))).length;
...
function familyEvidenceTier(evidence) {
    ...
    if (hasEvidence(evidence, 'reviewed_family_signal') ||
...
function familyAuthorityFloor(evidence, tier) {
...
function familyRoleCoverage(evidence, preparedQuery) {
    const roleTokens = preparedQuery.intent.roleTokens.length >= 2 &&
...
function familyDomainCoverage(evidence, preparedQuery) {
    return evidenceTokenCoverage(evidence, preparedQuery.intent.domainTokens, 'domain_coverage', 'matched_domain_terms');
}
function evidenceTokenCoverage(evidence, tokens, explicitCoverageKey, matchedTermsKey) {
    if (tokens.length === 0) {
        return 0;
    }
    ...
    for (const record of evidence) {
        ...
        addTokens(supported, tokens.filter((token) => tokenListHasEquivalent(matchedTerms, token) || tokenListHasEquivalent(matchedTokens, token)));
    }
...
function authoritativeRoleHeadTokens(preparedQuery) {
...
function tokenListHasEquivalent(values, token) {
    ...
    const foldedToken = foldSearchText(token);
    return (foldedValues.has(foldedToken) || expandTokenVariants([foldedToken], 'en').some((variant) => foldedValues.has(foldSearchText(variant))));
}
...
function familyGroupAgreement(familyNodeId, preparedQuery) {
...
function familyGroupMismatch(familyNodeId, preparedQuery) {
    const family = getOccupationFamilyContext(familyNodeId);
...
function jobFunctionPrior(family, supportingLeaves, preparedQuery, jobFunction) {
    ...
    const hasRoleEvidenceCoverage = evidenceTokenCoverage(family.evidence, groundingRoleTokens(preparedQuery), 'role_coverage', 'matched_role_terms') > 0;
...
function familyHasRoleGroundedLeaf(leaves, preparedQuery) {
    ...
    return leaves.some((leaf) => {
        ...
        return (roleHeadTokens.some((token) => tokenListHasEquivalent(labelTokens, token)) ||
...
function genericHeadPrior(familyNodeId, preparedQuery) {
    if (!hasGenericHeadVenueContext(preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens)) {
        return 0;
    }
    const prior = getGenericHeadFamilyPriors(authoritativeRoleHeadTokens(preparedQuery), preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens, Boolean(preparedQuery.commonRolePhraseMatch || preparedQuery.familyAliasMatch))?.find((entry) => entry.familyNodeId === familyNodeId);
...
function genericHeadContradiction(familyNodeId, preparedQuery) {
    return getGenericHeadFamilyContradiction(authoritativeRoleHeadTokens(preparedQuery), preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens, Boolean(preparedQuery.commonRolePhraseMatch || preparedQuery.familyAliasMatch), familyNodeId)
        ? 1
        : 0;
}
...
function familySpecializationPrior(familyNodeId, preparedQuery) {
...
function familyTokenRelevance(familyNodeId, preparedQuery, sourceName) {
...
function familyCapabilityContradiction(familyNodeId, preparedQuery, sourceName) {
    ...
    return clampScore(1 - familyCapabilityRelevanceMultiplier(lookup, preparedQuery.locale, familyNodeId, preparedQuery.usefulFoldedRecallTokens));
}
...
function hasEvidence(evidence, channel) {
    return evidence.some((record) => record.channel === channel);
}
function hasCoveredNgramAlias(evidence) {
...
function hasPreparedPhraseWindow(evidence) {
    return evidence.some((record) => {
        if (record.channel !== 'lexical') {
            return false;
        }
        return stringArrayDetail(record.details.matched_queries).some((query) => /^authority_(?:010|020|030|040|050)_prepared_.+_phrase_window_len_(?:[2-9]|\d{2,})_idx_\d+$/u.test(query));
    });
}
