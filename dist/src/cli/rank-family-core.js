...
import { getGenericHeadFamilyPriors, hasGenericHeadVenueContext } from '../search-pipeline/generic-head-family-priors.js';
...
export function selectFamiliesForRecovery(scoredFamilies, preparedQuery, topFamilyLimit) {
...
export function scoreFamilyCandidate(family, candidateLeafsByFamilyKey, preparedQuery, sourceName, jobFunction = null) {
...
export function rankFamilyCandidatesForRecovery(preparedQuery, sourceName, topFamilyLimit, candidateFamilies, candidateLeafsByFamilyKey, jobFunction = null) {
...
function expandFamilyCandidatesWithGenericHeadPriors(candidateFamilies, preparedQuery) {
    ...
    const priors = getGenericHeadFamilyPriors(authoritativeHeadTokens, preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens, Boolean(preparedQuery.commonRolePhraseMatch || preparedQuery.familyAliasMatch));
...
function buildBareFamilyCandidate(familyNodeId, familyLabel) {
    return {
...
function applyFamilyPriorEvidence(family, supportingLeafs, preparedQuery, jobFunction) {
    ...
    if (!hasEvidenceChannel(family.evidence, 'job_function_family_prior')) {
        ...
        const jobFunctionPrior = (Array.isArray(jobFunctionPriors) ? jobFunctionPriors : []).find((prior) => prior.familyNodeId === family.familyNodeId);
    ...
    if (!hasEvidenceChannel(family.evidence, 'generic_head_family_prior')) {
        const authoritativeHeadTokens = authoritativeIntentRoleHeadTokens(preparedQuery);
        const genericHeadPriors = getGenericHeadFamilyPriors(authoritativeHeadTokens, preparedQuery.intent.roleTokens, preparedQuery.intent.venueTokens, Boolean(preparedQuery.commonRolePhraseMatch || preparedQuery.familyAliasMatch));
...
function hasJobFunctionFamilyPriorRoleGate(family, supportingLeafs, preparedQuery) {
...
function hasGenericHeadFamilyPriorRoleGate(family, supportingLeafs, preparedQuery) {
    if (preparedQuery.intent.roleTokens.length === 0) {
        return false;
    }
    if (family.evidence.some((record) => record.channel === 'exact_alias' || record.channel === 'folded_alias')) {
...
function maxLeafFitScore(leafs) {
    if (leafs.length === 0) {
        return 0;
    }
