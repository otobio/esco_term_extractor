...
export function validateFamilies(_runtime, candidateLedger, exactFamilies, _comparisonQuery, queryProfile) {
    ...
    for (const candidate of candidateLedger.values()) {
        ...
        if (candidate.status === 'promotable') {
            ...
            const structureDecision = candidate.canonical.interestingResemblanceOrder > 0 || hasTrustworthyAliasMatch
                ? 'accept'
