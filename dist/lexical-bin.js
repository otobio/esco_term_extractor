...
export function pack(entries, byAlias) {
...
export class LexicalBin {
    ...
    constructor(ab) {
        ...
        if (dv.getUint32(0, true) !== MAGIC)
            throw new Error('not an LXB file');
        if (dv.getUint32(4, true) !== VERSION)
            throw new Error(`LXB version ${dv.getUint32(4, true)} != ${VERSION}`);
        ...
        this.size = n;
        const off = (i) => dv.getUint32(28 + i * 4, true);
        const bytes = new Uint8Array(ab);
        const readDict = (start, count) => {
            const list = [];
            let p = start;
