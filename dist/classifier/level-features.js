...
function charGrams(token) {
    ...
    for (let n = CHAR_MIN; n <= CHAR_MAX; n++) {
        for (let s = 0; s + n <= padded.length; s++)
            grams.push(`c:${padded.slice(s, s + n)}`);
    }
    return grams;
}
/**
 ...
 */
export function levelFeatures(title) {
    const toks = normalizeLoose(title).split(' ').filter(Boolean);
    const feats = [];
    toks.forEach((t, i) => {
        feats.push(`w:${t}`); // unigram
        if (i > 0)
            feats.push(`w2:${toks[i - 1]}_${t}`); // bigram — carries head×scope
 ...
 */
export function vectorize(title, vocab) {
