...
function readStringTableAt(bytes, dv, offset, expectedCount) {
...
export function packOccupationCapabilities(entries) {
    const sorted = [...entries].sort((a, b) => Buffer.compare(Buffer.from(a.occupationKey, 'utf8'), Buffer.from(b.occupationKey, 'utf8')));
    const n = sorted.length;
    const capSet = new Set();
    for (const e of sorted) {
        for (const k of e.essential)
            capSet.add(k);
        for (const k of e.optional)
            capSet.add(k);
    }
    ...
    const essentialPostingsList = [];
    const optionalPostingsList = [];
    sorted.forEach((e, i) => {
        essentialOff.writeUInt32LE(essentialPostingsList.length, i * 4);
        for (const k of e.essential)
            essentialPostingsList.push(capIdx.get(k));
        optionalOff.writeUInt32LE(optionalPostingsList.length, i * 4);
        for (const k of e.optional)
            optionalPostingsList.push(capIdx.get(k));
    });
...
export class CapabilitiesBin {
    ...
    constructor(ab) {
        ...
        if (dv.getUint32(0, true) !== MAGIC)
            throw new Error('not an OCB file');
        if (dv.getUint32(4, true) !== VERSION)
            throw new Error(`OCB version ${dv.getUint32(4, true)} != ${VERSION}`);
        const n = dv.getUint32(8, true);
        const m = dv.getUint32(12, true);
        this.size = n;
        const off = (i) => dv.getUint32(16 + i * 4, true);
    ...
    relation(occupationKey, capabilityKey) {
        ...
        for (let p = this.essentialOff[oi]; p < this.essentialOff[oi + 1]; p++) {
            if (this.essentialPostings[p] === ci)
                return 'essential';
        }
        for (let p = this.optionalOff[oi]; p < this.optionalOff[oi + 1]; p++) {
            if (this.optionalPostings[p] === ci)
                return 'optional';
        }
        return null;
    }
