...
const COMMON_OCCUPATION_EXEMPTIONS = [
    'engineer',
    'technician',
    'manager',
    'operator',
...
const COMMON_EMPLOYER_BRAND_PHRASES = ['bringo', 'pizza hut', 'travel free', 'delissima bakery', 'delissima'];
const COMMON_LOCATION_CONTEXT_MARKERS = [
    'pe teren',
    'zona',
    'oras',
    'municipiul',
...
const OCCUPATION_NOISE_PROFILE_INPUTS = {
    ro: {
        locale: 'ro',
...
export function getOccupationNoisePeelingProfile(locale) {
...
export function peelOccupationTitleNoiseWithProfile(title, profile) {
...
function didTransformChunks(originalChunks, classifiedChunks) {
...
function createNoopOccupationNoisePeelingResult(title, locale) {
    const originalTitle = String(title ?? '').trim();
    const normalizedTitle = normalizeSearchText(originalTitle);
    const chunk = {
...
export function extractOccupationTitleChunks(title) {
...
function classifyOccupationNoiseChunk(chunk, profile) {
...
function peelMixedNoiseSurface(surface, profile) {
...
function hasOccupationCoreToken(surface) {
...
function isPureNoisePeelToken(token, profile) {
...
function matchesNoiseRule(rule, surface, foldedText, searchText, profile) {
...
function classifyOrigin(surface, normalized, kind, locale, originHint) {
...
function containsKnownEmployerBrand(normalized) {
    return COMMON_EMPLOYER_BRAND_PHRASES.some((phrase) => containsNormalizedTerm(normalized, phrase));
}
function looksLikeVisibleAcronymBrand(surface, profile) {
    const normalizedSurface = normalizeSearchText(surface);
    const tokens = String(surface ?? '')
        .split(/\s+/u)
        .map((token) => token.replace(/^[\s"'“”‘’.,;:!?-]+|[\s"'“”‘’.,;:!?-]+$/gu, ''))
...
function looksLikeLanguageQualifier(normalized) {
