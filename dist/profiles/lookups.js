 ...
 */
import { finalizeFinite, osFinalize, SOURCE_PREF, } from '../matchers/finite.js';
...
const boundaryWordsFor = (locale) => BOUNDARY_WORDS[locale ?? ''] ?? BOUNDARY_WORDS.default;
/**
 ...
 */
export function computeResidual(clauses, scan, peelBuckets, locale, extraGrams) {
    ...
    return clauses.map((clause, ci) => {
        ...
        let run = [];
        const flush = () => {
            while (run.length && boundary.has(run[0]))
                run.shift();
            while (run.length && boundary.has(run[run.length - 1]))
                run.pop();
            if (run.length)
...
const uniq = (xs) => [...new Set(xs.map((s) => s.trim()).filter(Boolean))];
/** Per-clause grams tagged to `bucket` in the shared scan. */
const spansFor = (bucket, scan) => scan.map((hits) => hits.filter((h) => h.entry.bucket === bucket).map((h) => h.gram));
/**
 ...
 */
const GENERIC_OCCUPATION_HEADS = {
    en: new Set([
        'assistant',
        'manager',
        'coordinator',
...
const genericHeadsFor = (locale) => GENERIC_OCCUPATION_HEADS[locale ?? ''] ?? GENERIC_OCCUPATION_HEADS.default;
/** A bare (single-token) generic occupation head for the locale — folded to match. */
const isGenericHead = (surface, locale) => {
...
export function aliasLookup(bucket, opts = {}) {
    return {
        bucket,
 ...
 */
export function openSemanticLookup(bucket, opts = {}) {
    ...
    return {
        ...
        candidates: (clauses, scan, ctx) => {
            ...
            const perClause = spansFor(bucket, scan).map((grams) => dropGenericHeads ? grams.filter((g) => !isGenericHead(g, ctx.locale)) : grams);
            ...
            const add = (raw, source) => {
                ...
                if (!surface)
                    return;
                if (dropGenericHeads && isGenericHead(surface, ctx.locale))
                    return; // bare generic head → noise
                const prev = bySurface.get(surface);
                if (prev === undefined || SOURCE_PREF[source] > SOURCE_PREF[prev])
                    bySurface.set(surface, source);
            };
...
export const gazetteerLookup = {
    bucket: 'location',
    candidates: () => [],
    finalize: (_results, _clauses, ctx) => (ctx.locationTerms ?? []).map((t) => ({
        key: t.canonicalKey,
        name: t.displayName,
        score: t.score,
        lang: t.languageCode,
...
const PEEL_BUCKETS = ['level', 'workplace', 'schedule', 'employment', 'company_size'];
const OTHER_FINITE = ['benefits', 'company_type', 'collar_kind', 'qualifications', 'compensation'];
