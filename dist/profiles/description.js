...
const DESCRIPTION_ALLOWED_BUCKETS = new Set([
    'capabilities',
    'location',
    'qualifications',
    'benefits',
...
const CAPABILITY_CUE = /\b(experienta|experienta in|cunostinte|abilitati|competente|skill|skills|experience|knowledge|proficien(?:cy|t)|familiar(?:ity)?|expertise|competenc(?:e|ies)|ability|tapasztalat|ismeret|keszseg|kepesseg|jartassag|kogemus|teadmised|oskused|suutlikkus|vajalik|required|must have|nice to have)\b/i;
const QUALIFICATION_CUE = /\b(studii|diploma|diplome|licenta|certificat|autorizatie|limba|permis|licence|degree|diploma|certificate|certification|license|licence|language|diploma|vegzettseg|bizonyitvany|tanusitvany|jogositvany|nyelv|haridus|tunnistus|sertifikaat|keel)\b/i;
const BENEFIT_CUE = /\b(beneficii|oferim|avantaje|bonus|bonuri|bonuri de masa|asigurare|training platit|juttatas|juttatasok|elony|ajanlunk|pakume|huved|boonus|kindlustus|benefit|benefits|offer|we offer|perk|perks|insurance|voucher)\b/i;
const COMPENSATION_CUE = /\b(salariu|salariu fix|bonus|comision|prime|pachet salarial|ber|fizetes|jutalek|boonus|palk|tasu|salary|compensation|pay|commission|bonus)\b/i;
function headerLocales(locale) {
    if (locale === 'ro' || locale === 'hu' || locale === 'et')
        return [locale, 'en'];
    return ['ro', 'hu', 'et', 'en'];
}
...
function extractInlineHeaderRest(raw) {
    const m = raw.match(/[:?]\s*(.+)$/u);
    return m?.[1]?.trim() ?? '';
}
function scoreSectionFamily(kind, family, norm, raw) {
    let score = 0;
    const reasons = [];
    let matchedPrefix;
...
function scoreHeaderLine(line, locale) {
    ...
    if (!strong) {
        if (tokenCount > 4)
            return null;
        if (!/[?:]$/.test(raw) && !titleLike)
            return null;
        if (first.score < 4.5)
            return null;
    }
    else {
        if (!hasDelimiter && prefixWords > 0 && tokenCount > prefixWords + 1 && !titleLike)
            return null;
    }
    if (second && first.score - second.score < 1 && first.score < 7)
        return null;
    return first;
}
function scoreBlockKind(kind, text, locale) {
...
function classifyBlock(text, locale) {
    const scores = ['requirements', 'responsibilities', 'benefits', 'company', 'application']
 ...
 */
function descriptionClausesFromLines(lines) {
    const out = [];
    let buffer = '';
    const flushBuffer = () => {
        if (!buffer)
        ...
        for (const part of buffer.split(/[;]+/)) {
            const clause = part
                ...
                .replace(/[.,:]+$/g, '')
                .trim();
            if (clause && classifyClause(clause).keep)
                out.push(clause);
        }
        buffer = '';
    };
...
export function parseDescriptionSections(text, locale) {
    ...
    if (!out.length && text.trim())
        out.push({ kind: 'unknown', lines: [{ text: text.trim(), hadBullet: false }] });
...
function sectionedClauses(sections, kinds, gate) {
    return sections.flatMap((section) => kinds.has(section.kind)
        ? section.clauses
            .filter((clause) => (gate ? gate(clause, section.kind) : true))
            .map((clause) => ({ text: clause, source: section.kind, section: section.kind }))
        : []);
}
function shortListLike(clause) {
    const count = words(normalizeText(clause)).length;
    return count > 0 && count <= 6;
}
function capabilityClauseAllowed(clause, section) {
    if (section === 'requirements')
        return true;
    return CAPABILITY_CUE.test(clause) || shortListLike(clause);
}
function qualificationClauseAllowed(clause, section) {
    if (section === 'requirements')
        return true;
    return QUALIFICATION_CUE.test(clause);
}
function benefitClauseAllowed(clause, section) {
    if (section === 'benefits')
        return true;
    return BENEFIT_CUE.test(clause);
}
function compensationClauseAllowed(clause, section) {
    if (section === 'benefits')
        return true;
    return COMPENSATION_CUE.test(clause);
}
function allowedDescriptionBuckets(requested) {
...
function lexicalLanguages(locale) {
    if (!locale)
        return undefined;
...
async function resolveCapabilityTerms(clauses, deps) {
    ...
    const responses = await timed(() => deps.client.msearch(candidates.map((candidate) => {
        const q = strategyForBucket('capabilities').buildQuery({ bucket: 'capabilities', surface: candidate.surface, locale: deps.locale }, matchCtx);
        q._source = DISPLAY_SOURCE;
        return q;
    })), `description_capabilities clauses=${clauses.length} candidates=${candidates.length}`);
    const results = candidates.map((candidate, i) => ({ ...candidate, response: responses[i] }));
    return dedupeTerms(osFinalize('capabilities', results, { locale: deps.locale }));
}
function resolveFiniteDescriptionBucket(bucket, clauses, deps) {
    if (!clauses.length)
        return [];
    const langs = lexicalLanguages(deps.locale);
    const expand = (gram) => numberVariants(gram, deps.locale);
    const lexicalTerms = clauses.flatMap((clause) => deps.lexical.lookup(clause.text, bucket, langs, expand).map((hit) => toResolvedTerm(hit.entry, hit.gram)));
    return dedupeTerms(finalizeFinite(bucket, lexicalTerms, clauses, { locale: deps.locale, titleMode: false }));
}
function resolveLocationTerms(text, deps) {
    if (!deps.gazetteer)
        return [];
...
function sectionKindsOf(clauses) {
    return [...new Set(clauses.map((clause) => clause.section))];
}
export async function resolveDescription(rawText, deps) {
    ...
    const byBucket = {};
    const sectionsByBucket = {};
    if (buckets.includes('capabilities')) {
        const clauses = sectionedClauses(sections, new Set(['requirements', 'unknown']), capabilityClauseAllowed).slice(0, MAX_CLAUSES_PER_BUCKET);
    ...
    const finitePolicies = [
        {
            bucket: 'qualifications',
            sections: new Set(['requirements', 'unknown']),
            gate: qualificationClauseAllowed,
        },
        {
            bucket: 'benefits',
            sections: new Set(['benefits', 'unknown']),
        ...
        { bucket: 'workplace', sections: new Set(['requirements', 'benefits', 'unknown']) },
        { bucket: 'employment', sections: new Set(['requirements', 'benefits', 'unknown']) },
        { bucket: 'schedule', sections: new Set(['requirements', 'benefits', 'unknown']) },
    ];
    for (const policy of finitePolicies) {
        if (!buckets.includes(policy.bucket))
            continue;
