 ...
 */
function parseHeadcountNumber(raw) {
...
const STAGE_DISQUALIFIER = /\b(software|architecture|account|accounts|client|clients|customer|customers|solution|solutions|application|applications|system|systems|segment|sector|sales|market|markets|mindset|culture|environment|vibe|spirit|mentality|resource|grade|level|agreement|deal|deals|presence|reach|expansion|footprint)\b/;
...
const COMPILED_LOCS = RAW_LOCS.map((loc) => {
    ...
    const countPatterns = [
        ...
        new RegExp(`(${NUM_PATTERN})\\s*(?:\\+)?\\s*(?:-\\s*(?:${NUM_PATTERN})\\s*)?(?:de\\s+|of\\s+)?(?:${empGroup})`, 'giu'),
        ...
        for (const loc of COMPILED_LOCS) {
            ...
            for (const re of loc.countPatterns) {
                ...
                for (const m of loose.matchAll(re)) {
                    ...
                    if (loc.excludeRegex?.test(window))
                        continue;
                    const n = parseHeadcountNumber(m[1]);
                    if (!Number.isNaN(n) && n >= 1 && n <= 2_000_000)
