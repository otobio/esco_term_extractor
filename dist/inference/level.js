 ...
 */
import { defaultLevelLrModel, inferLevelLr, reconcileLevel } from '../classifier/level-lr.js';
import { inferFacetTerms } from './facets.js';
import { LOCALES } from './locales.js';
import { applyIdioms, collector, normalizeLoose, parseExperienceYears, } from './shared.js';
...
function levelFromYears(years) {
    if (years === 0)
        return { key: 'level:entry_level', score: 0.75 };
    if (years <= 2)
        return { key: 'level:junior', score: 0.7 };
    if (years <= 4)
        return { key: 'level:mid_level', score: 0.7 };
    return { key: 'level:senior', score: 0.75 };
}
export function inferLevel(clauses, languages, options) {
    ...
    for (const facet of inferFacetTerms('level', clauses, languages)) {
        ...
        if (BAND_KEYS.has(facet.canonicalKey))
            bandStated = true;
    }
    for (const c of clauses) {
        ...
        for (const L of LOCALES) {
            ...
            if (years !== null)
                yearsHits.push({ years, evidence: c.text });
        }
    }
    ...
    const base = terms();
    if (!options?.enableLr)
        return base; // default: pure regex, no model/data dependency
    const model = options.lrModel ?? defaultLevelLrModel();
    if (!model)
        return base; // no model available → graceful regex-only fallback
