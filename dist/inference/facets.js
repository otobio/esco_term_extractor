...
const RECORDS = [
    ...
    r('job_function', 'hu', ['Bébiszitter', 'Csecsemő- és Kisgyermeknevelő', 'Házvezető, Gondnok, Bejáró(nő)', 'Kisgyermekgondozó'], ['job_function:animal_care_childcare_cleaning']),
    r('job_function', 'hu', [
    ...
    r('job_function', 'hu', ['Belsőépítész', 'Divat-, Stílustervező', 'Építészmérnök', 'Lakberendező', 'Textiltervező, Ruhatervező'], ['job_function:architecture_design']),
    r('job_function', 'hu', ['Betanított munka', 'Egyéb fizikai munka', 'Ipari alpinista', 'Mezőgazdasági munka'], ['job_function:physical_manual_work']),
    r('job_function', 'hu', [
        'Biológus',
        'Biotechnika',
        'Fizikus',
    ...
    r('job_function', 'hu', ['Környezetvédelem, Vízgazdálkodás', 'Mezőgazdaság, Élelmiszeripar', 'Növényorvos', 'Törzskönyvezés'], ['job_function:mining_natural_resources']),
    r('job_function', 'hu', ['Munka-, Egészségvédelem, Biztonságtechnika'], ['job_function:health_safety']),
    r('job_function', 'hu', ['Művelődésszervező', 'Nonprofit szervezetek', 'Szociális munkás'], ['job_function:community_social_services']),
    r('job_function', 'hu', [
    ...
    r('job_function', 'hu', ['Product owner, -manager', 'Projektmanager', 'Projektmenedzser', 'Projektmenedzsment', 'Scrum master'], ['job_function:project_management']),
...
const OCCUPATION_FAMILY_RECORDS = [
    {
        locale: 'hu',
...
function buildOccupationFamilyLookup(records) {
    const byLocale = new Map();
    for (const record of records) {
        const localeMap = byLocale.get(record.locale) ?? new Map();
...
export function lookupOccupationFamilySlugs(surface, locale) {
    const localeMap = OCCUPATION_FAMILY_LOOKUP.get(locale);
    if (!localeMap)
        return [];
