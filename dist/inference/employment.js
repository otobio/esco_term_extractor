...
import { applyIdioms, collector, normalizeLoose, parseDailyHours, parseWeeklyHours, } from './shared.js';
const EN = [
...
export function inferEmployment(clauses, languages) {
    ...
    for (const c of clauses) {
        ...
        for (const L of LOCALES) {
            ...
            if (weekly !== null)
                add(weekly >= 35 ? 'employment:full_time' : weekly <= 30 ? 'employment:part_time' : 'employment:full_time', 0.9, c.text);
            const daily = parseDailyHours(loose, L);
            if (daily !== null && daily >= 7)
                add('employment:full_time', 0.8, c.text);
            else if (daily !== null && daily <= 5)
                add('employment:part_time', 0.8, c.text);
        }
    }
    return terms();
}
