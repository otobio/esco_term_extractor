...
export function createRuntime(config = {}) {
    ...
    return {
        ...
        capabilities: () => (capabilitiesP ??= timed(() => OccupationCapabilityMap.load(dataDir), 'runtime_capabilities_load')),
    };
}
...
async function resolveFiniteStructured(items, runtime, options = {}) {
    ...
    for (const it of items) {
        ...
        if (it.bucket === 'job_function') {
            for (const t of await inferAltFamilyFromJobFunction(it.surface, it.locale)) {
...
function altOccupationCanonicalKey(term) {
    if (term.bucket === 'capabilities')
        return term.canonicalKey;
    const prefix = term.termType === 'occupation_group' ? 'occupation:alt_family' : 'occupation:alt';
    return `${prefix}:${term.canonicalKey}`;
}
function validationKeyForMatch(match) {
    if (match.bucket !== 'occupation')
    ...
    if (match.canonicalKey.startsWith('occupation:alt:'))
        return match.canonicalKey.slice('occupation:alt:'.length);
    return match.canonicalKey;
}
async function validateKnownMatches(matches, runtime) {
    const store = await runtime.displayTitles();
    if (!store)
        return matches;
    return matches.filter((match) => {
        if (match.bucket === 'location')
            return true;
...
function altToMatch(term, sourceText) {
    ...
    const evidenceSignal = term.bucket === 'capabilities'
        ? 'alt_capabilities'
...
async function altFamilyCollarMatches(term, sourceText, collar) {
...
async function deriveProfile(input, opts) {
    ...
    const [lexical, gazetteer, collar, capabilities] = await timed(() => Promise.all([
 ...
 */
async function deriveCapability(input, opts) {
    const lexical = await timed(() => opts.runtime.lexical(), 'ingest_derive_capability_lexical');
    const match = searchCapability(input, opts.locale, lexical);
    if (match.matchType === 'none' || !match.escoUri || match.confidence < MATCH_ACCEPT_THRESHOLD)
        return [];
    return [
        toMatch({
            key: match.escoUri,
            ...
            lang: opts.locale ?? 'global',
            status: 'resolved',
            span: input,
        }, 'capabilities', input, 'structured'),
    ];
}
...
export async function derive(input, opts) {
    ...
    const output = await timed(async () => {
        ...
        if (opts.bucket === 'capabilities')
            return deriveCapability(input, opts);
        ...
        return validateKnownMatches(results.map((r) => toMatch(r.term, r.bucket, r.sourceText, 'structured')), { displayTitles: opts.runtime.displayTitles });
 ...
 */
export async function getEnglishRelatedVerbs(verb, options = {}) {
    ...
    for (const result of results) {
        ...
        if (!candidate || seen.has(candidate) || !isLikelyEnglishVerb(candidate))
            continue;
        seen.add(candidate);
        verbs.push(candidate);
        if (options.limit && verbs.length >= options.limit)
            break;
    }
    return verbs;
}
 ...
 */
export async function getRelatedObjects(object, options = {}) {
