...
export function packOccupationCollars(entries) {
    ...
    const collarKeysSorted = [...new Set(sorted.map((entry) => entry.collar))].sort((a, b) => Buffer.compare(Buffer.from(a, 'utf8'), Buffer.from(b, 'utf8')));
...
export class CollarBin {
    ...
    constructor(ab) {
        ...
        if (dv.getUint32(0, true) !== MAGIC)
            throw new Error('not a CLB file');
        if (dv.getUint32(4, true) !== VERSION)
            throw new Error(`CLB version ${dv.getUint32(4, true)} != ${VERSION}`);
