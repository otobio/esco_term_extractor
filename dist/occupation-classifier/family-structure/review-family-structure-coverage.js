...
export function reviewFamilyStructureCoverage() {
    const leafFile = JSON.parse(readFileSync(INPUT_PATH, 'utf8'));
    const table = readFamilyRuleTable();
    const rowsByFamilyId = new Map(table.rows.map((row) => [Number(row.values.family_node_id), row]));
    const familyLeafCounts = new Map();
    ...
    for (const leaf of leafFile.records) {
        ...
        const trueFamilyAccepted = shortlist.accepted.some((assessment) => assessment.familyNodeId === leaf.familyNodeId && assessment.decision === 'accept');
        const wrongFamilyAccepted = shortlist.accepted.some((assessment) => assessment.familyNodeId !== leaf.familyNodeId && assessment.decision === 'accept');
...
export function applyFamilyStructureCoverageReview() {
...
function serializeFamilyRuleTable(header, rows) {
...
function incrementEvidence(evidenceCounts, familyNodeId, field, value) {
    if (!value) {
        return;
    }
...
function collectRequiredTrueFamilyEvidence(requiredEvidenceKeys, row, familyNodeId, profile) {
...
function collectRequiredTrueFamilyAcceptanceEvidence(requiredEvidenceKeys, familyNodeId, profile) {
    for (const roleHead of new Set(profile.profile.role_head)) {
...
function formatEvidenceKey(familyNodeId, field, value) {
    return `${familyNodeId}\t${field}\t${value}`;
}
function formatRequiredEvidenceKey(familyNodeId, field, value, reason) {
    return `${formatEvidenceKey(familyNodeId, field, value)}\t${reason}`;
}
function requiredReason(requiredEvidenceKeys, evidenceKey) {
...
function isStrongEnough(evidenceCount, evidenceRatio) {
    return ((evidenceCount >= MIN_REPEATED_EVIDENCE_COUNT && evidenceRatio >= MIN_REPEATED_EVIDENCE_RATIO) ||
        evidenceRatio >= MIN_SINGLE_EVIDENCE_RATIO);
}
function isConceptField(value) {
    return CONCEPT_FIELDS.includes(value);
}
function compareAdditions(left, right) {
    return (left.familyNodeId - right.familyNodeId ||
