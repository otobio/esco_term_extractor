...
export class RankedGapAnalysisReporter {
    ...
    async run(options) {
        ...
        if (queries.length === 0) {
            throw new Error(`Search run ${scope.searchRunId} has no matching evaluation queries for source_name="${scope.sourceName}" and set_key="${scope.setKey}".`);
        }
        ...
        const queryAnalyses = queries.map((query) => analyzeQueryGap(query, expectationsByQueryId.get(query.id) ?? [], resultsByQueryId.get(query.id) ?? []));
        const promotionCandidates = queryAnalyses.filter((query) => query.bucket === 'selected_miss_top1_leaf_hit' ||
        ...
        const riskyGaps = queryAnalyses.filter((query) => query.bucket === 'selected_miss_no_top3_leaf_hit' ||
...
export function formatRankedGapAnalysisReport(report, format = 'text') {
    if (format === 'json') {
        return JSON.stringify(report, null, 2);
    }
    const lines = [];
    lines.push('Ranked gap analysis complete.');
    lines.push(`run_id=${report.searchRunId}, run_label=${report.runLabel}, source_name=${report.sourceName}, set_key=${report.setKey}, query_count=${report.queryCount}`);
...
function analyzeQueryGap(query, expectations, results) {
...
function classifySelectedOutcome(expectations, expectationCount, selectedRow) {
    ...
    if ((selectedRow.decision_stage === 'selected_family' && expectations.familyIds.has(selectedRow.graph_node_id)) ||
        (selectedRow.decision_stage === 'selected_group' && expectations.groupIds.has(selectedRow.graph_node_id))) {
        return 'family_or_group_hit';
    }
    return 'miss';
}
function classifyBucket(classification, top1LeafHit, top3LeafHit, bestBroaderBranchHit, hasRankedEvidence) {
...
function buildExpectationSets(expectations) {
    return {
...
function findRankedResults(results) {
...
function extractTopLeaves(rankedResults) {
    ...
    return rawTopLeaves.map((item, index) => {
        ...
        return {
            ...
            lexicalScore: toNumber(channelScores.lexical),
            capabilityTaskScore: toNumber(channelScores.capability_task)
        };
    });
}
function extractBestBroaderBranch(rankedResults) {
...
function isBestBroaderBranchHit(branch, expectations) {
    ...
    return ((branch.branchKind === 'family' && expectations.familyIds.has(branch.branchNodeId)) ||
        (branch.branchKind === 'group' && expectations.groupIds.has(branch.branchNodeId)));
}
function buildCategoryBucketCounts(queries) {
    const counts = {};
...
function appendQueryList(lines, queries) {
    if (queries.length === 0) {
        lines.push('- none');
        return;
    }
...
function formatQueryGap(query) {
    ...
    if (correctLeaf) {
        parts.push(`correct_scores=retrieval:${formatNumber(correctLeaf.retrievalScore)},resolver:${formatNumber(correctLeaf.resolverScore)},leaf_margin:${formatNumber(correctLeaf.leafMarginRatio)},branch_share:${formatNumber(correctLeaf.branchShare)},branch_margin:${formatNumber(correctLeaf.branchMarginRatio)},lexical:${formatNumber(correctLeaf.lexicalScore)},cap:${formatNumber(correctLeaf.capabilityTaskScore)},exact:${formatNumber(correctLeaf.exactAliasScore)},folded:${formatNumber(correctLeaf.foldedAliasScore)}`);
    }
    return parts.join(' ');
}
...
function normalizeRequiredRunId(value) {
