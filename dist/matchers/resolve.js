...
export function strategyForBucket(bucket) {
    if (ADDITIVE_BUCKETS.has(bucket))
        return additiveHybridStrategy;
    if (FINITE_BUCKETS.has(bucket))
        return finiteLexicalStrategy;
...
export const buildFilters = (bucket, locale) => {
    const filters = [{ term: { bucket } }];
    if (locale) {
...
export async function resolveSurfaces(items, client) {
    if (items.length === 0)
        return [];
