...
export const SOURCE_PREF = { span: 2, residual: 1, clause: 0 };
function sameLocaleSurfaces(hits, key, locale) {
    if (!locale)
        return [];
    const out = [];
    for (const h of hits) {
        const s = h?._source;
        if (s?.canonical_key !== key || s?.language_code !== locale)
            continue;
...
function isGrounded(hit, surface, locale) {
...
export function osFinalize(bucket, results, ctx, strategy = strategyForBucket(bucket)) {
    const rank = (a, b) => Number(b.grounded) - Number(a.grounded) || b.pref - a.pref || b.score - a.score;
    const byKey = new Map();
    const push = (t) => {
        const prev = byKey.get(t.key);
        if (!prev || rank(t, prev) < 0)
            byKey.set(t.key, t);
    };
    const info = (h) => ({
    ...
    for (const { surface, source, response } of results) {
        ...
        const res = strategy.select(response, { bucket, surface, locale: ctx.locale });
