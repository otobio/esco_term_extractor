 ...
 */
import { SALARY_LOCALES } from './locales.js';
const CURRENCY_RE = [
    [/€|\beur\b|euro/i, 'EUR'],
...
const ALL_PERIODS = ['month', 'year', 'hour', 'week', 'day'];
...
const INFER_PERIODS = ['month', 'year'];
const WINDOW = 36;
...
function detectCurrency(window) {
    for (const [re, cur] of CURRENCY_RE)
        if (re.test(window))
            return cur;
    return undefined;
}
function hasAny(window, words, minLen = 3) {
    for (const w of words) {
        if (w.length < minLen)
            continue;
        if (new RegExp(`(?:^|[^\\p{L}])${w.replace(/[/]/g, '\\/')}(?:$|[^\\p{L}])`, 'u').test(window))
            return true;
    }
    return false;
}
function detectPeriod(window) {
    for (const L of SALARY_LOCALES) {
        for (const p of ALL_PERIODS) {
            ...
            if (hasAny(window, L.period[p], 4))
                return p;
        }
    }
    return undefined;
}
function detectTax(window) {
    for (const L of SALARY_LOCALES) {
        if (hasAny(window, L.gross))
            return 'gross';
        if (hasAny(window, L.net))
            return 'net';
    }
    return undefined;
}
function excluded(window) {
    return SALARY_LOCALES.some((L) => hasAny(window, L.exclude));
}
function hasCue(window) {
...
function classify(amount, currency, explicit) {
    if (currency) {
        if (explicit)
            return { period: explicit, periodInferred: false, valid: inRange(amount, BOUNDS[currency][explicit]) };
        for (const p of INFER_PERIODS) {
            if (inRange(amount, BOUNDS[currency][p]))
                return { period: p, periodInferred: true, valid: true };
        }
...
export function extractSalary(text) {
    ...
    for (const raw of collectRaw(norm)) {
        if (!Number.isFinite(raw.min) || raw.min <= 0)
            continue;
        ...
        if (max !== undefined && currency && c.period && !inRange(max, BOUNDS[currency][c.period]))
            max = undefined;
