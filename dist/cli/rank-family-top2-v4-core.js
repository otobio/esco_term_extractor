...
import { buildFamilyVectors, buildQueryVector } from './rank-family-top2-v3-core.js';
...
function computeFamilySpecificityStats(family, searchMetaArtifact, leafStructureArtifact) {
    ...
    const specializationLeafCount = leaves.filter((leaf) => leaf.baseRoleKind === 'specialized_base_role' ||
        leaf.specializationKinds.length > 0 ||
        leaf.authorityKind !== 'none' ||
        leaf.headPreservingSpecialization ||
        leaf.broadAliasRisk !== 'low' ||
        leaf.capabilityDominanceRisk !== 'low').length;
...
function computeFamilySpecificityScore(stats) {
    ...
    return clamp01(leafCountPressure * 0.2 +
...
function computeQuerySpecificity(preparedQuery, queryVector) {
    ...
    return clamp01(tokenSpecificityScore * 0.34 + roleScore * 0.18 + roleHeadScore * 0.16 + domainScore * 0.14 + venueScore * 0.08 + tokenCountScore * 0.1);
}
function scoreFamilyVector(family, specificityStats, queryVector, querySpecificity, familyProfileArtifact) {
    ...
    const specificityDirection = Math.abs(specificityGap) < 0.08 ? 'aligned' : specificityGap > 0 ? 'family_more_base' : 'family_more_specialized';
...
function tokenSetForText(value, locale) {
    return new Set(tokenizeNormalizedText(foldSearchText(value)).filter((token) => token.length >= 2 && !isStopQueryToken(token, locale) && !isGenericQueryToken(token, locale)));
}
...
function expDecay(value, halfLife) {
