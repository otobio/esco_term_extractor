...
import { defaultOccupationVectorManifestPath, defaultOccupationVectorMetadataPath, defaultOccupationVectorValuesPath } from '../runtime/occupation-vector-artifact.js';
async function main() {
    ...
    await writeFile(metadataPath, exportResult.vectors.map(({ vector, ...record }) => JSON.stringify(record)).join('\n') + '\n', 'utf8');
...
function toVectorRecord(row, dimensions, index) {
...
function parseEmbeddingJson(value, dimensions) {
...
function vectorValueBuffer(records, dimensions) {
