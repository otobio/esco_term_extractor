...
import { buildAliasNgramIndexFromRows, retrieveAliasNgramHits } from '../retrieval/alias-ngram-retriever.js';
async function main() {
    ...
    const result = await withConnection(async (connection) => {
        ...
        const summaries = [];
        for (const mode of ['conservative', 'high_recall']) {
            const indexesByLocale = new Map();
            let aliasCount = 0;
            let indexLoadMs = 0;
...
async function evaluateMode(mode, options, queries, indexesByLocale, ancestorIdsByNodeId, aliasCount, indexLoadMs) {
    const summary = {
        mode,
        aliasCount,
...
function getQueryKindSummary(summary, queryKind) {
...
function rankOfExpectedNode(hits, expectation) {
...
function rankOfExpectedContext(hits, expectation, ancestorIdsByNodeId) {
    ...
    const index = hits.findIndex((hit) => hit.graphNodeId === expectedContextNodeId ||
...
async function loadAncestorIdsByNodeId(connection) {
    ...
    for (const row of rows) {
        ...
        const ancestors = ancestorsByNode.get(nodeId) ?? new Set([nodeId]);
        ancestors.add(Number(row.ancestor_node_id));
        ancestorsByNode.set(nodeId, ancestors);
    }
    return ancestorsByNode;
}
async function loadEvaluationQueries(connection, options) {
    const filters = [];
...
async function loadAliasRows(connection, sourceName, locale, includeFamilySupportingAliases) {
    const [rows] = await connection.query(`
      (
        SELECT
          meta.graph_node_id,
...
function formatResult(result) {
    ...
    for (const summary of result.summaries) {
        ...
        for (const kindSummary of Array.from(summary.byQueryKind.values()).sort((left, right) => left.queryKind.localeCompare(right.queryKind))) {
            lines.push(`  ${kindSummary.queryKind}: exact_top5=${pct(kindSummary.exactTop5, kindSummary.evaluated)} (${kindSummary.exactTop5}/${kindSummary.evaluated}) context_top5=${pct(kindSummary.contextTop5, kindSummary.evaluated)} (${kindSummary.contextTop5}/${kindSummary.evaluated})`);
        }
