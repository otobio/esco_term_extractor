...
import { OccupationSearchPipeline, recoveredFamilySelectionAuthority } from '../search-pipeline/occupation-search-pipeline.js';
...
const FAMILY_SELECTION_AUTHORITY_CASCADE = [
...
function formatResult(options, preparedQuery, families) {
    const lines = [];
    ...
    const authorityByFamilyKey = new Map(families.map((family) => [
        family.familyKey,
    ...
    for (let index = 0; index < families.length - 1; index++) {
        ...
        lines.push(`${left.rank}. "${left.familyLabel}" vs ${right.rank}. "${right.familyLabel}": ` +
            describeDecisiveCriterion(left, right, authorityByFamilyKey.get(left.familyKey), authorityByFamilyKey.get(right.familyKey)));
    }
    return lines.join('\n');
}
function describeDecisiveCriterion(left, right, leftAuthority, rightAuthority) {
...
function formatFamily(family, authority) {
    return [
