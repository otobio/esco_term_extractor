...
type ConceptRule = { canonical: string; dimension: string };

let lookupsByLocale: Map<string, SpecializationSchemaLookup>;
let conceptRulesById: Map<string, ConceptRule>;
...
test('every specialization concept alias resolves to its concept id and dimension via the live schema loader', () => {
  const conceptFilesByLocale: [string, string][] = [
