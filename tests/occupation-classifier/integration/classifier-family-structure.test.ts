...
} from '../../../src/occupation-classifier/family-structure/family-structure.js';
  ...
  for (const record of searchMeta.getAllCoreRecords()) {
    ...
    for (const concept of profile.profile.concepts) {
      ...
      if (!rule?.conceptsByDimension.get(concept.dimension)?.includes(concept.conceptId)) {
        failures.push(
          `${record.familyNodeId} ${record.familyLabel}: missing ${concept.dimension}:${concept.conceptId} from ${record.canonicalLabel}`
        );
      }
    }
  }
...
test('classifier family structure accepts narrow machine-operator leaf role heads', () => {
  ...
  assert.ok(
    rubberMachineOperators.matchedConcepts.some((match) => match.dimension === 'work_object' && match.values.includes('v_belt_work_object'))
  );
});
...
test('classifier family structure shortlist exposes accepted and rejected buckets', () => {
  ...
  assert.equal(
    shortlist.accepted.some((comparison) => comparison.familyNodeId === 14867),
    false
  );
});
...
test('classifier leaf ranking only sees promotable candidates from structurally surviving families', () => {
  ...
  const families = validateFamilies({} as never, candidateLedger, [], comparisonQuery, queryProfile);
  ...
  assert.deepEqual(
    rankedLeaves.map((leaf) => leaf.canonicalLabel),
    []
  );
...
test('classifier keeps sales advisor in a sales family without promoting unsupported specialised leaves', async () => {
  ...
  assert.equal(
    result.familyAssessments.some((family) => family.familyNodeId === 14723 && family.structureDecision !== 'reject'),
    false
  );
});
...
test('classifier maps Romanian goods handler wording to transport and storage labourers', async () => {
  ...
  assert.deepEqual(
    result.familyAssessments.filter((family) => family.structureDecision === 'accept').map((family) => family.familyNodeId),
    [15251]
  );
});
...
test('classifier keeps logistics leader unresolved instead of drifting to armed forces', async () => {
  ...
  assert.equal(
    result.familyAssessments.some((family) => family.familyNodeId === 14659 && family.structureDecision === 'accept'),
    false
  );
});
...
test('classifier keeps plausible families alive through soft family-level mismatches', async () => {
  ...
  assert.notEqual(
    firstOfficerPilot.familyAssessments.find((family) => family.familyNodeId === 14867)?.structureDecision,
    'reject'
  );
  ...
  assert.notEqual(
    aiOptimisation.familyAssessments.find((family) => family.familyNodeId === 14802)?.structureDecision,
    'reject'
  );
  assert.equal(
    aiOptimisation.familyAssessments.some(
      (family) => [14750, 14759].includes(family.familyNodeId) && family.structureDecision === 'accept'
    ),
    false
  );
});
...
test('classifier recovers a constrained family when all leaf candidates are rejected', async () => {
  ...
  assert.equal(
    result.familyAssessments.find((family) => family.familyNodeId === 14965)?.supportKind,
    'rejected'
  );
});
