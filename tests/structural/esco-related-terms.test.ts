...
import {
  buildEscoRelatedTermsDataset,
  buildEscoVerbInventory,
  extractEscoLabelFact,
  type EscoSkillLabelRecord,
  type EscoSkillRelationRecord
} from '../../src/skills/esco-related-terms.js';
...
test('buildEscoVerbInventory keeps only verbs that meet the minimum occurrence threshold', () => {
  const labels: EscoSkillLabelRecord[] = [
...
test('buildEscoRelatedTermsDataset derives verb and object relationships from ESCO labels and relations', () => {
  ...
  const relations: EscoSkillRelationRecord[] = [
    {
      relationId: 99,
  ...
  const sameSkillVerb = dataset.verbRelatedRows.find(
    (row) => row.source_verb === 'train' && row.related_verb === 'teach' && row.relationship_type === 'same_skill'
  );
  ...
  const sameVerbObject = dataset.objectRelatedRows.find(
    (row) => row.source_object === 'employees' && row.related_object === 'staff' && row.relationship_type === 'same_verb'
  );
  ...
  const explicitRelation = dataset.verbRelatedRows.find(
    (row) => row.source_verb === 'train' && row.related_verb === 'coach' && row.relationship_type === 'esco_related_skill'
  );
