...
test('noise peeler removes only matched employment phrases, not the whole chunk', () => {
  assert.equal(
    peelOccupationTitleNoise('Lucrator comercial Full Time - Hervis Alba Iulia', 'ro'),
    'Lucrator comercial - Hervis Alba Iulia'
  );
...
test('noise peeler removes only matched brand, identifier, and hour substrings inline', () => {
  ...
  assert.equal(
    peelOccupationTitleNoise('Sales Advisor Nespresso Boutique Afi Cotroceni 8h', 'ro'),
    'Sales Advisor Nespresso Boutique Afi Cotroceni'
  );
...
test('noise peeler preserves untouched separators and brackets', () => {
  assert.equal(
    peelOccupationTitleNoise('Antrenor / instructor pentru gimnastica ritmica', 'ro'),
  ...
  assert.equal(
    peelOccupationTitleNoise('Senior Specialist Aprovizionare (Piese de Schimb) OTOPENI', 'ro'),
    'Senior Specialist Aprovizionare (Piese de Schimb) OTOPENI'
  );
});

test('noise peeler removes explicit experience, hashtag, compensation, language, and location noise', () => {
  assert.equal(
    peelOccupationTitleNoise('Inginer Tehnolog-Industria Cărnii (Experienta min. 5 ani)', 'ro'),
    'Inginer Tehnolog-Industria Cărnii'
  );
  assert.equal(peelOccupationTitleNoise('KFC Bran cauta colegi! Hai intr-o echipa #pebune!', 'ro'), 'KFC Bran Hai intr-o echipa');
  assert.equal(
    peelOccupationTitleNoise('Game Presenter / Crupier–3000 lei+400 lei tichete masa+ 1000 lei BONUS, Pipera-București', 'ro'),
    'Game Presenter / Crupier, Pipera-București'
  );
  assert.equal(peelOccupationTitleNoise('Vopsitor industrial - 2 schimburi', 'ro'), 'Vopsitor industrial');
  assert.equal(
    peelOccupationTitleNoise('Lucrător comercial Legume-Fructe | 2 zile lucrate/2 libere', 'ro'),
    'Lucrător comercial Legume-Fructe'
  );
  ...
  assert.equal(
    peelOccupationTitleNoise('Inginer Constructor 10000RON NET-URZICENI/ ODORHEIU SECUIESC', 'ro'),
    'Inginer Constructor URZICENI/ ODORHEIU SECUIESC'
  );
  assert.equal(peelOccupationTitleNoise('Collections Process Expert with Polish / Czech', 'ro'), 'Collections Process Expert');
  assert.equal(
    peelOccupationTitleNoise('Manipulant marfa - Aricestii Rahtivani (Engleza mediu)', 'ro'),
    'Manipulant marfa - Aricestii Rahtivani'
  );
  ...
  assert.equal(
    peelOccupationTitleNoise('Bolti dolgozó - Esztergom, Bánomi út 21003 hrsz. (6 hrs munkaidő)', 'hu'),
    'Bolti dolgozó - Esztergom, Bánomi'
  );
  assert.equal(
    peelOccupationTitleNoise('Bolti dolgozó - Szentendre, Dózsa György út 20. (8, 7, 6, 5 és 4 órás munkaidő)', 'hu'),
    'Bolti dolgozó - Szentendre, Dózsa György .'
  );
