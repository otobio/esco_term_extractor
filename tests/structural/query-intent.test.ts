...
import {
  ...
  BUILTIN_VENUE_CONTEXT_TERMS_BY_LOCALE,
  classifyOccupationQueryIntent,
  type OccupationIntentVocabulary
} from '../../src/query/query-intent.js';

function builtinProfile(localeCode: string) {
...
test('Romanian common job titles classify role heads and modifiers cleanly', () => {
  ...
  for (const scenario of scenarios) {
    ...
    assert.deepEqual(
      result.unresolvedModifierTokens,
      [...scenario.unresolvedModifierTokens],
      `expected unresolved tokens for ${scenario.tokens.join(' ')}`
    );
...
test('Romanian refactor coverage handles common role-head and non-role phrases correctly', () => {
  ...
  for (const scenario of scenarios) {
    ...
    assert.deepEqual(
      result.unresolvedModifierTokens,
      [...scenario.unresolvedModifierTokens],
      `expected unresolved tokens for ${scenario.label}`
    );
...
test('Romanian structural role heuristics improve grouped standalone and technical-tail cases', () => {
  ...
  for (const scenario of scenarios) {
    ...
    if ('roleHeadReasonIncludes' in scenario) {
      assert.ok(
        result.diagnostics.some((entry) => entry.kind === 'role_head' && entry.reason.includes(scenario.roleHeadReasonIncludes)),
        `expected role-head reason containing "${scenario.roleHeadReasonIncludes}" for ${scenario.label}`
      );
    }
  }
});
...
test('built-in locale venue vocab keeps core parity for non-English locales', () => {
  ...
  for (const [localeCode, terms] of Object.entries(expectedVenueTerms)) {
    const normalizedTerms = new Set(
      Array.from(BUILTIN_VENUE_CONTEXT_TERMS_BY_LOCALE[localeCode as keyof typeof BUILTIN_VENUE_CONTEXT_TERMS_BY_LOCALE]).map((term) =>
        term.normalize('NFKD').replace(/\p{M}/gu, '')
      )
    );
