...
export function getOpenSearchConfig(): OpenSearchConfig {
  return {
    ...
    requestTimeoutMs: readPositiveInteger(
      ...
      DEFAULT_OPENSEARCH_REQUEST_TIMEOUT_MS
    )
  };
}

export function defaultOpenSearchTemplateName(indexName: string): string {
...
function readPositiveInteger(key: string, value: string | undefined, fallback: number): number {
  if (!value) {
    return fallback;
  }
