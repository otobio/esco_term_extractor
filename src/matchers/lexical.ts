...
function selectLexical(response: unknown, item: SurfaceQuery): TermResolution {
  const [first, second] = topHits(response, item, 2);
  if (!first || first.score < LEXICAL_FLOOR) return { status: 'unresolved' };
...
export const lexicalStrategy: TermMatchStrategy = {
  name: 'lexical',

  buildQuery(item: SurfaceQuery, ctx): Record<string, unknown> {
    return {
      query: {
        bool: {
 ...
 */
export const finiteLexicalStrategy: TermMatchStrategy = {
  name: 'finite-lexical',
