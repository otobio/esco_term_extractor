...
import {
  defaultOccupationEmbeddingDimensions,
  defaultOccupationEmbeddingModelKey,
  OccupationEmbeddingBuilder
} from '../embeddings/occupation/build-occupation-embeddings.js';
import {
...
async function main(): Promise<void> {
  ...
  const result = await withConnection(async (connection) => {
    ...
    return builder.run({
      ...
      onProgress: (progress) => {
        ...
        console.log(
          `Embedded chunk of ${progress.chunkRowCount} rows for model "${progress.modelKey}" through graph_node_id=${progress.lastGraphNodeId}; processed ${progress.processedNodeCount}, inserted ${progress.insertedEmbeddingCount}${remainingSummary}.`
        );
      }
    });
  });
  ...
  console.log(
    `Occupation dense_text embeddings completed for source "${result.sourceName}" with model "${result.modelKey}" (${result.dimensions} dimensions, provider ${result.provider}, model id ${result.modelId}), ${skipSummary}${limitSummary}.`
  );
  console.log(
    `Processed ${result.processedNodeCount} graph nodes, inserted ${result.insertedEmbeddingCount} embeddings, skipped ${result.skippedEmptyTextCount} empty dense_text rows.`
  );
}
