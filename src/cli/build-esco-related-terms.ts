...
import { buildEscoRelatedTermsDataset, type EscoSkillLabelRecord, type EscoSkillRelationRecord } from '../skills/esco-related-terms.js';
...
type SourceImportRunRow = RowDataPacket & {
  id: number;
};

type SourceLabelRow = RowDataPacket & {
  skill_id: number;
  skill_uri: string;
  label: string;
  label_type: 'preferred_label' | 'alt_label' | 'hidden_label';
  source_label: string;
};

type SourceRelationRow = RowDataPacket & {
  relation_id: number;
  relation_kind: 'skill_skill' | 'broader_skill';
  locale_code: string;
  source_skill_id: number;
  source_skill_uri: string;
  source_label: string;
  related_skill_id: number;
  related_skill_uri: string;
...
async function main(): Promise<void> {
  ...
  await withConnection(async (connection) => {
    ...
    try {
      const localeSummaries: Array<{ locale: string; verbCount: number; objectCount: number }> = [];
...
async function loadLatestSourceImportRunId(connection: Connection, sourceName: string): Promise<number> {
  const [rows] = await connection.query<SourceImportRunRow[]>(
    `
      SELECT id
...
async function loadSkillLabels(connection: Connection, sourceName: string, locale: string): Promise<EscoSkillLabelRecord[]> {
  const [preferredRows] = await connection.query<SourceLabelRow[]>(
    `
      SELECT
        c.id AS skill_id,
  ...
  const [aliasRows] = await connection.query<SourceLabelRow[]>(
    `
      SELECT
        c.id AS skill_id,
        c.external_uri AS skill_uri,
...
async function loadSkillRelations(connection: Connection, sourceName: string, locale: string): Promise<EscoSkillRelationRecord[]> {
  const [rows] = await connection.query<SourceRelationRow[]>(
    `
      SELECT
        r.id AS relation_id,
        r.relation_kind,
...
async function deleteExistingRows(connection: Connection, sourceName: string, locale: string): Promise<void> {
  await Promise.all([
    connection.execute(
      `
...
async function insertVerbRows(connection: Connection, buildRunId: number, rows: SerializedRelatedRow[]): Promise<void> {
...
async function insertObjectRows(connection: Connection, buildRunId: number, rows: SerializedRelatedRow[]): Promise<void> {
  for (const row of rows) {
    await connection.execute(
      `
...
async function markBuildRunCompleted(
  connection: Connection,
  buildRunId: number,
  localeSummaries: Array<{ locale: string; verbCount: number; objectCount: number }>,
  notes: string | null
): Promise<void> {
  await connection.execute(
    `
  ...
  console.log(
    `persisted_locale_counts=${localeSummaries.map((summary) => `${summary.locale}:${summary.verbCount}/${summary.objectCount}`).join(',')}`
  );
}

async function markBuildRunFailed(connection: Connection, buildRunId: number, notes: string): Promise<void> {
  await connection.execute(
    `
      UPDATE ose_esco_related_term_build_runs
      SET status = 'failed',
...
function serializeVerbRows(sourceName: string, localeCode: string, rows: SerializedVerbRow[]): SerializedRelatedRow[] {
  return rows.map((row) => ({
    sourceName,
    localeCode,
...
function serializeObjectRows(sourceName: string, localeCode: string, rows: SerializedObjectRow[]): SerializedRelatedRow[] {
  return rows.map((row) => ({
    sourceName,
    localeCode,
...
function mergeSerializedRows(rows: SerializedRelatedRow[]): SerializedRelatedRow[] {
  const byKey = new Map<string, SerializedRelatedRow>();
...
function normalizeLabelType(value: string): EscoSkillLabelRecord['labelType'] {
