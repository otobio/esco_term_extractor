...
import {
  formatManualReviewInspection,
  ManualReviewQueueInspector,
...
function parseCliOptions(args: string[]): InspectManualReviewQueueOptions & { format: ManualReviewInspectFormat } {
  const options: InspectManualReviewQueueOptions & { format: ManualReviewInspectFormat } = {
    status: 'pending',
    format: 'text'
  };
...
function parseStatus(value: string): ManualReviewQueueStatus {
...
function parseReviewType(value: string): ManualReviewType {
  const normalized = value.trim().toLowerCase();

  if (
    normalized === 'alias_conflict' ||
...
function parseFormat(value: string): ManualReviewInspectFormat {
  const normalized = value.trim().toLowerCase();
