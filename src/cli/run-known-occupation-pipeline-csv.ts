...
import { OccupationSearchPipeline, type OccupationSearchPipelineResult } from '../search-pipeline/occupation-search-pipeline.js';

const execFileAsync = promisify(execFile);

// Set this to a commit hash locally when you want comparison by default.
const DEFAULT_COMPARE_COMMIT: string | null = null;
...
type ResultSummary = {
  ...
  topFamilyId: number | null;
  topLeafLabel: string | null;
  topLeafId: number | null;
  familyCsv: string;
  leafCsv: string;
  spanResults: string;
  error: string | null;
...
type DisplayColumn = 'source' | 'query' | 'selected' | 'family' | 'topLeaf' | 'decision' | 'timeMs';
...
async function runCurrentResults(options: CliOptions): Promise<Map<string, ResultSummary>> {
...
async function runCommitResults(options: CliOptions, commit: string): Promise<Map<string, ResultSummary>> {
...
async function runCommitPipelineQuery(worktreePath: string, options: CliOptions, query: string): Promise<Record<string, unknown>> {
  const args = [
...
function summarizePipelineResult(query: string, result: OccupationSearchPipelineResult, elapsedMs: number): ResultSummary {
...
function summarizeJsonResult(query: string, payload: Record<string, unknown>, elapsedMs: number): ResultSummary {
...
function summarizeError(query: string, error: unknown): ResultSummary {
  return {
    query,
    effectiveQuery: null,
    decisionType: null,
...
function buildComparisons(
  queries: string[],
  currentResults: Map<string, ResultSummary>,
  baselineResults: Map<string, ResultSummary> | null
): QueryComparison[] {
...
function hasMeaningfulChange(current: ResultSummary, baseline: ResultSummary): boolean {
  return (
    current.decisionType !== baseline.decisionType ||
...
function comparisonsToCsvRows(runLabel: string, compareCommit: string | null, comparisons: QueryComparison[]): CsvRow[] {
  const rows: CsvRow[] = [];
...
function compactSpanResults(result: OccupationSearchPipelineResult): string {
...
function compactJsonSpanResults(items: unknown[]): string {
  if (items.length === 0) {
    return '';
  }
...
function printSummaryTable(comparisons: QueryComparison[], compareCommit: string | null): void {
  const color = createColor(process.env.NO_COLOR === undefined);
  const baselineLabel = compareCommit ? shortCommit(compareCommit) : '';
  const headers: DisplayColumn[] = ['source', 'query', 'selected', 'family', 'topLeaf', 'decision', 'timeMs'];
...
function toDisplayRow(source: string, summary: ResultSummary, highlight: boolean, isBaseline: boolean): DisplayRow {
  return {
    source,
    query: summary.query,
...
function timestampRunLabel(): string {
