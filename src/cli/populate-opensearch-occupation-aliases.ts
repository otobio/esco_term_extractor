...
async function main(): Promise<void> {
  ...
  const result = await withConnection(async (connection) => {
    ...
    return indexer.run({
      ...
      onProgress: (progress) => {
        ...
        console.log(
          `Indexed chunk of ${progress.chunkDocumentCount} occupation alias docs through graph_node_id=${progress.lastGraphNodeId}; indexed ${progress.indexedDocumentCount}, failed ${progress.failedDocumentCount}${remainingSummary}.`
        );
      }
    });
  });
  ...
  console.log(
    `OpenSearch occupation alias population completed for index "${result.indexName}" from source "${result.sourceName}" with chunk size ${result.chunkSize}.`
  );
  console.log(
    `Attempted ${result.attemptedDocumentCount}/${result.totalCandidateCount} alias documents, indexed ${result.indexedDocumentCount}, failed ${result.failedDocumentCount}.`
  );
}
