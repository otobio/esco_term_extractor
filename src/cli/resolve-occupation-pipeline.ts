...
import {
  ...
  CORE2_PIPELINE_FAMILY_RANKING_STRATEGY,
  OccupationSearchPipeline,
  createTop2V4PipelineFamilyRankingStrategy,
  ...
  const result =
    options.evaluationQueryId === undefined
      ...
      : await withConnection((connection) =>
          ...
          ).run({ ...options, ...debugOptions, debugCollector })
        );
...
async function buildDebugPipelineOptions(
  options: CliOptions,
  runtime: OccupationRuntimeContext
): Promise<Pick<OccupationSearchPipelineOptions, 'debugFamilyRankComparisonStrategies'>> {
...
function parseDebugMode(value: string): CliOptions['debug'] {
...
function formatFamily(family: RankedPipelineFamily, color: Colorizer, debug: boolean): string {
  ...
  const capabilityBreakdown = family.leaves
    ...
    .map(
      (leaf) =>
        `"${leaf.canonicalLabel}":${leaf.capabilityFit?.tier}:matched=${leaf.capabilityFit?.matchedCapabilityTerms.join('/') || 'none'}:missing=${leaf.capabilityFit?.missingCapabilityTerms.join('/') || 'none'}`
    )
    .join(' | ');
...
function formatFamilyRankOutputDebug(result: OccupationSearchPipelineResult): string[] {
...
function formatFamilyRankOutputRows(rows: OccupationSearchPipelineResult['debug']['familyRankComparison']): string[] {
  if (rows.length === 0) {
    return ['    - none'];
  }
...
function formatLeaf(leaf: RankedPipelineLeaf, color: Colorizer, debug: boolean): string {
  const line = [
...
function debugFormatFamilyStructureSections(result: OccupationSearchPipelineResult): string[] {
...
function summarizeCandidateRetrievalEvidence(
  evidence: Array<{
    channel: string;
    alias?: string;
    textRole?: string;
  ...
  return (
    evidence
      ...
      .map((record) => {
        ...
        const matchType =
          typeof record.details?.match_type === 'string' && record.details.match_type ? ` match_type=${record.details.match_type}` : '';
        const aliasWeight =
          typeof record.details?.alias_weight === 'number' ? ` alias_weight=${formatScore(record.details.alias_weight)}` : '';
...
function formatFieldSignals(value: unknown): string {
...
function formatEvidenceRecordDetail(record: PipelineEvidenceRecord): string {
...
function formatTokenBuckets(preparedQuery: OccupationSearchPipelineResult['preparedQuery']): string {
