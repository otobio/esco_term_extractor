...
import {
  buildOccupationFamilyCapabilityRelevanceReviewData,
  buildOccupationFamilyCapabilityRelevanceBinaryFiles,
  defaultOccupationFamilyCapabilityRelevanceManifestPath,
  FAMILY_CAPABILITY_RELEVANCE_BINARY_SCHEMA_VERSION,
  type OccupationFamilyCapabilityRelevanceArtifactManifest
...
async function main(): Promise<void> {
  ...
  const manifest: OccupationFamilyCapabilityRelevanceArtifactManifest = {
...
function parseCliOptions(args: string[]): CliOptions {
  const options: CliOptions = {
    ...
    reviewJsonOutPath: defaultRuntimeReviewJsonPath(
      runtimeReviewArtifactBaseName('occupation-family-capability-relevance', DEFAULT_ESCO_SOURCE_NAME)
    )
  };
  ...
  for (const arg of args) {
    if (arg.startsWith('--source-name=')) {
      ...
      options.reviewJsonOutPath = defaultRuntimeReviewJsonPath(
        runtimeReviewArtifactBaseName('occupation-family-capability-relevance', options.sourceName)
      );
      continue;
    }
