...
import { defaultOccupationFamilyTokenRelevanceArtifactPath, type OccupationFamilyTokenRelevanceArtifact } from '../query/occupation-family-token-relevance.js';
...
function buildOccupationFamilyTokenRelevanceArtifact(
  records: RuntimeSearchMetaRecord[],
  sourceName: string
): OccupationFamilyTokenRelevanceArtifact {
  const totalFamilies = new Set(records.map((record) => record.familyNodeId).filter((id): id is number => id !== null)).size;
  const familiesByLocale: OccupationFamilyTokenRelevanceArtifact['familiesByLocale'] = {};
  const genericityByLocale: OccupationFamilyTokenRelevanceArtifact['genericityByLocale'] = {};
...
function buildLocaleFamilies(
  records: RuntimeSearchMetaRecord[],
  locale: string,
  totalFamilies: number
): {
  families: OccupationFamilyTokenRelevanceArtifact['familiesByLocale'][string];
  genericity: OccupationFamilyTokenRelevanceArtifact['genericityByLocale'][string];
} {
  ...
  const genericity: Record<string, { documentFrequency: number; maxRelevance: number; maxRelevanceFamilyNodeId: number }> = {};
  const families = Array.from(accumulatorsByFamily.values())
    .map((accumulator) => {
      ...
      const tokens: OccupationFamilyTokenRelevanceArtifact['familiesByLocale'][string][number]['tokens'] = {};
...
function familyAccumulator(
  accumulatorsByFamily: Map<number, FamilyAccumulator>,
  familyNodeId: number,
  familyLabel: string
): FamilyAccumulator {
