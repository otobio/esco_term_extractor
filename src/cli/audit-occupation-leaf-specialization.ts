...
import { OccupationSearchPipeline, type RankedPipelineLeaf } from '../search-pipeline/occupation-search-pipeline.js';
...
async function main(): Promise<void> {
  ...
  for (const row of rows) {
    ...
    const topLeaf: RankedPipelineLeaf | null = result.decision.decisionType === 'leaf' ? (result.rankedLeaves[0] ?? null) : null;
...
function reportFindings(scanned: number, findings: AxisFinding[]): void {
  console.log(`Scanned ${scanned} unique job titles.`);
  console.log(`Unsupported-specialization winners: ${findings.length}`);

  const byKind = new Map<LeafSpecializationKind, AxisFinding[]>();
...
async function readCsvRows(options: CliOptions): Promise<CsvRow[]> {
