...
const CLAUSE_BOUNDARY: Record<Locale, string> = {
 ...
 */
const VERB_OBJECT_STOP_WORDS: Record<Locale, string> = {
 ...
 *  conjunction phrase. */
function verbObjectWordGroup(locale: Locale, wordClass: string): string {
...
const RO_EXPERIENCE_WITH =
  /\b(?:experiență|experienta)\s+(?:de\s+lucru\s+|practică\s+|practica\s+)?(?:în|in|cu|despre)\s+([^,.;\n]+)/gi;
...
function roAVerbForms(stem: string): string {
...
const RO_VERB_OBJECT_A = new RegExp(
  String.raw`\b(?:să\s+|sa\s+)?(?:${RO_A_VERB_STEMS.map(roAVerbForms).join('|')})\s+(?:un|o|unei|unui)?\s*(${verbObjectWordGroup('ro', String.raw`[a-zăâîșț][a-zăâîșț0-9-]*`)})${CLAUSE_BOUNDARY.ro}`,
  'gi',
);
 ...
 *  it. */
const RO_VERB_OBJECT_IRREGULAR = new RegExp(
  String.raw`\b(?:să\s+|sa\s+)?(?:construi(?:e[sș]te|re|rea|t|tă|ască)?|preg(?:ă|a)t(?:i|e[sș]te|ire|irea|it|ită)|întreț(?:ine|inere|inerea|inut|inută)|intret(?:ine|inere|inerea|inut|inuta)|conduc(?:e|erea|ere)|condus[ăa]?)\s+(?:un|o|unei|unui)?\s*(${verbObjectWordGroup('ro', String.raw`[a-zăâîșț][a-zăâîșț0-9-]*`)})${CLAUSE_BOUNDARY.ro}`,
  'gi',
);
...
const HU_EXPERIENCE_IN =
  /\b(?:tapasztalat|gyakorlati tapasztalat|munkatapasztalat)\s+(?:[a-záéíóöőúüű]+\s+)?(?:terén|teren|területén|teruleten|ben|ban)\s+([^,.;\n]+)/gi;

const HU_KNOWLEDGE_OF =
  /\b(?:ismeret|szaktudás|szaktudas|szakértelem|szakertelem)\s+(?:[a-záéíóöőúüű]+\s+)?(?:terén|teren|területén|teruleten|ben|ban|vel|val)\s+([^,.;\n]+)/gi;

const HU_SKILLS_IN =
  /\b(?:jártasság|jartassag|készség|keszseg)\s+(?:[a-záéíóöőúüű]+\s+)?(?:terén|teren|területén|teruleten|ben|ban|vel|val)\s+([^,.;\n]+)/gi;

const HU_EXPERIENCED_ADJ = /\b(?:tapasztalt|jártas|jartas|képzett|kepzett)\s+(?:a|az|ben|ban)\s+([^,.;\n]+)/gi;

const HU_VERB_OBJECT = new RegExp(
  String.raw`\b(?:kezelni|fejleszteni|tervezni|létrehozni|letrehozni|építeni|epiteni|elemezni|megvalósítani|megvalositani|karbantartani|üzemeltetni|uzemeltetni|előkészíteni|elokesziteni|feldolgozni|konfigurálni|konfiguralni|telepíteni|telepiteni|figyelni|tesztelni|támogatni|tamogatni|koordinálni|koordinalni|szervezni|vezetni|felügyelni|felugyelni|végrehajtani|vegrehajtani)\s+(${verbObjectWordGroup('hu', String.raw`[a-záéíóöőúüű][a-záéíóöőúüű0-9-]*`)})${CLAUSE_BOUNDARY.hu}`,
  'gi',
);

const PATTERNS: Record<Locale, Record<RegexPatternGroup, RegExp[]>> = {
  en: {
    linguistic: [
      new RegExp(
        String.raw`\b(?:experience|knowledge|proficiency|expertise|skills?|understanding|familiarity)\s+(?:in|with|of)\s+(.+?)${CLAUSE_BOUNDARY.en}`,
        'gi',
      ),

      new RegExp(
        String.raw`\b(?:experienced|proficient|skilled|familiar)\s+(?:in|with|at)\s+(.+?)${CLAUSE_BOUNDARY.en}`,
        'gi',
      ),
    ],
    ...
    verbObject: [
      new RegExp(
        String.raw`\b(?:manage|develop|design|create|build|analyse|analyze|implement|maintain|operate|prepare|process|configure|install|monitor|test|support|coordinate|organise|organize|plan|lead|supervise|perform)\s+(?:the\s+)?(${verbObjectWordGroup('en', String.raw`[A-Za-z][A-Za-z0-9]*(?:[.+#-][A-Za-z0-9]+)*`)})${CLAUSE_BOUNDARY.en}`,
        'gi',
      ),
    ],
 ...
 */
function splitCandidateList(candidate: string): string[] {
  return candidate
...
export function extractSkillSpans(text: string, locale: Locale): EscoCandidate[] {
  /**
   ...
   */
  const byKey = new Map<string, EscoCandidate>();
    ...
    for (const regex of regexes) {
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        for (const rawItem of splitItems) {
          ...
          const pattern = patternType as PatternGroup;
...
let lexicalIndexPromise: Promise<LexicalIndex> | undefined;
...
function getLexicalIndex(dataDir = 'data'): Promise<LexicalIndex> {
...
function lexicalLanguagesFor(locale?: SupportedLanguage): SupportedLanguage[] {
  if (!locale || locale === 'en') return ['en', 'global'];
  return [...new Set([locale, 'en', 'global'])] as SupportedLanguage[];
}
 ...
 */
const SHAKY_MATCH_CONFIDENCE: Record<Locale, number> = {
  en: 0.4,
  ro: 0.35,
  hu: 0.3,
};
...
function shakyConfidenceFor(locale?: SupportedLanguage): number {
  return SHAKY_MATCH_CONFIDENCE[locale as Locale] ?? SHAKY_MATCH_CONFIDENCE.en;
}
...
async function escoSearch(candidate: EscoCandidate, lexical: LexicalIndex): Promise<EscoMatchResult> {
...
export function previousNonBlankLine(text: string, lineStart: number): string | undefined {
 ...
 */
function looksLikeListItem(text: string, clauseStart: number): boolean {
 ...
 */
function extractLexicalCandidates(text: string, locale: Locale, lexical: LexicalIndex): EscoCandidate[] {
  const langs = lexicalLanguagesFor(locale);
  const candidates: EscoCandidate[] = [];
  let cursor = 0;

  for (const clause of splitClauses(text, 'lexical')) {
    const found = text.indexOf(clause.text, cursor);
    const start = found === -1 ? cursor : found;
    if (found !== -1) cursor = found + clause.text.length;
    ...
    // exists to catch.
    if (!lexical.lookup(clause.text, 'capabilities', langs).length) continue;
...
const SOURCE_PRIORITY: Record<CandidateSource, number> = { list: 2, lexical: 1, pattern: 0 };
...
function dedupeCandidates(candidates: EscoCandidate[]): EscoCandidate[] {
  const byKey = new Map<string, EscoCandidate>();
  for (const candidate of candidates) {
    const key = candidate.normalizedText.toLowerCase();
    const existing = byKey.get(key);
    if (existing) {
      for (const p of candidate.patterns) if (!existing.patterns.includes(p)) existing.patterns.push(p);
      if (SOURCE_PRIORITY[candidate.source] > SOURCE_PRIORITY[existing.source]) existing.source = candidate.source;
      continue;
    }
 ...
 */
export async function processTextForEscoSkills(
  text: string,
  locale: Locale,
  lexical?: LexicalIndex,
  opts?: { debug?: boolean },
