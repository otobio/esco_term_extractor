...
export async function applyAutomatedAliasReview(
  connection: Connection,
  options: ApplyAutomatedAliasReviewOptions = {}
): Promise<ApplyAutomatedAliasReviewResult> {
...
async function readDecisionRows(decisionsPath: string): Promise<DecisionCsvRow[]> {
...
function isApprovedFamilyDecision(row: DecisionCsvRow): boolean {
  return (
    row.decision === 'promote_family' &&
...
function countRowsByRule(rows: DecisionCsvRow[]): Record<string, number> {
