...
function primary(familyNodeId: number, familyLabel: string): FamilySpecializationPrior {
...
const FAMILY_SPECIALIZATION_PRIORS: Partial<Record<LeafSpecializationKind, Record<string, readonly FamilySpecializationPrior[]>>> = {
  population: {
    client: [primary(14965, 'Client information workers')]
  }
};

const FOLDED_TAG_TOKENS: Array<{ kind: LeafSpecializationKind; tag: string; tokens: Set<string> }> = Object.entries(
  FAMILY_SPECIALIZATION_PRIORS
).flatMap(([kind, tagPriors]) =>
  Object.keys(tagPriors ?? {}).map((tag) => ({
    kind: kind as LeafSpecializationKind,
    tag,
    tokens: new Set((ATOMIC_SPECIALIZATION_SYNONYMS[kind as LeafSpecializationKind]?.[tag] ?? []).map((word) => foldSearchText(word)))
  }))
);

export function getFamilySpecializationPriors(foldedTokens: Iterable<string>): readonly FamilySpecializationPrior[] {
  const tokenSet = new Set(foldedTokens);
  const byFamily = new Map<number, FamilySpecializationPrior>();
