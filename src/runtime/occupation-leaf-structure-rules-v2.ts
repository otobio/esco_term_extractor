...
export const LEVEL_SPECIALIZATION_SYNONYMS: Record<LeafLevelKind, string[]> = {
  none: [],
...
const SPECIALIZATION_MARKER_EQUIVALENCE_GROUPS: Partial<Record<LeafSpecializationKind, string[][]>> = {
  venue: [
    ['hotel', 'restaurant'],
    ['hospital', 'clinic']
  ],
...
function contradictionValue(...anchorGroups: string[][]): string[][] {
  return anchorGroups;
}
...
const TRADE_GOODS_DOMAIN_VALUES: string[][][] = [
...
const TECHNICIAN_INDUSTRY_DOMAIN_VALUES: string[][][] = [
...
const SOFTWARE_DEVELOPMENT_DOMAIN_VALUES: string[][][] = [
...
const CUSTOMER_SERVICE_CHANNEL_DOMAIN_VALUES: string[][][] = [
...
function leafMarkerTokensForKind(kind: LeafSpecializationKind, tokens: Set<string>): string[] {
...
const LEAF_LEVEL_CONTRADICTION_GROUPS: LeafLevelKind[][] = [
  ['assistant', 'manager'],
  ['junior', 'manager']
];

// if both leaf and query exists in the pair above then they contradict makes sesne
export function leafLevelKindsContradict(queryLevelKind: LeafLevelKind, leafLevelKind: LeafLevelKind): boolean {
...
// Explicit dimensional profile model. FOLDED_CANONICAL_SPECIALIZATION_CONTRADICTION_GROUPS is the set
// of specialization "dimensions" that matter for contradiction/alignment purposes (software_surface,
// trade_goods_domain, technician_industry_domain, software_development_domain,
// customer_service_channel_domain, ...); a SpecializationDimensionProfile is a leaf's
// or a query's position along every one of those dimensions, built once and then diffed -- rather than
// re-deriving "what does this side match" ad hoc at comparison time.
export type SpecializationDimensionProfile = Map<string, number[]>;

export function buildSpecializationDimensionProfile(tokens: Set<string>): SpecializationDimensionProfile {
  const profile: SpecializationDimensionProfile = new Map();
...
export function leafSpecializationDimensionProfile(canonicalTokens: Set<string>): SpecializationDimensionProfile {
  return buildSpecializationDimensionProfile(canonicalTokens);
}

export function querySpecializationDimensionProfile(preparedQuery: PreparedQuery): SpecializationDimensionProfile {
...
export function compareSpecializationDimensionProfiles(
  leafProfile: SpecializationDimensionProfile,
  queryProfile: SpecializationDimensionProfile
): { contradictionCount: number; alignmentCount: number; unsupportedSpecificityCount: number } {
...
): { contradictionCount: number; alignmentCount: number } {
...
function matchingContradictionValueIndexes(tokens: Set<string>, values: Array<Set<string>[]>): number[] {
...
// query-supported.
const VENUE_IMPLIES_INDUSTRY_CONTEXT: Record<string, string[]> = {
  clinic: ['medical'],
  hospital: ['medical'],
  spital: ['medical'],
  pharmacy: ['medical'],
...
function matchesTokenOrPlural(token: string, candidates: Set<string>): boolean {
