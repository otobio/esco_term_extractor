...
import {
  familyStructureVocabularyMatchesForTokens,
  familyStructureVocabularyValuesByDimension,
  type FamilyStructureVocabularyDimension
} from './occupation-family-structure-vocabulary.js';
...
export type FamilyOccupationLevel =
  | 'military_officer'
  | 'military_non_commissioned'
  | 'military_other_rank'
  | 'executive_manager'
  | 'professional'
  | 'associate_technical'
  ...
  | 'service_sales'
  | 'skilled_trades'
  | 'plant_machine_operator'
  | 'driver_transport'
  | 'elementary';

export type FamilyStructureDimension =
  | 'role_heads'
  | 'knowledge_domains'
  | 'work_objects'
  | 'settings'
...
type RawFamilyStructureRow = readonly [
  familyNodeId: number,
  familyLabel: string,
  occupationLevel: FamilyOccupationLevel,
  roleHeads: string,
...
function parseRawRow(line: string, index: number): RawFamilyStructureRow {
  ...
  return [
    familyNodeId,
    columns[1] ?? '',
    columns[2] as FamilyOccupationLevel,
    columns[3] ?? '',
    columns[4] ?? '',
    columns[5] ?? '',
    columns[6] ?? '',
...
function buildFamilyStructureRules(): readonly FamilyStructureRule[] {
...
export const FAMILY_STRUCTURE_RULES: readonly FamilyStructureRule[] = buildFamilyStructureRules();

export const FAMILY_STRUCTURE_RULE_BY_ID: ReadonlyMap<number, FamilyStructureRule> = new Map(
  FAMILY_STRUCTURE_RULES.map((rule) => [rule.familyNodeId, rule])
);
...
export function buildFamilyStructureQueryProfile(tokens: readonly string[], locale: SupportedQueryLocale = 'en'): FamilyStructureProfile {
  const valuesByDimension = new Map<FamilyStructureVocabularyDimension, Set<string>>();
  ...
  addDimensionValues(
    valuesByDimension,
    'role_heads',
    familyStructureVocabularyMatchesForTokens(tokens, locale)
  ...
  const roleHeads = valuesByDimension.get('role_heads') ?? new Set<string>();
  const authorityBands = valuesByDimension.get('authority_band') ?? new Set<string>();
...
export function buildFamilyStructureProfile(rule: FamilyStructureRule): FamilyStructureProfile {
  const valuesByDimension = new Map<FamilyStructureVocabularyDimension, Set<string>>();
  ...
  addDimensionValues(
    valuesByDimension,
    'role_heads',
    rule.roleHeads.flatMap((value) => [value, ...roleComparableTokens(value)])
  );
  ...
  const vocabularyProfile = familyStructureVocabularyValuesByDimension(
    [
      ...
      ...rule.authorityBand,
      ...rule.activities
    ].flatMap((value) => tokenizeNormalizedText(value)),
    'en',
    { expandLocaleVariants: false }
  );
...
export function compareFamilyStructureToQuery(
  ...
  const roleHeadMissing =
    (roleHeadComparison?.queryValues.length ?? 0) > 0 && (roleHeadComparison?.familyValues.length ?? 0) > 0 && !roleHeadAligned;
  ...
  if (hardRoleHeadMissing) {
    reasons.push(
      `role_heads mismatch: query=[${roleHeadComparison?.queryValues.join(', ') ?? ''}] family=[${
        roleHeadComparison?.familyValues.join(', ') ?? ''
      }]`
    );
  }
  ...
  for (const dimension of dimensions) {
    ...
    if (isHardContradiction(dimension)) {
      reasons.push(
        `${dimension.dimension} contradiction: query=[${dimension.queryValues.join(', ')}] family=[${dimension.familyValues.join(', ')}]`
      );
    }
  }
...
export function shortlistFamilyStructureMatches(
  queryTokens: readonly string[],
  locale: SupportedQueryLocale = 'en',
  options: FamilyStructureShortlistOptions = {}
): FamilyStructureShortlist {
...
function compareFamilyStructureProfiles(
  queryProfile: FamilyStructureProfile,
  familyProfile: FamilyStructureProfile
): FamilyStructureDimensionComparison[] {
  return FAMILY_STRUCTURE_COMPARISON_DIMENSIONS.map((dimension) => {
    ...
    const roleHeadOnlySharesBroadValue =
      dimension === 'role_heads' &&
...
function findRoleAuthorityBridge(dimensions: readonly FamilyStructureDimensionComparison[]): string | null {
  ...
  if (roleHeads.sharedValues.some((value) => BROAD_CONTEXT_BRIDGE_ROLE_HEAD_VALUES.has(value))) {
    ...
    const familyCanBeOperationalManagement =
      roleHeads.familyValues.includes('manager') ||
      roleHeads.familyValues.includes('service manager') ||
      roleHeads.familyValues.some((value) => value.endsWith(' manager'));
    const familyCanBeHealthSupport =
      roleHeads.familyValues.includes('health associate') ||
    ...
    const familyCanBeFoodService =
      roleHeads.familyValues.includes('cook') ||
      roleHeads.familyValues.includes('chef') ||
      dimensions.some(
        (dimension) =>
          dimension.dimension === 'work_objects' && dimension.sharedValues.some((value) => value === 'food_beverage' || value === 'food')
      );
    ...
    const familyCanBeAssistantSupport =
      roleHeads.familyValues.includes('assistant') ||
    ...
    const familyCanBeAssembly =
      roleHeads.familyValues.includes('assembler') || roleHeads.familyValues.some((value) => value.endsWith(' assembler'));
    const queryNamesAssembly = roleHeads.queryValues.includes('assembler') || roleHeads.sharedValues.includes('assembler');
    const assemblyActivityAligned = dimensions.some(
      (dimension) =>
        dimension.dimension === 'activities' &&
        dimension.sharedValues.some((value) => value === 'assemble_make_process' || value === 'assembly')
    );
    const familyCanBeBillingClerk =
      (roleHeads.familyValues.includes('clerk') || roleHeads.familyValues.includes('billing clerk')) &&
      dimensions.some(
        (dimension) =>
          dimension.dimension === 'work_objects' &&
          dimension.sharedValues.some((value) => value === 'money_accounts' || value === 'accounts')
      );
    const familyCanBeCncToolmaking =
      (roleHeads.familyValues.includes('cnc') ||
      ...
      dimensions.some(
        (dimension) => dimension.aligned && dimension.dimension === 'work_objects' && dimension.sharedValues.includes('metal')
      );
    const familyCanBeLogisticsClerk =
      (roleHeads.familyValues.includes('warehouse operator') ||
    ...
    const familyCanBeProfessionalSales =
      (roleHeads.familyValues.includes('sales professional') ||
    ...
    const familyCanBeSalesProfessional =
      (roleHeads.familyValues.includes('sales professional') ||
        roleHeads.familyValues.includes('business developer') ||
        roleHeads.familyValues.includes('marketing professional') ||
    ...
    const familyCanBeClientInformation =
      (roleHeads.familyValues.includes('customer') ||
  ...
  const hasConcreteContextAlignment = dimensions.some(
    (dimension) =>
      dimension.aligned &&
      (dimension.dimension === 'knowledge_domains' ||
        dimension.dimension === 'settings' ||
...
type DimensionValueSide = 'query' | 'family' | 'shared';

type OccupationLevelCompatibilityRule = {
  readonly bridge: string;
  readonly queryLevels: readonly FamilyOccupationLevel[];
  readonly familyLevels: readonly FamilyOccupationLevel[];
  readonly queryRoleValues?: readonly string[];
  readonly queryAuthorityValues?: readonly string[];
  readonly familyRoleValues?: readonly string[];
  readonly familyAuthorityValues?: readonly string[];
  readonly sharedRoleValues?: readonly string[];
  readonly sharedKnowledgeValues?: readonly string[];
  readonly sharedWorkObjectValues?: readonly string[];
  readonly sharedSettingValues?: readonly string[];
  readonly sharedPopulationValues?: readonly string[];
  readonly sharedTransportValues?: readonly string[];
  readonly sharedActivityValues?: readonly string[];
  readonly familyActivityValues?: readonly string[];
  readonly requiresRoleAlignment?: boolean;
  readonly requiresKnowledgeAlignment?: boolean;
};
...
const OCCUPATION_LEVEL_COMPATIBILITY_RULES: readonly OccupationLevelCompatibilityRule[] = [
  {
...
function findOccupationLevelBridge(dimensions: readonly FamilyStructureDimensionComparison[]): string | null {
  ...
  for (const rule of OCCUPATION_LEVEL_COMPATIBILITY_RULES) {
    ...
    if (
      (rule.queryRoleValues || rule.queryAuthorityValues) &&
      !(
        dimensionContainsAny(dimensionValues(dimensions, 'role_heads', 'query'), rule.queryRoleValues ?? []) ||
        dimensionContainsAny(
          dimensionValues(dimensions, 'authority_band', 'query'),
          rule.queryAuthorityValues ?? rule.queryRoleValues ?? []
        )
      )
    ) {
      continue;
    ...
    if (
      (rule.familyRoleValues || rule.familyAuthorityValues) &&
      !(
        dimensionContainsAny(dimensionValues(dimensions, 'role_heads', 'family'), rule.familyRoleValues ?? []) ||
        dimensionContainsAny(
          dimensionValues(dimensions, 'authority_band', 'family'),
          rule.familyAuthorityValues ?? rule.familyRoleValues ?? []
        )
      )
    ) {
      continue;
    ...
    if (
      (rule.sharedKnowledgeValues || rule.sharedWorkObjectValues || rule.sharedSettingValues || rule.sharedActivityValues) &&
      !(
...
function dimensionValues(
  dimensions: readonly FamilyStructureDimensionComparison[],
  dimensionName: FamilyStructureVocabularyDimension,
  side: DimensionValueSide
): readonly string[] {
...
function dimensionContainsAny(values: readonly string[], expected: readonly string[]): boolean {
  return expected.length > 0 && expected.some((value) => values.includes(value));
}

function findTransportModeBridge(dimensions: readonly FamilyStructureDimensionComparison[]): string | null {
  ...
  const materialHandlingAligned = dimensions.some(
    (dimension) =>
      (dimension.dimension === 'work_objects' || dimension.dimension === 'activities' || dimension.dimension === 'role_heads') &&
      dimension.aligned &&
      dimension.sharedValues.some(
        (value) => value === 'goods_products' || value === 'materials' || value === 'clean_prepare_handle' || value === 'labourer'
      )
  );
  ...
  const callCentreTravelAligned = dimensions.some(
    (dimension) =>
      dimension.dimension === 'population_or_channel' &&
...
function applyDimensionBridges(
  dimensions: readonly FamilyStructureDimensionComparison[],
  bridges: {
    readonly roleHeads: string | null;
    readonly occupationLevel: string | null;
    readonly transportMode: string | null;
  }
): FamilyStructureDimensionComparison[] {
  return dimensions.map((dimension) => {
    const bridgeValue =
      dimension.dimension === 'role_heads'
        ? bridges.roleHeads
...
function addDimensionValues(
  valuesByDimension: Map<FamilyStructureVocabularyDimension, Set<string>>,
...
const FAMILY_STRUCTURE_COMPARISON_DIMENSIONS: readonly FamilyStructureVocabularyDimension[] = [
  'occupation_level',
  'role_heads',
  'knowledge_domains',
  'work_objects',
...
const HARD_REJECTION_DIMENSIONS = new Set<FamilyStructureVocabularyDimension>(['occupation_level', 'transport_mode']);

const FAMILY_STRUCTURE_DIMENSION_SUPPORT_WEIGHT: Record<FamilyStructureVocabularyDimension, number> = {
  occupation_level: 5,
  role_heads: 8,
  knowledge_domains: 3,
  work_objects: 3,
...
function isHardContradiction(dimension: FamilyStructureDimensionComparison): boolean {
...
function dimensionHasQueryValue(dimension: FamilyStructureDimensionComparison, value: string): boolean {
  return dimension.queryValues.includes(value);
}

const BROAD_QUERY_ROLE_HEAD_VALUES = new Set<string>([
  'assistant',
  'asistent',
  'asistenta',
  'asistentă',
...
const BROAD_SHARED_ROLE_HEAD_ALIGNMENT_VALUES = new Set<string>(['manager', 'supervisor']);
const BROAD_CONTEXT_BRIDGE_ROLE_HEAD_VALUES = new Set<string>(['labourer', 'worker', 'service_worker']);

function hasHardRoleHeadMismatchEvidence(dimensions: readonly FamilyStructureDimensionComparison[]): boolean {
  const roleHeads = dimensions.find((dimension) => dimension.dimension === 'role_heads');
  if (!roleHeads?.contradicted) {
    return false;
  }
...
export function familyStructureSupportScore(comparison: FamilyStructureComparison): number {
  let score = 0;
...
function compareShortlistCandidates(left: FamilyStructureShortlistCandidate, right: FamilyStructureShortlistCandidate): number {
...
function roleComparableTokens(value: string): string[] {
...
export function validateFamilyStructureRules(): void {
  ...
  for (const rule of FAMILY_STRUCTURE_RULES) {
    ...
    if (expectedLabel !== rule.familyLabel) {
      throw new Error(
        `Family structure rule label mismatch for family id ${rule.familyNodeId}: expected "${expectedLabel}", got "${rule.familyLabel}".`
      );
    }
