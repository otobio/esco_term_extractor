...
type QueryKind = 'title' | 'description' | 'keyword' | 'mixed';
type ExpectationLevel = 'exact_leaf' | 'acceptable_leaf' | 'family' | 'group';
type NodeLevel = 'occupation' | 'family' | 'group';
...
type EvaluationExpectationSeed = {
  level: ExpectationLevel;
  target: EvaluationTarget;
  rankCeiling: number;
};

type EvaluationQuerySeed = {
  caseKey: string;
  category:
    | 'exact_english'
    | 'english_alias'
  ...
  queryKind: QueryKind;
  description: string;
  expectations: EvaluationExpectationSeed[];
};

type ResolvedExpectationSeed = EvaluationExpectationSeed & {
  expectedNodeId: number;
};

type ResolvedEvaluationQuerySeed = EvaluationQuerySeed & {
  normalizedQuery: string;
  resolvedExpectations: ResolvedExpectationSeed[];
};
...
const PHASE8_CORE_EVALUATION_SET: EvaluationQuerySeed[] = [
  {
...
const PHASE15_EXPANDED_EVALUATION_SET: EvaluationQuerySeed[] = [
...
const EVALUATION_SET_FIXTURES: Record<string, EvaluationQuerySeed[]> = {
...
export class EvaluationSetSeeder {
  ...
  public async run(options: SeedEvaluationSetOptions = {}): Promise<SeedEvaluationSetResult> {
  ...
  private async resolveSeeds(sourceName: string, seeds: EvaluationQuerySeed[]): Promise<ResolvedEvaluationQuerySeed[]> {
    const resolvedSeeds: ResolvedEvaluationQuerySeed[] = [];

    for (const seed of seeds) {
      const resolvedExpectations: ResolvedExpectationSeed[] = [];
  ...
  private async resolveTarget(sourceName: string, target: EvaluationTarget, expectationLevel: ExpectationLevel): Promise<GraphNodeRow> {
  ...
  private async resolveCanonicalLabel(sourceName: string, canonicalLabel: string, nodeLevel: NodeLevel): Promise<GraphNodeRow> {
  ...
  private async resolveAlias(sourceName: string, target: Extract<EvaluationTarget, { kind: 'alias' }>): Promise<GraphNodeRow> {
  ...
  private async resolveHierarchyTarget(
    sourceName: string,
    leafCanonicalLabel: string,
    hierarchyLevel: 'family' | 'group'
  ...
  private async resetSet(
    sourceName: string,
    setKey: string,
    seeds: ResolvedEvaluationQuerySeed[]
  ): Promise<{ deletedQueryCount: number; deletedExpectationCount: number }> {
  ...
  private async deleteOwnedExpectations(sourceName: string, setKey: string): Promise<number> {
  ...
  private async deleteOwnedQueries(sourceName: string, setKey: string): Promise<number> {
    const [result] = await this.connection.execute<ResultSetHeader>(
      `
        DELETE query
  ...
  private async deleteMatchingExpectation(queryIds: number[], expectation: ResolvedExpectationSeed): Promise<number> {
  ...
  private async upsertEvaluationQuery(
    sourceName: string,
    setKey: string,
    seed: ResolvedEvaluationQuerySeed
  ): Promise<{ queryId: number; inserted: boolean }> {
  ...
  private async findExistingQuery(
    ...
    setKey: string,
    seed: ResolvedEvaluationQuerySeed
  ): Promise<EvaluationQueryRow | null> {
  ...
  private async insertExpectationIfMissing(queryId: number, expectation: ResolvedExpectationSeed): Promise<boolean> {
    const [existingRows] = await this.connection.query<EvaluationExpectationRow[]>(
      `
        SELECT id
...
export function defaultEvaluationSetKey(): string {
  return DEFAULT_SET_KEY;
}

export function evaluationSetSize(setKey: string = DEFAULT_SET_KEY): number {
  return evaluationSetSeeds(setKey).length;
}

function exactLeafByLabel(canonicalLabel: string, rankCeiling: number): EvaluationExpectationSeed {
  return {
    level: 'exact_leaf',
    target: {
      kind: 'canonical_label',
...
function acceptableLeafByLabel(canonicalLabel: string, rankCeiling: number): EvaluationExpectationSeed {
  return {
...
function familyByLabel(canonicalLabel: string, rankCeiling: number): EvaluationExpectationSeed {
  return {
    level: 'family',
    target: {
      kind: 'canonical_label',
...
function evaluationSetSeeds(setKey: string): EvaluationQuerySeed[] {
...
function familyOfLeaf(leafCanonicalLabel: string, rankCeiling: number): EvaluationExpectationSeed {
  return {
    level: 'family',
    target: {
      kind: 'family_of',
...
function groupOfLeaf(leafCanonicalLabel: string, rankCeiling: number): EvaluationExpectationSeed {
  return {
    level: 'group',
    target: {
      kind: 'group_of',
...
function assertExpectationLevelMatchesNode(expectationLevel: ExpectationLevel, node: GraphNodeRow, targetLabel: string): void {
  const expectedNodeLevelByExpectation: Record<ExpectationLevel, NodeLevel> = {
  ...
  if (node.node_level !== expectedNodeLevel) {
    throw new Error(
      `Resolved target "${targetLabel}" to node "${node.canonical_label}" at level "${node.node_level}", but expectation_level="${expectationLevel}" requires node_level="${expectedNodeLevel}".`
    );
  }
}
...
function buildNotes(sourceName: string, setKey: string, seed: EvaluationQuerySeed): string {
...
async function countOwnedQueries(connection: Connection, sourceName: string, setKey: string): Promise<number> {
...
export async function countSeededEvaluationQueries(
  connection: Connection,
  options: Pick<SeedEvaluationSetOptions, 'sourceName' | 'setKey'> = {}
): Promise<number> {
