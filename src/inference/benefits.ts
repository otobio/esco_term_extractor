...
const RULES: readonly IdiomRule[] = [...RO, ...HU, ...ET, ...EN];

export function inferBenefits(clauses: Clause[]): InferredTerm[] {
  const { add, terms } = collector();
  for (const c of clauses) applyIdioms(normalizeLoose(c.text), c.text, RULES, add);
  return terms();
}
