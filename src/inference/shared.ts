...
export function applyIdioms(loose: string, rawClause: string, rules: readonly IdiomRule[], add: Add): void {
  for (const r of rules) {
    const m = r.re.exec(loose);
    if (m && !isNegated(rawClause, m[0])) add(r.key, r.score, rawClause);
  }
}

export function collector(): { add: Add; terms: () => InferredTerm[] } {
  const found = new Map<string, InferredTerm>();
  const add: Add = (key, score, evidence) => {
    const prev = found.get(key);
    if (!prev || score > prev.score) found.set(key, { canonicalKey: key, score, evidence });
  };
...
export function parseWeeklyHours(loose: string, L: Locale): number | null {
...
export function parseDailyHours(loose: string, L: Locale): number | null {
...
export function parseExperienceYears(loose: string, L: Locale): number | null {
  const Y = alt(L.years);
  const E = alt(L.experience);
  const m =
    new RegExp(`(\\d{1,2})\\s*-\\s*\\d{1,2}\\s*\\+?\\s*${Y}\\b[^.]{0,15}?${E}`, 'i').exec(loose) ||
...
function clampYears(s: string): number | null {
  const n = Number(s);
...
export function parseTimeRanges(loose: string, L: Locale): TimeRange[] {
  ...
  for (const m of loose.matchAll(re)) {
    ...
    if (!hasColon && !cue) continue;
    const start = Number(m[1]);
    const end = Number(m[3]);
    if (start > 24 || end > 24) continue;
    out.push({ start, end });
  }
  return out;
}
