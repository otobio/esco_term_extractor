...
const ALL_LANGS: readonly SupportedLanguage[] = ['en', 'ro', 'hu', 'et'];

type WordMap = Partial<Record<SupportedLanguage, string[]>>;
...
interface CompiledFamily {
  head: RegExp;
  modifiers: readonly CompiledModifier[];
  defaultKey?: string;
  defaultScore?: number;
}

/** Non-capturing alternation of literal words/phrases, boundary-anchored on both sides. */
function wordsRe(words: string[], flags: string): RegExp {
  return new RegExp(`\\b(?:${words.join('|')})\\b`, flags);
}

function compileFamilies(families: readonly Family[]): Partial<Record<SupportedLanguage, CompiledFamily[]>> {
  const out: Partial<Record<SupportedLanguage, CompiledFamily[]>> = {};
  for (const lang of ALL_LANGS) {
    const compiled: CompiledFamily[] = [];
    for (const family of families) {
      ...
      for (const m of family.modifiers) {
        ...
        // assignment below.
        if (words?.length) modifiers.push({ key: m.key, score: m.score, re: wordsRe(words, 'gi') });
      }
    ...
    if (compiled.length) out[lang] = compiled;
  }
  return out;
}
...
function deriveVariations(
  ...
  compiled: Partial<Record<SupportedLanguage, CompiledFamily[]>>,
): InferredTerm[] {
  ...
  for (const c of clauses) {
    ...
    for (const lang of langs) {
      for (const family of compiled[lang] ?? []) {
        ...
        const modHits: ModHit[] = [];
        for (const m of family.modifiers) {
          for (const mm of loose.matchAll(m.re)) modHits.push({ index: mm.index, key: m.key, score: m.score });
        }

        const assignedMod = new Map<number, ModHit>(); // head array index -> its modifier
        if (modHits.length) {
          const pairs: { h: number; m: number; dist: number }[] = [];
          heads.forEach((h, hi) => {
            modHits.forEach((mh, mi) => {
              const dist = Math.abs(mh.index - h.index);
              if (dist <= MAX_PAIR_DISTANCE) pairs.push({ h: hi, m: mi, dist });
            });
          });
          pairs.sort((a, b) => a.dist - b.dist);
          const usedMod = new Set<number>();
          for (const p of pairs) {
            if (assignedMod.has(p.h) || usedMod.has(p.m)) continue;
        ...
        heads.forEach((h, hi) => {
          if (isNegated(c.text, h[0])) return;
          const hit = assignedMod.get(hi);
          if (hit) add(hit.key, hit.score, c.text);
          else if (family.defaultKey) add(family.defaultKey, family.defaultScore ?? 0.75, c.text);
        });
      }
    }
  }
...
const BENEFIT_FAMILIES: readonly Family[] = [
  // -------------------------------------------------------------
    ...
    modifiers: [
      ...
      },
      {
        ...
        words: {
          ...
          ro: ['privat', 'privata', 'abonament', 'medical', 'medicala', 'stomatologie', 'oftalmologie'],
          hu: ['magan', 'orvosi', 'egeszseg', 'fogaszat'],
        },
      },
    ],
  ...
  {
    head: {
      en: ['leave', 'vacation', 'holiday', 'days off', 'time off', 'pto', 'annual leave', 'days of annual leave'],
    ...
    modifiers: [
      {
        ...
        score: 0.85,
        words: {
          en: ['sick', 'medical', 'annual'],
          ro: ['medical', 'boala', 'anual'],
          hu: ['beteg', 'eves'],
          et: ['haigus', 'aastane'],
        },
      },
    ...
    modifiers: [
      {
        ...
        words: {
          ...
          ro: ['transport', 'naveta', 'combustibil', 'auto', 'parcare', 'abonament metrou'],
          hu: ['utazasi', 'bejarasi', 'munkaba jaras', 'uzemanyag', 'auto', 'parkolas'],
          et: ['auto', 'parkimine'],
        },
      },
      {
        ...
        score: 0.85,
        words: {
          en: ['wellness', 'wellbeing', 'mental health', 'health program', 'fitness', 'gym', 'sports'],
          ro: ['wellness', 'sanatate mentala', 'sala', 'fitness'],
          hu: ['wellness', 'egeszseg', 'mentalis', 'edzoterem', 'fitnesz', 'sport', 'sport tamogatas'],
          et: ['wellness', 'vaimne tervis', 'sport'],
        },
      },
      {
  ...
  {
    head: {
      en: ['discount', 'discounts', 'discounted', 'staff discount', 'employee discount'],
      ro: ['discount', 'reducere', 'discount angajati', 'reducere angajati'],
      hu: ['kedvezmeny', 'dolgozoi kedvezmeny', 'kedvezmenyes'],
      et: ['soodustus', 'soodustused'],
    },
    modifiers: [
      {
        key: 'benefits:employee_discount',
        score: 0.85,
        words: {
          en: ['accommodation', 'hotel', 'food', 'beverage', 'drinks', 'friends', 'family', 'staff'],
          ro: ['cazare', 'mancare', 'bauturi', 'prieteni', 'familie'],
          hu: ['szallas', 'etel', 'ital', 'csalad', 'baratok'],
        },
      },
    ],
  ...
  },
  {
    head: {
      ...
      hu: ['etkezesi', 'ebed', 'szep kartya', 'szép kártya', 'melegetkezes', 'uzemi etkezes'],
      et: ['toitlustus', 'lunch', 'soojustoidu toetus'],
    },
    modifiers: [],
    ...
    modifiers: [
      ...
      },
      {
        ...
        words: {
          ...
          hu: ['wellness keret', 'home office tamogatas', 'eletmod tamogatas', 'kepzesi keret', 'internet tamogatas'],
          et: ['sporditoetus', 'kodukontori eelarve', 'wellness toetus', 'koolituskrediit', 'internetitoetus'],
        },
      },
      {
...
export function deriveBenefitVariations(clauses: Clause[], languages?: SupportedLanguage[]): InferredTerm[] {
...
const COMPENSATION_FAMILIES: readonly Family[] = [
  ...
  {
    head: {
      en: ['equity', 'ownership stake'],
      ro: ['actiuni', 'participare la actiuni'],
      hu: ['reszveny', 'tulajdonresz'],
      et: ['aktsiad', 'osalus'],
    },
    modifiers: [],
  ...
  },
  {
    head: {
      en: ['stock options'],
      ro: ['optiuni pe actiuni'],
      hu: ['reszvenyopcio', 'opcios reszveny'],
      et: ['aktsiaoptsioon'],
    },
    modifiers: [],
  ...
  {
    head: {
      en: ['rsu', 'rsus', 'restricted stock'],
      ro: ['actiuni restrictionate'],
      hu: ['korlatozott reszveny'],
      et: ['piiratud aktsiad'],
    },
    modifiers: [],
  ...
  },
  {
    ...
    head: {
      en: ['phantom stock', 'phantom shares', 'phantom equity', 'stock appreciation rights'],
      ro: ['actiuni fantoma'],
      hu: ['fantom reszveny'],
      et: ['fantoomaktsiad'],
    },
  ...
  },
  {
    head: {
      ...
      ro: [
        ...
        'al 13 lea salariu',
        'al 13-lea salariu',
        'al 13 lea si al 14 lea salariu',
        'salarii duble',
        'salariu dublu',
      ],
      hu: ['14 havi fizetes', '13 havi fizetes'],
      et: ['14 palk', '13 palk'],
    },
    modifiers: [],
  ...
  {
    head: {
      en: ['vacation bonus', 'holiday bonus', 'annual leave bonus'],
      ro: ['prima de vacanta', 'indemnizatie de vacanta'],
      hu: ['szabadsagi penz', 'uzemi szabadsag'],
      et: ['puhkusetoetus', 'puhkuseraha'],
    },
    modifiers: [],
    defaultKey: 'compensation:vacation_bonus',
    defaultScore: 0.8,
  },
];

export function deriveCompensationVariations(clauses: Clause[], languages?: SupportedLanguage[]): InferredTerm[] {
