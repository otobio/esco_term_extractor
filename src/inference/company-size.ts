...
interface Loc {
  startup: string[];
  scaleup: string[]; // includes SME/mid
  enterprise: string[];
  employee: string[];
...
const STAGE_DISQUALIFIER =
  /\b(software|architecture|account|accounts|client|clients|customer|customers|solution|solutions|application|applications|system|systems|segment|sector|sales|market|markets|mindset|culture|environment|vibe|spirit|mentality|resource|grade|level|agreement|deal|deals)\b/;
...
const parseNum = (s: string) => Number(s.replace(/[.,\s]/g, ''));
    ...
    for (const L of LOCS) {
      ...
      for (const [key, words] of [
        ...
        for (const w of words) {
          ...
          for (const m of loose.matchAll(re)) {
            ...
            if (isNegated(c.text, w)) continue;
            add(key, 0.9, c.text);
            break;
          }
        }
      ...
      for (const re of patterns) {
        for (const m of loose.matchAll(re)) {
          ...
          if (L.exclude.some((w) => hasWord(window, w))) continue;
          const n = parseNum(m[1]);
          if (n >= 1 && n <= 2_000_000) add(stageFromCount(n), 0.8, c.text);
        }
      }
    }
  }
