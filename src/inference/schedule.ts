...
const HU: IdiomRule[] = [
  ...
  { key: 'schedule:rotational_shift', score: 0.85, re: /\b(tobb muszak|valto muszak|muszakos)\b/ },
...
export function inferSchedule(clauses: Clause[], languages?: SupportedLanguage[]): InferredTerm[] {
  ...
  for (const c of clauses) {
    ...
    for (const L of LOCALES) {
      for (const { start, end } of parseTimeRanges(loose, L)) {
        ...
        if (overnight) add('schedule:night_shift', 0.85, c.text);
        else if (daytime) add('schedule:9_to_5', 0.75, c.text);
      }
    }
  }
  return terms();
