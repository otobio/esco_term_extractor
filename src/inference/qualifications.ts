...
const LANG_NAMES: [RegExp, string][] = [
  ...
  for (const c of clauses) {
    ...
    for (const [nameRe, key] of LANG_NAMES) {
      ...
      while ((m = g.exec(loose))) {
        ...
        if ((options?.titleMode || hasCtx(window)) && !isNegated(c.text, m[0])) add(key, 0.85, c.text);
      }
    }

    for (const [re, key] of PHRASE_RULES) {
      const m = re.exec(loose);
      if (m && !isNegated(c.text, m[0])) add(key, 0.85, c.text);
    }
  }
