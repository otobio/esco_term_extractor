...
interface IndexMeta {
  ...
  count: number;
  terms: StoredTerm[];
  /** ranges[bucket][language] = [startIndex, endIndex) into `terms` / vectors. */
  ranges: Record<string, Record<string, [number, number]>>;
  /** Per-term similarity to the global centroid (hubness bias); aligned to terms. */
  bias?: number[];
}
...
export class VectorStore {
  ...
  private readonly byKey: Map<string, number[]>;
  ...
  private constructor(
    ...
    for (let i = 0; i < terms.length; i++) {
      ...
      if (list) list.push(i);
      else this.byKey.set(terms[i].canonicalKey, [i]);
    }
  }

  static async load(dir: string): Promise<VectorStore> {
    const meta: IndexMeta = JSON.parse(await readFile(join(dir, META_FILE), 'utf8'));
    if (meta.schemaVersion !== undefined && meta.schemaVersion !== INDEX_SCHEMA_VERSION) {
      throw new Error(
        `Index schema version ${meta.schemaVersion} is incompatible with this build ` +
  ...
  buckets(): BucketName[] {
    return Object.keys(this.ranges) as BucketName[];
  }
  ...
  searchBest(query: Float32Array, bucket: BucketName, languages?: SupportedLanguage[]): BestMatch | null {
    ...
    const langs = languages?.length ? languages : (Object.keys(byLang) as SupportedLanguage[]);
    const dim = this.dim;
    ...
    for (const lang of langs) {
      ...
      for (let i = start; i < end; i++) {
        const base = i * dim;
        let dot = 0;
        for (let d = 0; d < dim; d++) dot += query[d] * vecs[base + d];
  ...
  searchTopK(query: Float32Array, bucket: BucketName, k: number, languages?: SupportedLanguage[]): BestMatch[] {
    ...
    if (!byLang) return [];
    const langs = languages?.length ? languages : (Object.keys(byLang) as SupportedLanguage[]);
    const bestByKey = new Map<string, BestMatch>();
    for (const lang of langs) {
      const range = byLang[lang];
      if (!range) continue;
      for (let i = range[0]; i < range[1]; i++) {
        ...
        if (!prev || score > prev.score) bestByKey.set(key, { index: i, score });
      }
    }
    return [...bestByKey.values()].sort((a, b) => b.score - a.score).slice(0, k);
  }

  private dot(query: Float32Array, index: number): number {
    const base = index * this.dim;
    let d = 0;
    for (let k = 0; k < this.dim; k++) d += query[k] * this.vectors[base + k];
    return d;
  }
  ...
  similarityTo(query: Float32Array, canonicalKey: string, language?: SupportedLanguage): number | null {
    ...
    let best = -Infinity;
    for (const i of idxs) {
      if (language && this.terms[i].languageCode !== language) continue;
  ...
  static computeBias(dim: number, vectors: Float32Array, count: number): Float32Array {
    ...
    for (let i = 0; i < count; i++) {
      const base = i * dim;
      for (let d = 0; d < dim; d++) centroid[d] += vectors[base + d];
    }
    let norm = 0;
    for (let d = 0; d < dim; d++) {
      centroid[d] /= count;
    ...
    for (let i = 0; i < count; i++) {
      const base = i * dim;
      let dot = 0;
      for (let d = 0; d < dim; d++) dot += vectors[base + d] * centroid[d];
      bias[i] = dot;
    }
    return bias;
  }
  ...
  private static buildArrays(
    dim: number,
    entries: { term: DictionaryTerm; vector: Float32Array }[],
  ): { count: number; vectors: Float32Array; terms: StoredTerm[]; ranges: IndexMeta['ranges']; bias: Float32Array } {
    // Group by (bucket, language) so ranges are contiguous.
    entries.sort((a, b) => {
      if (a.term.bucket !== b.term.bucket) return a.term.bucket < b.term.bucket ? -1 : 1;
      if (a.term.languageCode !== b.term.languageCode) return a.term.languageCode < b.term.languageCode ? -1 : 1;
      return 0;
    });

    const count = entries.length;
    const vectors = new Float32Array(count * dim);
    const terms: StoredTerm[] = new Array(count);
    const ranges: IndexMeta['ranges'] = {};
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!r) byLang[term.languageCode] = [i, i + 1];
      else r[1] = i + 1;
    }
